...
`en.ts` だけ直すと、コンポーネント側に旧語彙が残る。**新しい文字列を足すときも 5 系統のどれに置くかを決める**。
...
| 5 | 歳時記プレビュー | production source の `effectEn` / `exampleEn`。英語側だけを lint する | 102 項目（2026-08-23） |

**新規は原則 系統 1（`en.ts`）に置く。** 系統 2・3 は既存分だけ。系統 4 はモデル情報カードの
日英同時表示を保つ場合、系統 5 は歳時記プレビューの対訳を持つ場合に限る。鍵を足したら `ja.ts` にも同じ鍵を足す
