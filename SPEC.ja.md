...
6. **文章は正規化されたDDLに変換され、Typed Compilerで検証される** — 入力は自由にすることが可能だが、DDLには明確な形式とルールがある。完全な自由形式はユーザーを圧倒する。適度な構造が創造を助ける
...
決定的な層については、Rsutによる単一パッケージとし、linux container / ios / Androidで同じ実装を共有する。
これは、複雑な描画コアのマルチプラットフォーム対応には、共有コンポーネント以外のアプローチが非現実的であることが、
Androidへのテスト実装を通じて判明したためである。
