...
**Target version: v1.94.0 / Build 599**
...
v1.93 (Build 593) adds the RAW trace option. `include_trace` (default false) on `/api/paint` and `/api/compose` returns each layer's intermediate output in a single response; it is observation-only, changes no Score/render/branch/count, and is never persisted (the entry point for the intent-audit harness).
