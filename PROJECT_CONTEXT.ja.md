...
直近のv1.89 / Build 580では、作品メニューから手動推敲を除き、選択作品を対象に描画要素・モデル・言語の比較を開けるようにした。系譜タブで下部履歴を選んでもタブを維持して対象作品を中心表示し、履歴ストリップは処理秒数の代わりに系譜の世代数と状態を示す。ゴミ箱操作は高コントラストの明示ラベルとした。
