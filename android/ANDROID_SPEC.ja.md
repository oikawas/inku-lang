...
進行表示は実際の処理状態に追随する。写真の準備、端末内モデルの準備、写真内容の確認、構図の検討、色と形の組立て、描画の仕上げ、作品保存を区別し、その段階で行っていることを短い文で伝える。写真を用いる演出は表示専用のプレビューへ適用し、入力写真や保存写真を書き換えない。構図・色・輪郭などの演出は処理段階を表すものであり、未完成の描画結果とは扱わない。架空の中間SVG、進捗率、残り時間、演出のための待ち時間を加えず、Direct DDLで省略されるStage 1を表示しない。システムのアニメーション無効時は同じ段階を静止表示し、実際の保存完了後に作品を見せる。
