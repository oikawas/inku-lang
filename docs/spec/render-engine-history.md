...
| `ddl_engine_version` | deterministic transforms (expansion, coerce, validator) | `10` | the same input and seed produce different output, **or the declaration order of `Instruction`'s fields changes** |
...
| Deterministic DDL layers | `server/reference/ddl-engine-10/` | **A** = expanded DDL from `expand_intermediate_ddl` / **B** = coerced Score plus `branch_report` from `coerce_score` | 34 (A 13 / B 21) |
