...
_render_effect_hint = _planning._render_effect_hint
...
_norm_label = _planning._norm_label
...
def render(
    ...
    *,
    ...
    if ground_defs_xml or surface_filter_xml or solid_mottle_filter_xml or performance_filter_xml:
        ...
        if "<defs />" in svg:
            svg = svg.replace("<defs />", f"<defs>{extra_defs_xml}</defs>", 1)
        elif "<defs/>" in svg:
            svg = svg.replace("<defs/>", f"<defs>{extra_defs_xml}</defs>", 1)
        else:
            svg = svg.replace("<defs>", f"<defs>{extra_defs_xml}", 1)
