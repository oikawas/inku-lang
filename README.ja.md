...
夜の水面に、青い線がゆっくりほどける
```
inkuアプリケーションの入り口は、まず上のような短い詩歌・散文を書く事からはじまります。言葉は、LLMによって絵画要素に分解され、絵画言語DDLに書き起こされます。DDLはTyped Compilerによって、SVG描画が可能なJSONデータにCompileされます。
...
```
あなたの一文（母語で書く）
     │  解釈 — 言葉を歳時記の語による絵画要素に読み解く（Stage 1・LLM）
     ▼
指示書（正規化DDL — 読み解いた要素から決定的に書き起こした、人間が読める実行仕様）
...
解釈と構造化を分けるのは、求められる能力が違うからです。解釈は連想的・創造的なので LLM に任せます。ただし LLM は指示書の文章を直接は書きません。どの形を、どの画材・色で、どこにどう置くかを、歳時記の語だけで決まった書式に埋めて返し、指示書の文章はそこから決定的に書き起こされます。構造化は機械的・規則遵守的なので、LLM ではなく型付きコンパイラが担います。意味を推測で埋めることはせず、読み切れない句は、その範囲だけを LLM に補完させて、あなたの承認を待ちます。解釈に使うモデルは選択でき、モデルによって出来上がる作品には大きな変化が見られます。**モデルの選択そのものが、創造的な変数です。**
...
- **多段パイプライン** — 写生と色カタログの自動選択（どちらも任意）・Stage 1（言葉を絵画要素に読み解く）・型付きコンパイラ・Stage 1.5（焦点と変奏）・Renderer。AIによる非決定論的な層と、アルゴリズムによる決定論的な層が交互になっています
...
- **プラグイン** — `Nature.YoungLeaves`（別名 `Nature.若葉`）のような名前空間付きの語彙マクロ。コア語彙への展開のみが許され、コアは変更できません。記述に対象の語があれば、Stage 1 の解釈でも使われます。未登録のプラグインを含む指示書は、その文だけ理由を示して残りを描きます。プラグインの定義を付けて指示書を書き出し、別の環境で読み込めます
