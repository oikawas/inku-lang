...
Primitive-only direct coordinated groups with `place:center` reach Score 0.7.0 `placement_groups`. Omitted internal placement uses `overlap` to align member bounding-box centers; “place in a row” uses source-order `horizontal_source_order`; “overlap” uses `overlap`. One named region resolves once from the performance seed and moves the whole group. Member owners, counts, seeds, and geometry remain intact, and repetition remains a symbolic plan. This direct carrier adds neither a Macro authoring operator nor instance materialization.
