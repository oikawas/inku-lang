	...
	function saijikiPreview(categoryKey: string, canonicalWord: string, word: string, wordLang: ResolvedInstructionLang): SaijikiPreview {
		...
		const touchSvg = (kind: string) => {
			...
			const paths: Record<string, string> = {
				...
				oil_paint: '<path d="M20 58 C47 20 106 72 160 34 C132 62 65 56 20 58 Z" fill="#2b2b2b" opacity="0.9"/><path d="M27 56 C64 37 111 59 154 39" fill="none" stroke="#fffdf8" stroke-width="2" opacity="0.42"/><path d="M31 60 C68 44 112 63 151 45" fill="none" stroke="#8d5a42" stroke-width="1.4" opacity="0.52"/>',
		...
		const previews: Record<string, PreviewEntry> = {
			...
			油彩: { effect: '厚みのある色層と筆筋を重ねる。', example: '油彩の赤い線を引く', effectEn: 'Layers a dense paint body with visible brush streaks.', exampleEn: 'Draw a red oil paint line', svg: touchSvg('oil_paint') },
			...
			速く: { effect: '揺れの周期を短くする。', example: '速く揺れる線', effectEn: 'Shortens the period of the sway.', exampleEn: 'A quickly swaying line', svg: shapeSvg('<path d="M20 48 L32 40 L44 56 L56 40 L68 56 L80 40 L92 56 L104 40 L116 56 L128 40 L140 56 L158 48" fill="none" stroke="#2b2b2b" stroke-width="4" stroke-linecap="round"/>') },
			揺れる: { effect: '線や輪郭に連続した揺れを与える。', example: '揺れる線', effectEn: 'Gives lines and contours a continuous sway.', exampleEn: 'A swaying line', svg: lineSvg() },
			...
			にじみ: { effect: '墨の縁から不均一な広がりを加える。', example: 'にじみの黒い円を置く。', effectEn: 'Adds irregular spread at the edges of the ink.', exampleEn: 'Place a bleeding black circle.', svg: shapeSvg('<defs><filter id="pblur"><feGaussianBlur stdDeviation="3"/></filter></defs><circle cx="90" cy="46" r="24" fill="#2b2b2b" opacity="0.72" filter="url(#pblur)"/><circle cx="90" cy="46" r="20" fill="#2b2b2b" opacity="0.62"/>') },
			...
			中心: { effect: '画面の中心へ配置する。', example: '中心に円を置く', effectEn: 'Places at the center of the frame.', exampleEn: 'Place a circle at center', svg: shapeSvg('<circle cx="90" cy="46" r="16" fill="#2b2b2b"/>') },
			...
			始点: { effect: '線または弧の始点を選ぶ。', example: '前の線の始点につながる', effectEn: 'Selects the start of a line or arc.', exampleEn: 'Place a blue arc connected to the start of the previous line', svg: shapeSvg('<path d="M35 62 H145" stroke="#2b2b2b" stroke-width="5" stroke-linecap="round"/><circle cx="35" cy="62" r="6" fill="#2b2b2b"/>') },
			終点: { effect: '線または弧の終点を選ぶ。', example: '前の弧の終点につながる', effectEn: 'Selects the end of a line or arc.', exampleEn: 'Place a blue line connected to the end of the previous arc', svg: shapeSvg('<path d="M35 62 H145" stroke="#2b2b2b" stroke-width="5" stroke-linecap="round"/><circle cx="145" cy="62" r="6" fill="#2b2b2b"/>') },
			途中: { effect: '線や弧の両端以外につなぐ。接ぐ位置は演奏で決まる。', example: '赤い線を置く。前の線の途中につながる小さな青い弧を置く。', effectEn: 'Connects along a line or arc, excluding both ends. The position is decided during performance.', exampleEn: 'Place a red line. Place a small blue arc connected partway along the previous line.', svg: shapeSvg('<path d="M30 62 H150" fill="none" stroke="#2b2b2b" stroke-width="5" stroke-linecap="round"/><path d="M72 62 Q94 20 116 46" fill="none" stroke="#2c5fb8" stroke-width="5" stroke-linecap="round"/>') },
			...
			置く: { effect: '指定した場所に一つ置く。', example: '中心に円を置く', effectEn: 'Places one element at the given location.', exampleEn: 'Place a circle at center', svg: shapeSvg('<circle cx="90" cy="46" r="22" fill="#2b2b2b"/>') },
			並べる: { effect: '図形や組を列として並べる。', example: '円を横に並べる', effectEn: 'Lines shapes or groups up as a row.', exampleEn: 'Line circles up horizontally', svg: shapeSvg('<circle cx="55" cy="46" r="12" fill="#2b2b2b"/><circle cx="90" cy="46" r="12" fill="#2b2b2b"/><circle cx="125" cy="46" r="12" fill="#2b2b2b"/>') },
			交互に: { effect: '二つの色や図形、組などを交互に繰り返す。組は一つとして数える。', example: '赤い円と青い線の組を、灰の弧と交互に5つ並べる。', effectEn: 'Alternates two colors, shapes, or groups. Each group counts as one.', exampleEn: 'Line up five, alternating a group of a red circle and a blue line with a gray arc.', svg: shapeSvg('<circle cx="28" cy="46" r="8" fill="#c9362d"/><path d="M20 54 L36 38" stroke="#2c5fb8" stroke-width="3"/><path d="M51 50 Q58 32 65 50" fill="none" stroke="#777777" stroke-width="3"/><circle cx="90" cy="46" r="8" fill="#c9362d"/><path d="M82 54 L98 38" stroke="#2c5fb8" stroke-width="3"/><path d="M115 50 Q122 32 129 50" fill="none" stroke="#777777" stroke-width="3"/><circle cx="154" cy="46" r="8" fill="#c9362d"/><path d="M146 54 L162 38" stroke="#2c5fb8" stroke-width="3"/>') },
			順に: { effect: '記述した色や図形、組などの順を保って繰り返す。', example: '赤い円・青い線・灰の弧の順に繰り返して、八つ並べる。', effectEn: 'Repeats colors, shapes, or groups in the written order.', exampleEn: 'Line up eight, repeating a red circle, a blue line, and a gray arc in order.', svg: shapeSvg('<circle cx="27" cy="46" r="7" fill="#c9362d"/><path d="M40 53 L48 39" stroke="#2c5fb8" stroke-width="3"/><path d="M54 50 Q61 32 68 50" fill="none" stroke="#777777" stroke-width="3"/><circle cx="78" cy="46" r="7" fill="#c9362d"/><path d="M91 53 L99 39" stroke="#2c5fb8" stroke-width="3"/><path d="M105 50 Q112 32 119 50" fill="none" stroke="#777777" stroke-width="3"/><circle cx="129" cy="46" r="7" fill="#c9362d"/><path d="M142 53 L150 39" stroke="#2c5fb8" stroke-width="3"/>') },
			埋める: { effect: '面や領域を密に満たす。', example: '点で埋める', effectEn: 'Fills a face or region densely.', exampleEn: 'Fill with points', svg: shapeSvg('<circle cx="50" cy="28" r="5" fill="#2b2b2b"/><circle cx="80" cy="32" r="5" fill="#2b2b2b"/><circle cx="112" cy="29" r="5" fill="#2b2b2b"/><circle cx="62" cy="55" r="5" fill="#2b2b2b"/><circle cx="97" cy="58" r="5" fill="#2b2b2b"/><circle cx="132" cy="55" r="5" fill="#2b2b2b"/>') },
			散らす: { effect: '要素を不規則に散布する。', example: '黒い点を散らす', effectEn: 'Scatters elements irregularly.', exampleEn: 'Scatter black points', svg: scatter },
			引く: { effect: '線や弧を描く動作。', example: '線を引く', effectEn: 'The act of drawing a line or an arc.', exampleEn: 'Draw a line', svg: lineSvg() },
			敷き詰める: { effect: '図形を規則的・反復的に配置し、明示した個数と寸法を保つ。', example: '四角で敷き詰める', effectEn: 'Places shapes regularly and repeatedly, preserving stated count and size.', exampleEn: 'Tile with squares', svg: shapeSvg('<g fill="none" stroke="#2b2b2b" stroke-width="3"><rect x="24" y="20" width="32" height="30"/><rect x="60" y="20" width="32" height="30"/><rect x="96" y="20" width="32" height="30"/><rect x="132" y="20" width="32" height="30"/><rect x="24" y="54" width="32" height="30"/><rect x="60" y="54" width="32" height="30"/><rect x="96" y="54" width="32" height="30"/><rect x="132" y="54" width="32" height="30"/></g>') },
			...
			上弦: { effect: '右に膨らむ180度の開弧として扱う。', example: '上弦の月', effectEn: 'Read as a 180-degree open arc bulging to the right.', exampleEn: 'A waxing moon', svg: shapeSvg('<path d="M90 18 A28 28 0 0 1 90 74" fill="none" stroke="#2b2b2b" stroke-width="6" stroke-linecap="round"/>') },
			下弦: { effect: '左に膨らむ180度の開弧として扱う。', example: '下弦の月', effectEn: 'Read as a 180-degree open arc bulging to the left.', exampleEn: 'A waning moon', svg: shapeSvg('<path d="M90 18 A28 28 0 0 0 90 74" fill="none" stroke="#2b2b2b" stroke-width="6" stroke-linecap="round"/>') },
			...
			鏡写し: { effect: '二つの形の間の軸を挟んで、位置と向きを鏡像にする。', example: '上に斜めの小さな赤い弧を置く。下に前の形と鏡写しの青い弧を置く。', effectEn: 'Mirrors the positions and orientations across the axis between two shapes.', exampleEn: 'Place a small diagonal red arc at top. Place a blue arc at bottom, mirrored with the previous shape.', svg: shapeSvg('<path d="M55 30 Q92 6 125 28" fill="none" stroke="#c9362d" stroke-width="5" stroke-linecap="round"/><path d="M55 62 Q92 86 125 64" fill="none" stroke="#2c5fb8" stroke-width="5" stroke-linecap="round"/>') },
		};
