...
REASON = (
    "the ground is a support you can name. Six values existed and three of them "
    "arrived: Stage 1 rewrote washi as paper on its first ground line, "
    "`charcoal_ground` appeared in neither prompt nor example, and production "
    "held 0 works on washi out of 3,086 and 0 on a charcoal ground. Two supports "
    "join the enum, the saijiki gains an eleventh category holding the seven "
    "that draw something, and both prompts gain a fixed phrase and an example "
    "per support in each language. NOT ONE CASE HERE MOVES, and that is the "
    "whole description of this version: the layer frozen in this corpus holds no "
    "prompt -- it is the expander, coerce, and the plugin closure -- and the new "
    "category carries no closure marker, so nothing it added is reachable from "
    "any input this corpus states. What moved is what the model is offered, and "
    "the frozen record of that is the Android prompt fixtures keyed by this "
    "number. Engines 3 and 5 froze on the same terms."
)
