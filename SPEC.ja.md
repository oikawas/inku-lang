...
系譜カードの作品メニューは、見出し「作品を編集する」の下にこの順で提供する — 描画要素・記述・DDL・モデル・AI に自律推敲させる・ゴミ箱への移動（項目名は対象語のみに短縮する）。比較系の 2 操作が開くダイアログの題は「描画要素を編集する」「モデルを編集する」である。記述と DDL の編集は、選んだ作品から初期化したモーダルダイアログを開く。描画は `description_edit` または `ddl_edit` の子を保存し、系譜へ戻り、最新の子とその祖先へ焦点を当てる。比較系の 2 操作は選んだカードを対象にし、対応する既存の推敲サブビューをモーダルで開く — 比較のロジックを複製しない。ダイアログを閉じると元の系譜ビューへ戻り、通常のトップレベルの推敲タブはパネル配置を保つ。旧「手動推敲」モーダルにメニュー項目は無い。ゴミ箱は比較操作から視覚的に分け、明示の高コントラストな結果ラベルを持つ。
