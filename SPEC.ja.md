...
region（`at`）と relation を両方持つ instruction（プラグイン member 由来の双弧など）は、region 配置を先に適用した後で relation を解決する（v1.93）。touching では直前要素の演奏後端点が位置を確定するため、region は連鎖の起点・情報として扱われる。

解決不能な関係（例: 直前要素が背景塗りで輪郭を持たない）は、validator / coerce が relation を drop し、警告記録を残す。演奏時にのみ判明する解決不能（退化幾何・grid 配置・端点を持たない直前要素）も同様に、Renderer が警告記録付きで drop する（v1.93）。instruction は relation なしの通常配置で描画される（graceful degradation）。
