...
- **設定モーダル**: モデル・色カタログ・DB 状態・プラグイン状態・書き出しテンプレート・ユーザー・テーマ・デモ
...
入力側は「次に描く条件」、作品側は「表示中作品の条件」を示す。ライブラリの閲覧用プレビューを読むだけでは次の描画の条件を変えず、両側の値が異なることはエラーではない。設定は「表示と操作」「制作」「書き出し」「接続と管理」「拡張と詳細」に分け、既存の権限による表示条件や保存形式を変えない。本文・記述は14px、補助情報と小ボタンは12pxを基準とし、選択は枠だけでなくチェックや「表示中」で示す。小さな操作にはキーボードフォーカスを持たせ、今回の編集・書き出し・設定モーダルを閉じたときは入口へフォーカスを戻す。
...
履歴管理の「アニメーション」は、1作品以上の選択時に出力モーダルを開く。1作品だけなら、保存済みSVGの背景から描画順にレイヤーを増やし、完成品へ至る疑似制作過程を書き出す。背景と完成を含む分割枚数（2〜120枚、既定12枚）、アニメーション間隔（0.1〜30秒、既定0.3秒）、リプレイ設定を選べる。「最初に戻る」は繰り返し、「逆に再生する」は完成後に逆順で戻る往復再生、「最初だけ再生する」は一度再生して完成品で停止する。描画グループと重なり順、背景・クリップ・フィルターを保ち、完成フレームには元の保存SVGを使う。分割枚数よりレイヤーが少ない場合は同一状態の表示時間をまとめる。

2作品以上では従来どおり、選択作品を古い順で切り替える。設定の書き出しタブの値を初期値として、形式（APNG／GIF）、解像度と任意の高さ、1作品用の分割・間隔・リプレイ、複数作品用の切り替えパターン・表示秒数を管理する。出力モーダルには選択数に対応する項目を表示する。サイズと保存先は両方式で共通で、対応ブラウザでは保存先パス指定ボタンからフォルダを選択できる。モーダル内の設定・保存先の変更は今回の出力にだけ適用し、既定設定に保存しない。保存先の指定がない場合は既存の保存先設定に従い、フォルダ選択に非対応のブラウザではブラウザのダウンロード設定を使う。今回指定したフォルダへの保存失敗はモーダル内に表示し、別の保存先へ自動で切り替えない。

系譜タブの「アニメーションを書き出す」も、1作品以上のチェック時に同じ出力モーダルを開き、1作品ならレイヤーアニメーション、複数作品なら古い順の切り替えを使う。ボタンを押した時点でチェックされている作品を対象として保持する。チェックされていない祖先や現在表示中の作品は自動で追加しない。
...
ページを再読み込みした後は、このセッション内の未完スナップショットは残らない。既存の保存済み履歴の発見だけで再開候補を探し、行番号と記述の両方で照合する。**未保存の成功が0件のバッチを、再読み込み後にも再開できることは約束しない。** 再読み込み後に条件を復元できるときは、最後に保存された作品の記録を読む。記録の無い条件は作らない —— 欠落は「off」ではなく「記録より古い」を意味する。
