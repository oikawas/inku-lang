...
`inku`（インク）は、短い記述から抽象的なベクターグラフィックを生み出す描画言語 DDL（Drawing Description Language）のリファレンスとなるアプリケーションです。絵画の経験や道具が無くても、自分のイメージを形に出来ます。心に残った光景をひとつ、短く書くことがスタートで、そこからイメージを作っていくことが出来ます。

```
夜の水面に、青い線がゆっくりほどける
```

この一文が解釈され、楽譜になり、レンダラーに演奏されて、一枚の絵が現れます。同じ一文からもう一度描画すれば、ゆらぎによって少し違う絵が返ってきます。作品は系譜で管理され、世代を追って保存されます。自分の作品をベースにバリエーションを作り、推敲し、並んだ候補からあなたが選び新しい世代が生まれる——**書くことと選ぶことの往復**が、inku での創作です。

inku は三つの文化的資産から学んだアイディアの交点で生まれ、そこに生成AIの技術をどう適用するかを思考したアプリケーションです。生成AIが使われていますが、単なるプロンプトのガチャではありません。
...
現時点（2026年）での生成AIによる画像生成は、過程が創作者自身にも隠されており、呪術的と言っても良いプロンプトを書いてあとはお祈り、の試行のくり返しと言えます。DDLのコンセプトは、作品生成のプロセスを層毎に分け、生成AIと決定論的にプログラムされたアルゴリズムを交互に重ねています。

LLMをベースにしていますが、語彙・図形・座標といった指示要素には厳しい制約を設け、作者の意図を組みつつ型を守るようにプログラムされています。この制約は、作品製作の上での限界ではありません。あなたが作品を作り、その意図を可視化するための道具立てです。
