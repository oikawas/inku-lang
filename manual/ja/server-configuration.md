...
`INKU_RENDER_CONCURRENCY`と`INKU_CLIENT_FANOUT_LIMIT`は**初期値だけを与えます**。以後の正本はDBの設定で、設定画面を`詳細`表示にして現れる`その他（サーバー）`または `inku-cli config update` から変更します。サーバー同時描画数を超えた要求は待たずに503で拒否され、クライアントが短い間隔で再試行します。
...
管理者は`設定`の`ユーザー管理`で利用者の作成、所属・権限グループと他の利用者のpasswordの変更を行えます。対象を選び、`ユーザー変更`の`新しいパスワード`へ入力して保存します（**現在のpasswordは要りません**）。現行Web UIの`ユーザー管理`は`admins`にだけ表示されます。`leaders`はCLIまたはAPIから自分の組織グループ内の通常利用者を管理します。利用者が自分で変えるときは`プロフィール`から現在と新しいpasswordの両方を入れます。**管理者が1人だけで、その人が忘れた場合はこの道が使えません** —— 戻り道は2.2の`inku-admin reset-password`です。
...
| `接続と管理` | `モデル設定`、`ユーザー管理`、`DB設定`、`ログ保存`、詳細時の`その他（サーバー）`と`制限値` | 現行Web UIの管理項目は`admins`のみ |
