...
夜の水面に、青い線がゆっくりほどける
```
inkuアプリケーションの入り口は、まず上のような短い詩歌・散文を書く事からはじまります。言葉は、LLMによって絵画要素の作品計画に分解され、絵画言語DDLに書き起こされます。DDLはTyped Compilerによって、SVG描画が可能なJSONデータにCompileされます。
...
```
あなたの一文（母語で書く）
     │  解釈 — 言葉を歳時記の語で組んだ作品計画に読み解く（Stage 1・LLM）
     ▼
...
解釈と構造化を分けるのは、求められる能力が違うからです。解釈は連想的・創造的なので LLM に任せます。ただし LLM が返すのは、歳時記の語だけで組む閉じた型の作品計画で、指示書の文字列そのものは決定的に書き起こされます。構造化は機械的・規則遵守的なので、LLM ではなく型付きコンパイラが担います。意味を推測で埋めることはせず、読み切れない句は、その範囲だけを LLM に補完させて、あなたの承認を待ちます。解釈に使うモデルは選択でき、モデルによって出来上がる作品には大きな変化が見られます。**モデルの選択そのものが、創造的な変数です。**

非決定性を持つのは、LLM が介在する箇所だけです——Stage 1 の解釈と、任意で使う写生・色カタログの自動選択、そして指示書の補完案（Stage 2）。保存された指示書から先、楽譜への変換と Renderer の演奏（描画）は常に決定的で、同じ楽譜・同じ seed・同じ描画条件から同じ作品を再現できます。ただし、inku の仕様は常に進んでおり、新たな inku が描く作品は常に昔とは違うものです。
...
| **別の構図**（配置を変える） | 焦点（閉じた六つの候補から）、かたむきの具体角度、隅の位置 | 指示書、技法・色・タッチ・要素数 | 高速・LLM を呼ばない |
...
- **多段パイプライン** — 写生（記述に場所と光の補足を添える）と色カタログの自動選択（どちらも任意）・Stage 1（作品計画）・型付きコンパイラ・Stage 1.5（焦点と変奏）・Renderer。AIによる非決定論的な層と、アルゴリズムによる決定論的な層が交互になっています
...
- **プラグイン** — `Nature.YoungLeaves`（別名 `Nature.若葉`）のような名前空間付きの語彙マクロ。コア語彙への展開のみが許され、コアは変更できません。記述に対象の語があれば作品計画でも選ばれます。未登録のプラグインを含む指示書は、その文だけ理由を示して残りを描きます。プラグインの定義を付けて指示書を書き出し、別の環境で読み込めます
