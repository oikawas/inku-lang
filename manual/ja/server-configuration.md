...
この文書は、未リリース版inku v2.13.30（Web Build 917）を継続運用する管理者向けの設定基準です。環境変数template、現行DB schema、Web管理UI、systemd参照templateを対象にします。
...
**作品を描き直すときは、その作品の行に記録された値が使われます**（v2.13.30）。設定を下げても、既に描かれた作品は描かれたときの数のまま出ます。記録を持たない古い作品でだけ今日の設定が使われ、**どちらで描いたかは応答が名乗ります**（`render_limits_source` ＝ `work` / `settings` / `work_unrecorded`）。**注文の側は 1 枚のあいだ値を下げられますが、上げられません** —— 要素ごとに現行の設定と小さいほうが採られます。
