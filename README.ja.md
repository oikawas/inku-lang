...
夜の水面に、青い線がゆっくりほどける
```
inkuアプリケーションの入り口は、まず上のような短い詩歌・散文を書く事からはじまります。言葉は、LLMによって絵の設計図にあたる**作品計画**にまとめられ、そこから絵画言語DDLが書き起こされます。DDLはTyped Compilerによって、SVG描画が可能なJSONデータにCompileされます。
...
```
あなたの一文（母語で書く）
     │  解釈 — 言葉を作品計画に読み解く（Stage 1・LLM）
     ▼
指示書（正規化DDL — 作品計画から決定的に書き起こした、人間が読める実行仕様）
...
**作品計画 — LLM に文章を書かせない。** Stage 1 の LLM は、指示書の文章を書きません。返すのは作品計画です。どの形を、どの画材・色で、どこにどう置くかを、決まった項目に歳時記の語だけで埋めた、絵の設計図です（形ごとの層と、地・背景からなります）。指示書の文章は、この計画から決定的に書き起こされます。
...
作品計画は、これを書き方の指導ではなく構造で解きます。計画の各項目に入れられるのは歳時記の語だけで、形ごとにどの値を組み合わせられるかは、コンパイラに一文ずつ問い合わせて作った表が決めています。範囲外の値はその項目だけが未指定に戻り、計画は必ず読める指示書に書き起こされます。日英の無作為な計画で確かめたとおり、書き起こした指示書はすべて診断なしでコンパイルでき、最初の解釈で句が捨てられることはありません。LLM の創造性は何を・どう描くかを選ぶことに向けられ、文法の正しさは仕組みが保証します。

作品計画は一時的なもので、残るのは指示書と楽譜です。あなたが手で書いたり編集したりする指示書は計画の型に縛られず、DDL の文法全体で読まれます。
...
- **多段パイプライン** — 写生と色カタログの自動選択（どちらも任意）・Stage 1（作品計画）・型付きコンパイラ・Stage 1.5（焦点と変奏）・Renderer。AIによる非決定論的な層と、アルゴリズムによる決定論的な層が交互になっています
...
- **プラグイン** — `Nature.YoungLeaves`（別名 `Nature.若葉`）のような名前空間付きの語彙マクロ。コア語彙への展開のみが許され、コアは変更できません。記述に対象の語があれば、作品計画でも選ばれます。未登録のプラグインを含む指示書は、その文だけ理由を示して残りを描きます。プラグインの定義を付けて指示書を書き出し、別の環境で読み込めます
