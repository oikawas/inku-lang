...
From the settled Score plus `render_seed`, `composition_seed`, `wild`, the resolved color map, profile, and canvas, registry-selected `render_engines/default/adapter.py` creates one canonical JSON request. The independent `inku-render-python` wheel passes that request to the platform-independent `inku-render` core, which owns planning, geometry, mark/surface/layer construction, SVG serialization, and performance metadata. The adapter returns SVG and metadata together without fine-grained Python/Rust calls. `renderer.py` is only the SVG compatibility facade and delegates through the same registry. The detailed module dependency diagram lives in `server-components.md`. **The same Score, seeds, and conditions reproduce the same work** — the Engine 41 corpus guards this contract. There is no runtime fallback or past-engine selector; history display returns the stored SVG.
