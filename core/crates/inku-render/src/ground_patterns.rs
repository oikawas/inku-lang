//! Deterministic finite-pattern builders for named physical supports.

use crate::determinism::hash01;
use crate::svg::{Element, format_number};
use crate::types::{CanvasGroundSpec, GroundGrain, GroundMaterial, Seed};

const GROUND_MM: f64 = 1000.0 / 210.0;
const GROUND_DENSITY_DEFAULT: f64 = 0.20;

#[derive(Clone)]
pub(crate) struct GroundLayer {
    pub(crate) width: f64,
    pub(crate) height: f64,
    pub(crate) rotation: f64,
    pub(crate) opacity: f64,
    pub(crate) body: Vec<Element>,
    pub(crate) definitions: Vec<Element>,
}
...
pub(crate) fn build_ground_layers(ground: &CanvasGroundSpec, seed: Seed) -> Vec<GroundLayer> {
