...
| `APP_VERSION` | the application version | v2.15.32 | every stamping. **`web/APP_VERSION` is the one file that owns it**, and the UI, `/api/info` `version` and the CLI all read it |
| `server/pyproject.toml` | the distributed package | 2.15.32 | **only when a release is tagged**. Returned as `/api/info` `release_version`; it lags the application version while releases are on hold |
| `web/BUILD_NUMBER` | build serial | 1108 | **moves for UI-only changes too. It is a shared counter, not a per-branch value, so numbers can be skipped. Since v2.9.23 a merge driver named in `.gitattributes` keeps the larger side, so two branches bumping it no longer conflict** (run `scripts/git/setup.sh` once per clone) |
