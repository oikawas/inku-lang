...
- **Verification:** focused fail-first checks caught the four former navigation/layout behaviors and one Batch selection boundary. At the branch tip, five `AppMenuNavigationTest` cases and six `WordingLintTest` cases passed, production Kotlin compilation succeeded, and the diff check was clean. Room schema and migrations, pipeline and rendering, Rust, Server/Web/shared, instrumentation, and pentala were unchanged or not run.
