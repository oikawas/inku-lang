...
左に記述、右に作品。その間に**指示書（正規化DDL）**が挟まり、書いた語がどう読まれたかを墨の濃淡で示します。下段の履歴と、右の系譜が「選ぶこと」を受け持ちます。系譜では、いま表示している作品が次の推敲の親になります。

<img src="docs/assets/ui/describe-paint-lineage-dark.png" width="900" alt="画面左に記述の入力欄と、その下に語ごとに色を付けた指示書（正規化DDL）。右は系譜のツリーで、世代ごとの作品が枝分かれして並ぶ。最下段に履歴のサムネイル列">

指示書はそのまま手で編集して描き直せます。右の歳時記は、語を選ぶとその筆致の見本を返します——語彙を覚えてから書くのではなく、見ながら書けます。ライト・ダーク両テーマで同じ構成です。

<table>
<tr>
<td width="50%"><img src="docs/assets/ui/edit-instructions-saijiki-dark.png" width="100%" alt="指示書エディタのダイアログ。左に語ごとに色分けされた指示書の全文、右は歳時記の語彙一覧で、選んだ細筆の筆致見本が上に表示されている。ダークテーマ"></td>
<td width="50%"><img src="docs/assets/ui/edit-instructions-saijiki-light.png" width="100%" alt="同じ指示書エディタのライトテーマ表示。歳時記でビュランを選び、片側にバリのある線の見本が出ている"></td>
</tr>
</table>

（画面は英語 UI での表示です。日本語 UI も構成は同じで、`JA` / `EN` はいつでも切り替えられます。）
