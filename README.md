...
At least one LLM provider is needed for Stage 1 and Stage 2 (`INKU_LLM_BACKEND` plus an API key, or the model settings page in the web UI). There is no self-signup, so on a new DB nobody can sign in until you create the bootstrap admin with `INKU_BOOTSTRAP_ADMIN_PASSWORD` (8+ characters).
