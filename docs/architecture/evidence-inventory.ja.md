...
|---|---|
| 作成日 | 2026-08-10（JST） |
| 公開側 branch / commit | `main` / `dfa7b25569c10f45fe504fdb39be1335eebb9e87` |
| 公開側の未コミット変更 | なし（更新後snapshot確認時） |
| Project Context | `PROJECT_CONTEXT.ja.md`、対象 `v2.11.18 / Build 874` |
| 日本語仕様 | `SPEC.ja.md`、文書版 `v1.92.0` |
| Web / app | `web/APP_VERSION` = `v2.11.18`、`web/BUILD_NUMBER` = `874` |
| Render Engine | 実装 `default` / `29` |
| DDL | `ddl_version=3` / `ddl_engine_version=11` |
| Android | `android/VERSION` = `2.1.4-android.22`、実装が名乗る Render Engine `26` |
...
| API-ROUTERS | Router群 | 10分類、82 endpoint | `api_core/routers/{public,auth,me,plugins,settings,users,history,lineage,render,feedback}.py`; `test_route_authorization.py` | Project Context「server」 | 確認済み | 公開可 |
| API-AUTH | 認証・認可 | Bearer/cookie session、role guard、公開6経路 | `api_core/deps.py`; `routers/auth.py`; `test_route_authorization.py` | §22 | 確認済み | 公開可 |
...
| TEST-CORPUS | 凍結コーパス | render 29の549件、DDL 11の34件を再生成照合 | `server/reference/render-engine-29/manifest.json`; `ddl-engine-11/manifest.json`; workflow | §11, §22 | 確認済み | 公開可 |
