...
| `INKU_LLM_STAGE1_ATTEMPT_TIMEOUT_SECONDS` | 通常DDLを作るStage 1の一回あたり上限秒。既定は300 |
| `INKU_LLM_STAGE1_TOTAL_TIMEOUT_SECONDS` | 再試行待機を含むStage 1全体の有限上限秒。既定は540。試行上限より小さい値は試行上限まで持ち上げる |
