...
REASON = (
    "a fill is a surface word like the other eight. SPEC 3.1 names nine quality "
    "words for the interior of a closed shape, and eight of them landed in "
    "`surface.texture` while the ninth landed in the boolean `filled`. Measured "
    "2026-08-13 against a live model, that road did not carry: `Surface: flat.` "
    "reached `filled` 0 of 14 times in English and 2 of 4 in Japanese, against "
    "12 of 14 for the words that go to `texture`. `SurfaceTexture` gains "
    "`solid`, the vocabulary gains the `score_value` every other quality word "
    "already had, and a branch on both exits derives each way of saying a fill "
    "from the other -- `texture=\"solid\"` gives `filled=true` so every reader "
    "that only knows the boolean still draws it, and `filled=true` with no "
    "surface of its own gives `texture=\"solid\"` so the works already saved "
    "say their interior in the same vocabulary the new ones do. Three coerce "
    "cases move their Score: five closed-shape instructions that carried "
    "`filled=true` with no surface. The other twenty-seven are listed for the "
    "branch report key alone, the way engine 11's twenty-one were, and the 13 "
    "expand and 6 plugin-expand cases do not move at all. The drawing does not "
    "move either: `solid` is folded out of the performance seed, so an "
    "instruction that gains a surface here keeps the key it had with `filled` "
    "alone."
)
