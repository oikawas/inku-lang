...
**Target version: v2.13.47 / Build 974**
...
- **Render Engine 40** — the SVG performance.
...
`render-engine-40` (610 cases) and `ddl-engine-20` (49 cases) are current, and CI enforces
...
The **deterministic layers** are `coerce/`, `ddl_expander.py`, `render_engines/default/`,
`renderer.py`, `stroke_engine.py`, `schema.py`, `saijiki.py`, and `language_support/{ja,en}.py`.
Touching any of them requires running the frozen-corpus comparison.
Within rendering, `default/mark_kernel.py` owns pure geometry over scalars and point collections,
while `default/marks.py` owns SVG emission.
