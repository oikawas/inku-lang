...
- AI／手動推敲、系譜の奥書、生成情報・プロンプト・JSONインスペクタ
- HTTP APIを操作するCLI、管理コマンド、ベンチマーク補助
- `default` Render Engineと、将来のEngine Packに備えた内部境界

直近のv1.89 / Build 578では、系譜の自律推敲を「ランダム」と「AI Vision」から選択可能にした。Vision方式はモデルを明示選択し、点数化や自動棄却をせずに各世代の観察と次に試す方向を次世代へ渡す。全世代を系譜へ残し、最終判断は人間が行う。
