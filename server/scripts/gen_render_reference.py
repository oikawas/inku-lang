...
REASON = (
    "The hand swings wider. Engines 25 and 26 took the congruence out of a "
    "repeated group -- each member its own size, then its own angle -- at the "
    "pair the author ruled on that day, +/-25% and +/-12 degrees. Reading "
    "round 2b of the sample sheets the same day, the author marked the size "
    "variation slightly short and the angle variation short and asked for the "
    "same two rules to swing further (author, 2026-08-08): +/-35% and "
    "+/-27 degrees. This version is those two constants and nothing else. No "
    "rule is added, no exclusion is changed, and no case is added: `grid`, a "
    "group of one, the machine tools, `line` and `circle` for the angle, and "
    "a group that states its own `rotation` are all excluded exactly as they "
    "were, so the reach of the change is the reach the two stages already "
    "had. Of the 549 cases frozen for engine 26, 45 move and 504 do not -- "
    "the 45 are every corpus group a hand tool draws outside a `grid`, which "
    "is 37 circles, 3 ellipses and one each of `line`, `square`, `triangle`, "
    "`arc` and `cloudform`; only 5 of those 45 are groups the angle rule "
    "reaches as well, the rest moving on size alone. The placement is "
    "untouched: `_fit_group_to_anchor` reads the members' anchors, and both "
    "`_scale_member` and `_turn_member` leave every anchor where it was, so "
    "the frame correction fires on the same 40 of the corpus's 50 groups it "
    "fired on at the narrower swing, and the anchors agree to within one step "
