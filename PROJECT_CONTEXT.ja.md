...
直近のv1.90.0 / Build 586では、あいだの第5語として「触れる（touching）」を正式化した。`contact: both_ends` はline / arc間だけで、直前要素の演奏後の両端へ現在要素を固定する。弧は二端点と矢高から180°未満の劣弧を再構成し、直前弧と反対側へ膨らむ。劣弧の符号規約はRendererのSVG描画と共有し、variationと筆致は端点を動かさない。接触の明示句がある時だけStage 1 / Stage 2が選べ、閉形・端点なしは警告記録付きdrop-only、退化形は演奏時のdrop-onlyとし、補修やgovernorは持たない。`続きから` は第二段候補のままである。 Build 587では、relation全種がrotation等を合成したキャンバス座標の端点・輪郭を参照するよう統一し、SVG回帰も祖先transform合成後の座標だけを検査する。
