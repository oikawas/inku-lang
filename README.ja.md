...
**変奏**だけは、どこまで動かすかを三段階で指定します。生成後、実際に動いた軸が表示されるので、何が変わったのかを推測せずに済みます。
...
自分で軸を選ぶかわりに、世代を重ねること自体を AI に任せることもできます。二つの方式があります。

- **ランダムな自動推敲**——許可した要素の中から、世代ごとにランダムに軸を選んで重ねます
- **AI Vision による自動推敲**——各世代の画像を実際に観察し、次に試す方向を次の世代へ渡します

どちらにも方向性（「華やかで、かつ、涼しげに」のような一文）を与えられます。任せている間も、生まれた世代はすべて系譜に残るので、途中の一枚を選んでそこから手で引き直しに戻れます。

### 世代を重ねる

候補は一案ずつでも、グリッドに四案並べてでも作れます。気に入ったものを選んで残し（複数選択可）、**選んだ理由をひとこと添えられます**。**選ぶことは、書くことと並ぶ創作の一部です。**

残した一枚は次の親になります。そこからまた別の演奏を、別の色を、変奏を——という往復のすべてが**系譜**に記録され、どの世代から何を引き直して今の一枚に至ったかを後から辿れます。履歴には seed と作品エディション ID が保存されるので、途中のどの世代もいつでも同じ姿で再現できます。
...
inku の描画エンジンには、**過去の版が存在しません**。第 10 世代のエンジンがあるだけで、第 9 世代を選び直すことはできません。コードも残していません。
...
エンジンの世代が上がるとき、**その世代が何をどう描いていたか**が分からなくなると、変更の意味を後から確かめられません。そこで各世代につき、決まった入力から得た出力の実物を凍結しています（`server/reference/`）。220 ケース——10 種類の道具 × 8 種類の図形を基盤に、揺らぎ・塗り・キャンバス比率・境界値を加えたものです。

| | |
|---|---|
| 凍結するもの | 出力の**実物**（SVG と、その要素数・class・座標の digest） |
| 守り方 | 既存ケースを再生成してバイト一致しなければ CI が落ちる |
| 一致しないとき | それは描画が変わったということ。**エンジンの版を上げる** |

版数は「変わった」という 1 ビットしか運びません。**何がどう変わったかは、出力そのものを突き合わせないと分からない。** 世代が上がるたびに、動いたケースだけを記録していきます。ディレクトリの数が、そのままエンジンが何回変わったかの記録になります。

**この代償は既に払っています。** 第 1〜9 世代の出力は失われました。コードも絵も残っていません。参照コーパスが第 10 世代から始まるのはそのためです。だから、現役のうちに刷りを取る。
