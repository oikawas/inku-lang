...
- 比較タブの生成作品は、推敲候補と同じ画像右上の丸い `+` で採用し、履歴保存後は `✓` で採用済みを示す。スター操作は採用とは独立して維持する。
