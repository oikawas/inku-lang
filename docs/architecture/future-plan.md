...
- **Stream commentary** — `/api/paint/stream` had only two events, `stage1` and `done`. Build 926 added `sketch` (when the sketch is settled) and `score` (when the Score is settled). After the cutover the compatibility stream was briefly a single `done` line; on 2026-09-25 it returned to re-reading the shared-pipeline execution and reporting in the same order. On 2026-09-26 an `attempt` event joined them, reporting each model-call attempt.
