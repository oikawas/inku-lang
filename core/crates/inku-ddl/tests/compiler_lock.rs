...
fn unresolved_clause_owns_its_invalid_sequence_diagnostic() {
    ...
    assert_eq!(
        ...
        "赤いクレヨンの折れ線を右上がりから右下がりへ交互に組み合わせて、上端から下端へ一本引く"
    );
}
...
fn fixture_schema_and_closed_ids_are_stable() {
    ...
    assert_eq!(
        CANONICAL_SEMANTIC_DDL_SCHEMA_ID,
        "inku.semantic-document.v21"
    );
    assert_eq!(
        SEMANTIC_SOURCE_PROVENANCE_SCHEMA_ID,
        "inku.semantic-source-provenance.v6"
    );
    assert_eq!(
        EXPANDED_MACRO_MEANING_SCHEMA_ID,
        "inku.expanded-macro-meaning.v3"
    );
    assert_eq!(
        TYPED_DDL_COMPILER_LOCK_SCHEMA_ID,
        "inku.typed-ddl-compiler-lock.v18"
    );
    assert_eq!(
        TYPED_DDL_COMPILATION_SCHEMA_ID,
        "inku.typed-ddl-compilation.v17"
    );
