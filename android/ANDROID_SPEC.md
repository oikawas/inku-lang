...
The display SVG export is the saved SVG itself; the editable, compat and live SVGs (live has editable's structure with display's textures and touch, as the Web's "Live") are the saved Score drawn again in that profile with the work's colors, seeds and Wild (as the server's `GET /api/history/{id}/svg?profile=`). Files written to `cacheDir/exports` for sharing are removed by the next export once they are a day old.
...
A photo always goes through a description to the normal Stage 1. The "Direct DDL" mode, in which the description model wrote DDL itself (`camera-ddl-v2`), is removed. Its prompt passed only the Stage 1 grammar and vocabulary, with no composition plan and no sentence limit. On the device, the remote `gemini:gemma-4-31b-it` repeated "中心に緑のコンピュータの塗りの四角を1個置く。" 112 times until the output limit cut it off (119 of 121 sentences at "center"; 270 seconds to the drawing), and the on-device E2B produced bracketed, broken DDL or lists of vocabulary. The mode choice is removed from Settings, and a stored `camera_vision_output_mode` is no longer read (a device that chose `ddl` draws from a description). The headless `vision_mode` extra is removed as well.
...
The display follows real processing states. It distinguishes photo preparation, on-device model loading, examining the photo, planning the composition, building colors and forms, finishing the performance, and saving the work. Each phase has a short explanation of the current activity. Photo effects operate on a display preview and never alter the input or retained original. Examining the photo shakes the preview and layers offset horizontal photo slices, scan lines, a moving light band, and intermittent signal noise over it. Finishing progressively stacks large overlapping geometric forms in varied colors. Form, color, and outline effects represent processing phases, not partial drawing results. Text and the source photo stay legible, without rapid full-screen flashes. There are no invented intermediate SVGs, percentages, remaining-time estimates, or delays added for presentation. Direct DDL omits the Stage 1 display it does not execute. With system animations disabled, the same phases have static displays, and the actual work appears after saving completes.
...
The snapshot does not change the visible or persisted normal Stage 1, Stage 2, catalog, sketch, or auto-repair settings. Normal drawing without camera origin, batch, demo, direct DDL drawing, refinement, and saved-work replay retain their existing routes. A camera draw calls the existing `interpret` and `composeFromDdl` paths once each and saves one root work through the existing renderer and transaction. It creates no lineage edge to a work selected before capture or to an unsaved refinement candidate.
...
## 2026-08-26 One-touch abstract instant print ([I-407])

M4 of Abstract Instant Print keeps the existing confirmation before replacing the current description or interpretation and validates both local-E2B readiness and the enabled fixed NVIDIA NIM provider, Base URL, and API key before capture starts. After the capture result is confirmed, one camera run owns image preparation, local Vision, fixed-NIM Stage 1, fixed-NIM Stage 2, Rust rendering, root saving through the existing transaction, and selection of the completed work without another edit or Draw action.

The run fixes both stages to `nvidia:google/gemma-4-31b-it`, the catalog to `vivid_material`, Sketch from Life off, catalog auto-selection off, and `autoRepair=false` without changing or persisting ordinary settings. Only on-device local Vision receives the camera image; NIM receives the generated text alone. Photo bytes, URI, path, and EXIF enter neither a remote request nor history or metadata, and there is no fallback to a provider other than NIM. The saved root keeps I-401 camera provenance and creates no lineage edge to a work or unsaved refinement selected before capture.

After the capture result, a white paper surface maps image preparation and local Vision to `光を読み取っています` / Reading the light, Stage 1 to `かたちを起こしています` / Bringing out the forms, Stage 2 to `色と配置を定着させています` / Fixing color and placement, rendering and saving to `現像しています` / Developing, and successful saving to `現像できました` / Developed. Paper and exposure, grains and forms, `vivid_material` color fields, and settling outlines are procedural effects derived only from real processing phases. There is no invented intermediate SVG, percentage, ETA, or artificial delay. With system animation scale zero, the same phases use static surfaces and reveal the saved real SVG immediately.

While the camera run is active, another camera action, drawing, settings, and primary navigation are unavailable; Cancel and system Back use the same cancellation boundary. Cancellation closes the job and temporary file, rejects late results and saving through the run ID plus a coroutine gate immediately before the repository save, and restores the process-local pre-run Compose state. A local-Vision failure requires a retake. A NIM or render failure after successful local Vision retains the description and provenance; Retry restarts at fixed Stage 1 without repeating image preparation or local Vision. Rotation re-renders the ViewModel's current phase and does not restart external calls. This milestone adds no Room schema or migration, photo persistence, Photo Picker, direct DDL mode, per-stage resume, foreground service, or custom reduce-motion setting.

## 2026-08-26 Camera local DDL direct mode ([I-412])

M5 of Abstract Instant Print adds a camera-analysis mode under Misc settings. Description (recommended) remains the default and preserves M4's local-Vision description, fixed-NIM Stage 1, fixed-NIM Stage 2, render, and save route. Only a run explicitly set to Direct DDL (advanced) asks local Gemma 4 E2B for normalized DDL with `camera-ddl-v1`, skips fixed-NIM Stage 1, and enters Stage 2. The mode is stored under `camera_vision_output_mode` in the existing `app_settings`; a missing, blank, unknown, or malformed value fails safe to Description. A run snapshots the mode before capture and rejects mode changes while active.

The DDL prompt derives from Android's existing `WebDdlSpec` and `ServerDdlText` authority. It never follows text visible in the photo as an instruction and does not identify people or infer personal attributes. One typed validator removes template markers and applies existing Stage 1 normalization and placement sanitizing before checking canonical JA and EN drawable vocabulary. Blank output, code fences, JSON, SQL, preamble-only output, and zero drawable vocabulary fail closed with a retake message and make zero NIM Stage 1 or 2, render, save, or fallback calls. The validator does not infer or add shape, count, color, or placement.

Image-bearing local inference references `WebDdlSpec`'s existing compact Stage 1 authority for LiteRT; it neither copies nor sends the full display specification used by ordinary providers. A contract test holds the JA and EN prompts including the camera boundary to at most 4,096 characters, and performance logging records only the character count, never the prompt content.
