...
- **公開手順の注意（レポートより）:** イメージが GHCR へ上がるのは `v*` タグ push の瞬間だけ。`git push --tags` の巻き込み事故に注意。初回 publish 後の GHCR パッケージは private 既定のため、匿名 pull には手動で public 化が要る。タグ削除とイメージ削除は別操作。
