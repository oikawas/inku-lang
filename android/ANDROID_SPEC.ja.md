...
接続形式ごとに、Serverの`pipeline_provider.py`と同じ形で送る。`openai-compatible`は`/chat/completions`、`gemini`は`/v1beta/models/{model}:generateContent`、`anthropic`（Claude API）は`/v1/messages`で、`x-api-key`と`anthropic-version: 2023-06-01`を付け、描画の応答は強制したtool呼び出しの`input`、写真はbase64の画像ブロックで送る。既定の接続先はServerの一覧と同じで、ovmsを持たず、Ollama Cloud（`ollama-cloud`、`https://ollama.com/v1`、キー必須）を持つ。接続先のモデル選択で一覧を取得する前に出す候補も、Serverの組み込みの一覧と同じにする（Geminiは`gemini-3.5-flash-lite`と`gemma-4-31b-it`。2.5系は提供終了の後に作ったAPIアカウントでは呼べない）。一覧から外したovmsの行は、キー・名前・Base URL・モデル一覧のどれかを作者が設定していれば作者の接続先として残し、既定のままなら起動時に消す。`openai-compatible`の描画の答えは強制した関数呼び出しで求めるが、`ollama`だけはJSON schemaの`response_format`で求め、`ollama`と`ollama-cloud`は`reasoning_effort: none`で推論を切る。既定の接続先は起動時に不足分を補うだけで、作者が変えたサービス名とBase URLを保持する（接続形式は既定のまま、端末内の接続先のBase URLは目印なので変えない）。既定の接続先の「サービス削除」は行を消さず、Serverと同じく無効にして一覧から隠し、APIキーを消す。同じIDで追加すれば戻る。作者が追加した接続先は行ごと消す。端末内の接続先は消せない。APIキーは伏せ字とパスワード用キーボード（予測・学習なし）で入力し、「AIサービスを追加」は閉じると入力値を消す。「ローカルLLMはキー無しで使える場合がある」の注記は、キーが必須でない接続先にだけ出す。
...
書き出しの表示用SVGは保存したSVGそのもの、編集用SVG・汎用SVG・ライブ用SVG（`live`。編集用の構造に表示用の質感とタッチを持ち、Webの「ライブ用」と同じ）は保存Scoreを作品の色・seed・Wildでそのprofileに描き直したもの（Serverの`GET /api/history/{id}/svg?profile=`と同じ）。共有用に`cacheDir/exports`へ書いたファイルは、1日を過ぎたものを次の書き出しで消す。
...
起動時に表示する最新作品は表示のためだけで、次の描画の親にしない（Webは起動時に作品を戻さない）。作品画面・系譜で選んだ作品が親になる。起動時の「前回の描画の復元」は、実行中か作者を待っている実行と、完了したが作品が保存されていない実行だけを戻し（失敗・取り消しの実行は戻さない。Webは起動時に何も戻さず、これはシステムに止められたアプリのためのもの）、推敲を開いている間や候補を作っている間は行わない。この端末ですでに動かしている実行（起動の直後に始めた描画など）は動かさず、状態を読むだけにする（二つ目の駆動が次のモデル呼び出しを取り合い、描画が途中の状態で返っていた）。描画の表示へ切り替えるときは推敲の実行を止める。推敲の候補が作者の判断を要する状態で止まったときは、Webの`failGrid`と同じく失敗を推敲の中に出し、その実行を取り消す。保留中の実行を表示しているときの「新規作成」は、その実行を取り消す。書き出し設定の「白背景時アルファチャンネルを有効にする」はWebと同じ文言で、SVGが全面の背景を持つため見た目には影響しない。
...
写真は常に記述を経て通常のStage 1へ渡る。記述生成モデルにDDLを直接書かせる「DDL直接」モード（`camera-ddl-v2`）は廃止した。この指示文はStage 1の文法と語彙だけを渡し、構図の計画も文数の上限も持たなかったため、実機では外部の`gemini:gemma-4-31b-it`が「中心に緑のコンピュータの塗りの四角を1個置く。」を112回繰り返して出力上限で切れ（全121文中119文が「中心」、描画まで270秒）、端末内のE2Bも角括弧付きの崩れたDDLや語彙の羅列を出した。設定のモード選択は削除し、保存済みの`camera_vision_output_mode`は読まない（`ddl`を選んでいた端末も記述で描く）。headlessの`vision_mode`指定も削除した。
...
作品一覧の右端には、現在位置が見え、タップとドラッグで移動できるスクロールバーを置く。Pixel 9の画面幅では作品の2列表示を維持する。制作ツールの操作には開閉方向を示す矢印を添え、読み上げでも開閉状態を伝える。

写真分析の取消時には実行中の処理とモデル準備を止め、端末内Visionモデルを解放する。次回の撮影時には必要に応じて再初期化する。別アプリの音楽再生を操作しない。

## 2026-09-24 現行の写真入力・進行表示・元写真保持

この節は、新しく撮影またはPhoto Pickerで選択する写真についての現行契約である。末尾に残す2026-08-26および2026-08-27の各実装記録のうち、「写真を保存しない」「photo persistenceを追加しない」という制限を、以下のアプリ内保持に置き換える。過去の実装・受入記録は履歴として維持する。

進行表示は実際の処理状態に追随する。写真の準備、端末内モデルの準備、写真内容の確認、構図の検討、色と形の組立て、描画の仕上げ、作品保存を区別し、その段階で行っていることを短い文で伝える。写真を用いる演出は表示専用のプレビューへ適用し、入力写真や保存写真を書き換えない。写真内容の確認中はプレビュー写真を揺らし、横帯ごとにずれた写真片、走査線、移動する光帯、断続的な信号ノイズを重ねる。仕上げ中は大きな幾何学図形を多彩な色で重ね、順に積み上がる様子を見せる。構図・色・輪郭などの演出は処理段階を表すものであり、未完成の描画結果とは扱わない。文字と元写真を読める明度を保ち、画面全体を急に点滅させない。架空の中間SVG、進捗率、残り時間、演出のための待ち時間を加えず、Direct DDLで省略されるStage 1を表示しない。システムのアニメーション無効時は同じ段階を静止表示し、実際の保存完了後に作品を見せる。

撮影結果またはPhoto Pickerから受け取った写真のbytesを、そのままアプリ専用の`filesDir/original-photos/`へ保持する。元写真の保存には縮小・再圧縮・メタデータ除去を行わないため、元のEXIF等が含まれていればそのまま残る。端末内Visionには従来どおり向き補正、長辺1280px、JPEG品質85の正規化写真を渡し、保存する元写真と区別する。NIMへ渡すのは従来どおりテキストだけで、写真、URI、ローカルパス、EXIFをremote modelへ送らない。

元写真は成功したカメラ経路の作品に、履歴IDから導く不透明なファイル名で関連付ける。Roomの新列・スキーマ版・migrationは追加せず、写真bytesやローカルパスを既存の履歴metadataへ入れない。記述画面は関連する元写真を小さくプレビューし、後で作品を選び直しても参照できる。表示用の縮小・向き補正はプレビューだけへ適用する。既存作品への写真の補完は行わず、写真を持たない作品にはプレビューを出さない。

`cacheDir/camera/`の撮影・読取用一時ファイルと、保存前のアプリ専用一時コピーは別に管理する。取消・失敗・再試行の境界では必要な入力を保持または回収し、保存失敗で作品に結び付かない確定写真を残さない。保存済み作品を完全削除すると関連写真も削除する。Photo Pickerで選んだ元のファイルは変更・削除しない。
