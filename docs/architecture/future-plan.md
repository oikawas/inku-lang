...
- **Stream commentary** — `/api/paint/stream` had only two events, `stage1` and `done`. Build 926 added `sketch` (when the sketch is settled) and `score` (when the Score is settled). After the cutover the compatibility stream returns the final result as one `done` line, and Web's shared-pipeline view shows progress (`known-differences.md` F-08).
