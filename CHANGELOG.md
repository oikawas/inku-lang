...
A sentence whose plugin name does not resolve is still omitted alone while the rest is drawn, and the author is now told why: not installed, disabled, not matching an installed name (with one suggestion), or a different edition of the definition. Web lists it with the work's diagnostics in Japanese and English. The saved-work export menu gains DDL (with plugin definitions), which saves `inku.ddl-export.v1`: the DDL and the definitions it names. The new-DDL dialog's Load and the CLI's `paint --input-mode ddl --file` read such a file and use its definitions for that work only, saying when one differs from the installed edition or is not installed. The CLI gains `ddl-export`. DDL, Score, and render versions are unchanged.
