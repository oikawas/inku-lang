...
const HOLE_ATTACHMENT_GRAMMAR_JA: &str = r#"単独図形では、色・道具・線の連続性・向き・面・揺らぎ・比率・寸法・太さ・形の修飾をhead前の名詞句へまとめ、名詞修飾を「の」でつなぐ。対象と動作は「<head>を…<動作>」、個数・助数詞はその「を」と動作の間へ置ける。位置は受理済みの位置語に直接「に」を付けて動作の前へ置く。未指定の属性は追加しない。
typed_factsは認識された語の種類、confirmed_bindingsは確定した所有先と役割である。未結合lexical factの役割は原文の文脈で解釈し、confirmed_bindingsにないことだけを所有先の確定、明示意味の削除、推測の理由にしない。位置指定のないscatterはcanvas寸法の分布領域を既定で使う。語句がそのextentの既定だけを言い直す場合は省けるが、明示された領域や位置をこの既定と同一視しない。angleはheadの向き、layout_directionは動作の配置方向であり、語順変更で両者を入れ替えない。日本語の配置方向は受理方向語に「に」を付ける形で表す。
...
const HOLE_ATTACHMENT_GRAMMAR_EN: &str = r#"For a standalone shape, put the action before the count and shape noun phrase. Keep color, tool, continuity, orientation, surface, fluctuation, proportion, size, thinness, and shape-form modifiers together before the head. Attach a named position after the head with at/in/on/to followed by an accepted place term. Do not add unspecified attributes.
typed_facts lists recognized lexical categories; confirmed_bindings gives established owners and roles. Interpret the role of an unbound lexical fact from the original context; absence from confirmed_bindings alone neither establishes its owner nor permits removing or guessing explicit meaning. An unpositioned scatter already uses the canvas-sized distribution domain. Wording that only restates that extent default may be omitted, but never equate an explicitly specified region or position with this default. angle modifies the shape head, while layout_direction modifies placement. Do not interchange these roles by reordering words or add a second shape angle to express layout direction.
...
accepted_saijiki_vocabularyと共有文法を用いる。unresolved_clauseは原文の描画headとactionを同じ命令へ保持し、背景だけで済ませない。地は受理済みの地の名詞だけで指定でき、Ground:やSurface:という見出しを付けない。地の支持体を面の質感へ変えない。背景の受理形は「背景を<色>で埋める。」。図形を横に並べる既存配置と図形自体の縦向きを混同しない。短いidごとに必ず一結果を返す。Score、思考過程、説明、管理情報は返さず、指定されたJSONだけを返す。"#;
...
Use accepted_saijiki_vocabulary and the shared grammar. An unresolved_clause must retain its drawing head and action in the same instruction, not replace them with background alone. A ground can be written as its accepted ground noun alone, without a Ground: or Surface: heading. Never change ground material into surface quality. The accepted background form is "fill the background with <color>." Existing line up uses a horizontal left-to-right layout by default; keep a shape's own vertical orientation separate from that layout. Return exactly one result for every short id. Return only the specified JSON, without Score, chain of thought, explanation, or management metadata."#;
