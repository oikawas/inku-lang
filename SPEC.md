...
The current consumer accepts rotation-only `transform`. When it rotates multiple child shapes together, it uses the exact center of their combined bounding box before rotation. Relative positions and each shape's orientation rotate together. Neither the canvas center nor separate child centers replace the group pivot. Internal relations resolve before rotation, which proceeds from inner to outer groups using the combined child bounding-box center at each level. If placement is determined during performance, that placement must be resolved before the pivot is computed. A `group` is transparent structure that carries the range, references, and ownership; it creates neither placement nor a drawing instruction. A transform containing `translate` or `scale` remains an unsupported structural subtree.

Rotation-only transform reaches both count-one actual Scores and repeated symbolic plans. The DDL-side `TransformGroupPlan { start, end, rotation_degrees, fixed_position_indices, provenance }` records a contiguous original-output range in inner-before-outer postorder. Ranges include start and exclude end; they are disjoint or nested, and equal ranges transform in listed order. Score 0.4.0 `transform_groups` retains the same range, angle, and original instruction indices of numerically fixed members. An empty group list stays off the wire; saved 0.1.0 / 0.2.0 / 0.3.0 Scores and versionless artifacts retain their existing compatibility. `surface_intensity` is valid from Score 0.3.0 onward. In a repeated plan, `PlanRelation { kind, gap, target_object_index, position_authority, touching_constraints }` retains checked relation intent. Connected outside a rotated group leaves the prior unchanged and translates the entire outermost group separating the connected member from the prior by one shared delta. Connected is supported across rotation-group boundaries; other external relations remain diagnostic cases in the current performer. A group with a numerically fixed member rejects an additional nonzero translation; must-fit applies only to those numeric members, while named members retain existing clipping. Stop halts before drawing; under OmitAndContinue, group failure omits the whole group and retains owners, original indices, seeds, and lost references. This does not complete Step11 instance materialization or Step13 runtime / UI / persistence cutover.
...
The macro head is joined exactly across its source instruction slot, source invocation ordinal, locked definition, and expanded invocation. Only `place:center` uses the effective focus at `MacroEmit { invocation_ordinal, expansion_path, generated_ordinal, field: place }`. Multiple complete Emits replace the head in their existing order as ordinary instructions; an origin through `use`, bounded `repeat`, or `vary` is not itself a rejection. Output instructions correspond in order to either a direct source slot or generated provenance. Only adjacent bound Emits with exact center placement in the same Macro and original generated order may deliver `connected` / `touching` through the shared checked performer, preserving original reference order and ownership. Touching joins both Line / Arc endpoints with the existing Arc reconstruction and fixes explicit relative scale (including normal), dimensions, and chord direction. `not_touching` also reaches the ordinary DDL Score representation with a Medium gap from adjacent bound Emits in the same Macro. The current Emit requires exact center. `along` / `cutting` also reach the same checked performer from adjacent bound Line Emits, using named-movable or numeric-fixed position authority and the direction/dimension rules in §14.4. Adjacency includes unbound Emits in the original order, and an omitted from never retargets to a survivor. Under the default Stop mode, an incomplete Emit, unknown key, category or type mismatch, unbound caller fact, repeated outer count, or expanded unsupported Transform axes / `anchor` / unsupported `relation` stops the entire Score. Under explicit OmitAndContinue, an independent appearance field omits only that field, an invalid Emit omits that Emit, an unsupported structural node omits that subtree, and invalid outer caller meaning omits the invocation; unrelated siblings, including those inside Groups, remain in source and generated-provenance order. A missing reference omits the target Emit with its original dependency, without retargeting to a survivor. No child Emit is extracted from an unsupported structural subtree, and adjacency is not created across an unsupported subtree. An unused parameter or unreferenced Emit binding ID alone is not rejected.
...
The shared Rust compiler foundation exists through parse / validation / identity / lock / binding / deterministic expansion and carries the finite Emit subset above, including placement-free Groups, through the ordinary lowerer to an actual Score. Production runtime integration, a package catalog, preview, legacy cutover, and an arbitrary user-package loader remain incomplete. Later package / catalog / preview work belongs to separate PLAN steps. `PLUGIN.md` is the current authoring guide governed by this section; it must not treat an unimplemented loader or directory-addition procedure as authority.
...
The current pipeline consists of optional Stage 0.5, Stage 1 finite typed
normalization, deterministic Stage 1.5, Stage 2 conversion into the closed
Score, and Renderer performance. This chapter states each current authority and
compatibility boundary.
...
Stage 2 structures effective DDL / typed meaning into the closed JSON Score
schema. It preserves color, material, quantity, movement, arrangement path,
rotation, canvas, and explicit relations. An explicit element that cannot be
delivered fails clearly instead of silently becoming something else.
...
is the effective DDL / typed meaning consumed by Stage 2.
...
Coerce owns Score-schema delivery and safety for explicit content and performs
only finite actions such as a warned drop of an invalid relation. It must not
add a visual event, composition anchor, density floor, or accent shape.
...
One plan per instruction / Emit retains exact count, resolved dimensions, appearance, angle, position, layout recipe, and source / generated origin. There are no count-proportional arrays, instance geometry, or duplicated Score instructions. Stop returns no plan when blocked; Continue preserves typed owners, spans, reasons, and actual omissions, and never marks an entirely omitted result Ready. Unsupported fields, relations, and coordination are not silently discarded. Ready is not drawing or Score success: materialization, including numeric must-fit and instance bounds checks, has not run. Existing Score wire, lowering outcomes, compiler execution success, and field-absent count-one / place / relation compatibility remain. The same `inku.geometry-resolution-policy.v1` attests this resolution. Remaining geometry, whole Step10, Step11 instance materialization, and runtime / UI / persistence cutover are incomplete.
...
behavior. This contract is separate from full typed production-pipeline, UI,
and save integration.
