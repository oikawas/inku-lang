...
| API route/model | 107 routes, three public paths, response shapes | `api.py`, `api_core/*`, `pipeline_api.py` | Route authorization, module split, API surface baseline | None | Web/CLI/Android sender census; when routes or schemas change, update the count constant and regenerate the baseline from the live app |
