...
// supplements the extent of place and the seasonal or time-of-day light beside
// it. Two choices, off by default: blind comparisons on unseen descriptions
// found no general rule for when a sketch helps (author decision 2026-09-25),
// so the author turns it on per drawing. It never waits for a confirmation.
//
...
// 'not_needed' means it was asked for and found nothing to add.
...
export const SKETCH_MODES: SketchMode[] = ['off', 'on'];
export const SKETCH_GRAINS: SketchGrain[] = ['fine', 'coarse'];

// The default: no sketch.
export const DEFAULT_SKETCH_MODE: SketchMode = 'off';
...
export function sketchModeLabel(mode: SketchMode, isJapanese: boolean): string {
	return mode === 'on' ? (isJapanese ? 'あり' : 'On') : (isJapanese ? 'なし' : 'Off');
}
...
export function sketchModeHint(mode: SketchMode, isJapanese: boolean): string {
	...
	return isJapanese
		...
		: 'Supplement the extent of place and the seasonal or time-of-day light beside the description';
}
...
export function sketchStateNote(state: SketchState | null, isJapanese: boolean): string {
	...
	if (state === 'not_needed') return isJapanese ? '写生を通したが、補うものがなかった' : 'The layer ran and found nothing to add';
 ...
 *  supplement sketch hold a grain: the layer was on. */
