...
### Android `2.1.4-android.65` — intentionally rebuilding legacy databases with Room v10 and closing the portable gaps (2026-08-25, [I-376])
...
- **Verification:** fail-first was red because the current schema was 9 instead of the expected 10. At the branch tip, the Pixel 9 safety runner passed the reset/refusal/retention/WAL/constraint matrix 8/8, refusal screen 1/1, and duplicate-render-hash, primary-key-collision, and lineage-atomicity gates 1/1 each. The full debug+release Android JVM suite passed 518 tests with zero failures, errors, or skips. The portable verifier reported gap 0, its focused server test passed 6 tests, and independent review found no issue. No ordinary APK install or app launch, author-database reset, render parity, benchmark, container, or pentala action was performed.
