...
On 2026-09-22, retiring Nearby works from the web UI removed nearby state, `loadNearby`, calls on work changes and generation completion, and the dedicated neighbors API and candidate reader. The current `LineageQueryState` owns only Lineage queries; Work and `runCurrentWork` no longer receive a nearby-loading capability. The nearby portion of Stage 5A above records the earlier boundary.
