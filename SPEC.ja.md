...
制限値タブは項目を能力・可読性・安全の群に分け、保存済み値・既定値・単位を添える。編集と既定値の復元は保存前の変更として扱い、変更件数を確認してまとめて保存または取り消せる。保存後はサーバーの返却値を採用し、整合性のために調整された項目を知らせる。未保存時の再取得と保存中の編集を防ぎ、失敗時は入力を保つ。
