...
- Dark Compose UI in transition from the web-style workbench to a Pixel 9
  ...
  - bottom navigation: Description, Camera, History, Lineage. Description returns to Write; Camera turns a full-image result into either the default on-device Gemma description or an explicitly selected advanced DDL and proceeds one-touch through fixed NIM, `vivid_material` rendering, and saving
