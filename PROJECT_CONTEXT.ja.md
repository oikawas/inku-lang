...
Android 版（`android/`、Kotlin + Compose + Room、端末内で全パイプライン）は **`2.0.0-android.1` / Build 148080 で Phase 2 完了＝ render engine 10 の移植が全段そろった**（2026-07-24。engine 10 への到達自体は 2f / Build 148077）。版の名前空間は web/server と別で、`android/VERSION` と `android/BUILD_NUMBER` が正本。移植は server を正本として後から追随する形であり、**server 側の設計を Android に合わせて曲げない**。**Phase 3a（Stage 1.5 展開層の中核、android Build 148081）もマージ済み**。さらに **Phase 2h でマスターグリッドへ追随し、Android の `render_engine_version` は `"11"` になった**（2026-07-24）。**幾何は 1 行も変えず、数値を文字列にする箇所だけを 6 桁固定へ寄せた改修**で、`android/VERSION` も `android/BUILD_NUMBER` も動いていない。これで**描画層は web/server に追いついている**。残るは Stage 1.5 の Phase 3b〜3d。詳細は CHANGELOG の Android entry と `android/ANDROID_SPEC.ja.md`。
