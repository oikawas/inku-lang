...
各モデルは、用途（LLM/Vision）、5段階のオススメ度、日英の評価コメント、実測値に基づく速度区分と速度ラベルを持てる。管理者はモデル設定でこれらを編集し、設定時とユーザーのモデル選択時にはマウスオーバーで確認できる。速度は計測時点の観測値を示す診断情報であり、恒久的な性能保証や生成品質の受け入れゲートにはしない。
...
- 類似特徴、距離、モチーフ頻度、vision 所見、coerce 発火率は監査のための鏡であり、生成分岐、抑制、受け入れゲート、品質目的関数へ接続しない。削除数や多様性にも数値目標を置かない。
...
- `meta/llama-3.2-90b-vision-instruct` による逆翻訳と contact sheet の後ずさり所見は、必要時に手動実行する視覚の鏡である。生成、受け入れ、チューニングには接続しない。
