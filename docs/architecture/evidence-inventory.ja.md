...
| API-ROUTERS | Router群 | 10分類のrouterと`/api/pipeline`の共有pipeline router。94 + 13 = 107 endpoint（件数の正本は`test_route_authorization.py`の`EXPECTED_ROUTE_COUNT`） | `api_core/routers/{public,auth,me,plugins,settings,users,history,lineage,render,feedback}.py`; `pipeline_api.py:pipeline_router` | Project Context「server」 | 確認済み | 公開可 |
