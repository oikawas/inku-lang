...
じゅんは色・画材などの同一属性、図形、組の有限列を指定する。「赤と灰を交互にして、円を五つ並べる。」は赤・灰・赤・灰・赤、「赤い円・青い線・灰の弧の順に繰り返して、八つ並べる。」は円・線・弧・円・線・弧・円・線となる。交互には2項、順には空でない有限列を使い、同じ項の重複も記述順に保つ。並べる・散らす・敷き詰める・埋めるへ接続し、明示個数は列長にかかわらず配置単位の総数とする。順序語のない複数の指定から循環を推測しない。不成立な列は局所診断とし、独立した他の描画を続ける。

組は括弧内の命令でなく「赤い円と青い線の組」のような名詞句で表す。「赤い円と青い線の組を、灰の弧と交互に5つ並べる。」は、円と線の組、弧、組、弧、組という5単位で、実際の図形は円3個・線3本・弧2本となる。組の中の図形数を外側の個数へ掛けず、資源需要には内部の図形を含める。組・Macroのbodyと内部の指定を保持し、資源上限超過は外側の配置だけを個体生成前に省略して後続を続ける。「組」は文法であり、歳時記のカテゴリ・チップは増やさない。

日本語では「1つ〜9つ」、十個を表す「10／十／とお」、および「1個・2個・10個・11個」などを使える。「10つ・11つ」は個数表現として採用しない。名詞の前の修飾語は、個数句「N＋助数詞＋の」の前にも後ろにも置ける。「大きな四つの赤い円」「赤い大きな四つの円」「四つの大きな赤い円」は同じ意味で、「細い三本の黒い線を引く」も「三本の細い黒い線を引く」と同じく線3本を引く（DDL 13）。英語もgroup ofで組を記述し、`Line up five, alternating a group of a red circle and a blue line with a gray arc.` は同じ5単位となる。

色列は共有Scoreの既存 `Arrangement.color_cycle` を維持する。その他の列は完全なmember templateを既存の配置／fillへ保持し、Score 0.14の`cycle_members.occurrence_count`から順番に選ぶ。各配置の先頭から列を始め、旧作品の必要最小Score版、既存の資源上限、個数省略の規則を保つ。`繰り返して`も組の表記も通常文法であり、別の歳時記語は増やさない。
...
- **ゆらぎカテゴリは痕の不均一さを言う語彙**：「細かく揺れる」「ゆっくり波打つ」「にじみ」は許容し、「美しく揺れる」「激しく揺れる」は排除する（詳細は Section 13）。英語の振幅語は形容詞のまま（`fine` / `large`）でも副詞形（`finely` / `largely`）でも書ける。`large`は大小の`large`と同形なので、大きさと同じ句に振幅を書くときは`largely`を使う（DDL 12）
...
有限列の巡回選択は `{"expr":"cycle","items":[{"expr":"integer","value":1},{"expr":"integer","value":2}],"index":{"expr":"local","name":"i"}}` と書く。非負の整数indexを列の長さで巡回し、同じ型のitemsから一つを選ぶ。空列・型の混在・整数でないindexは定義診断、実行時の負indexは評価診断となる。repeatの個数を減らさず、奇数個でも交互の色や順序付き位置を表せる。選択された値はexact decimalやsemantic IDの型・正規化を保ち、既存の評価予算内で解決する。新しい乱数・一般算術・外部処理は導入しない。
...
`Nature.leaves`は、`Nature.YoungLeaves`（別名`Nature.若葉`）、`Nature.Undergrowth`（`Nature.下草`）、`Nature.SummerLeaves`（`Nature.青葉`）、`Nature.AutumnLeaves`（`Nature.紅葉`）、`Nature.FallenLeaves`（`Nature.落葉`）、`Nature.WitheredGrass`（`Nature.枯草`）、`Nature.WitheredLeaves`（`Nature.枯葉`）を同梱するshared catalogである。英語の見出しが正式名で、日本語名は別名（§4.13）。ServerとAndroidは同じcatalogからこれらの定義と表示を読む。これは外部runtime loader、任意にinstallするpackage、または`Nature`名前空間全体のregistryを実装したという主張ではない。`Bamboo`、`Nature.雨`、`Nature.風`は将来または説明用のreference vocabulary名として残る。将来reference definitionを提供する場合も、同じMacroDefinition v1 schemaと通常のlock / expansion境界に従い、plugin固有の実行経路を持たない。
...
直前の通常 Arc との checked `Touching` が成功し、後続 Arc の明示 Solid の塗り意図が renderer 入力で `filled=true` かつ `surface=None`、または `surface.texture=solid` として存在し、両者の演奏時の fill scope と mode が同じときだけ、後続 Arc は二つの弧を一つの閉輪郭として使う。Macro の明示 Solid は共通 lowerer により前者へ正規化され、保存済みの直接 Score は後者を保てる。揺らぎ、各自の回転、包含 Transform、反復個体、成立した関係による配置をその輪郭へ反映し、後続 Arc の解決済みの色・素材で、変更しない二本の輪郭の下に内部を塗る。`wash` / `none`、三日月、unchecked・不成立・省略された `Touching`、任意の重なりはこの塗りを生まない。
...
現行の参照 UI は、記述入力と解釈結果、描画キャンバス、履歴帯、推敲を制作画面に置き、保存済み作品の一覧管理は別のライブラリ画面で行う。画面を行き来しても制作中の入力・推敲とライブラリの閲覧状態を保つ。Stage 1 の正規化 DDL は描画完了を待たずに表示でき、保存済み作品からは DDL・Score・生成情報・系譜を辿れる。推敲は元作品との差分を派生 metadata と表示で示すが、プログラミングの diff を制作面の中心には置かない。
...
**設定ダイアログにも同じ考えの 2 モードを置く（v2.13.18）。** `標準`は日々使う設定だけを出し、`詳細`は`プラグイン`・`制限値`・`未読語台帳`・`その他（サーバー）`の 4 タブを加える。**タブの名前は 1 つのモジュールが持ち、タブバーと本文の番人が同じ表を読む** ——**バーが隠して番人が許すと本文へ届かない箱ができ、逆だと押しても何も起きないボタンが残る。**詳細表示の選択はブラウザに残り、サーバーの保存データは変わらない。`制作`カテゴリの`デモ`は標準表示の通常ユーザーにも見せ、デモを開いた設定タブはユーザーごとに保存する。
...
1. **白紙の記述エリア**: 自分の言葉で書く記述欄には補完候補を自動表示しない。短歌の原稿用紙に近い純度
...
| 記述欄への常時表示の候補一覧 | 自分の言葉で書き始める前に、既存語彙から選ぶ順序を強制する |

**設計の根拠**

記述者が自分の言葉から書き始められるよう、記述欄へ候補を押し出さない。一方、指示書を直接作成・編集するときは、語彙を見渡して短い指示を組み立てられるよう、歳時記を初めから表示する。記述入力と指示書編集の目的を区別し、歳時記を参照するか、閉じて書くかは利用者が選ぶ。
...
- 記述欄には語彙候補を自動表示しない
- 閲覧用の歳時記はキャンバス下部のボタンから能動的に開く
- 指示書エディタでは本文の下に歳時記を初めから表示し、必要に応じて閉じられる（§7.8）
...
調整候補は**生成元の作品が所有する一時状態**であり、履歴・系譜・移動から作品を明示的に選ぶか、新しい生成や DDL 描画を始めると消える。調整とモデル比較を行き来しただけでは消えない。対象が変わると、対象が所有するモデル比較結果・読み取り差分・再演エラー・中間系譜の通知・系譜取得状態も戻る。実行中のモデル比較は中断し、系譜要求は最新のものだけが表示を更新できる。
...
作品ヘッダの「推敲する」と系譜カードは、同じ作品編集メニューを提供する — 描画パラメータの編集、記述を編集、指示書を編集、写生の区切りを変える、使用モデル変更、AI自動推敲プロセスの順とする。メニューを開くだけでは描画・保存を行わない。作品ヘッダの入口は保存済みの表示中作品を対象とし、未保存プレビューでは保存を案内する。DDL直接生成作品では、記述を編集・写生の区切りを変える・使用モデル変更を出さない。描画パラメータの編集と使用モデル変更は、対象作品の既存の推敲サブビューを開き、比較のロジックを複製しない。記述と指示書の編集は対象作品から初期化したダイアログを開き、描画は `description_edit` または `ddl_edit` の子を保存する。保存せず閉じると入口の作品または系譜へ戻り、入力中の記述を置き換えない。子を保存した後は系譜で最新の子とその祖先を表示し、他の保存済みの枝も保持する。最新の子は品質順位を意味しない。通常のトップレベルの推敲タブはパネル配置を保ち、旧「手動推敲」モーダルにメニュー項目は無い。推敲マークは旗のアイコンと「推敲マーク」の名称で示し、編集操作と区別する。保存値 `for_revision` の意味は変更しない。
...
- **Canvas パネル**: SVG 表示・ズーム・パン・出力タブ・作品条件ヘッダ・下部の作品操作と書き出し
- **履歴ストリップ**: 最近の作品・ホバーのメタデータ・スター印・ページング。**サムネイルの下に添える情報は読み手が選ぶ** —— 世代・モデル・engine バージョン・ファイル容量から最大 2 つで、**0 個も選択可能**（何も選ばなければ絵だけが並ぶ）
- **ライブラリ**: 大きな履歴ビュー・ゴミ箱・復元・完全削除・スターのフィルタ・**共有のみのフィルタ**・作品ごとの共有。共有ダイアログは宛先と権限（読み取り／書き込み）を選び、いま誰に渡っているかを一覧する。**他人から共有された作品には印を出す** —— 人が選んで消す画面なので、印が無いと他人の作品が自分のものと同じ見た目で並ぶ。**宛先は自分と同じ組織グループの人を名前で選べ**、候補を取れない利用者は id を直接入力する（全員の名簿は開かない）
- **設定モーダル**: 表示と操作・制作・エクスポート・接続と管理・拡張と詳細。表示項目は権限と標準／詳細の選択に従う

表示中作品のモデル・色カタログ・キャンバスは上部の作品条件ヘッダに示す。スター・共有マークと書き出しなどの作品操作はキャンバス下部に置く。

履歴リストの記述は画像の隣の広い列に置き、文字数では切り詰めない。初期表示は最大3行で、全文の開閉はクリック・キーボードで操作でき、作品の選択や描画を伴わない。サムネイル表示でも全文へ到達できる。モデル名を一つにまとめるのは、解釈・描画の両段に提供元を含む同一のモデルIDが記録されている場合だけとし、欠損は「未記録」として段階ごとに表示する。提供元と完全なモデル名は詳細に残す。元の記述や保存済み条件は表示整理で変更しない。

履歴管理は同じアプリ内の**ライブラリ**として開き、制作画面・入力・推敲の一時状態を作り直さない。ライブラリを閉じて作品や系譜を見てから戻っても、検索・絞り込み・表示形式・ページ・選択・スクロール位置と閲覧用プレビューを保つ。閲覧用プレビューの行・画像を選ぶことは閲覧だけであり、次に描く条件を変えない。作品で開く、系譜を開く、推敲する入口は別に置く。表示形式（サムネイル／リスト）とまとめ方（時系列／系譜）は別の操作群で、検索・絞り込み・ごみ箱への切替だけがページと一括選択を更新する。ページをまたぐ選択と選択解除は明示し、通常履歴とごみ箱の選択を混ぜない。

ライブラリの「サムネイル」と「系譜ごと」を組み合わせると、各系譜の作品を起点から世代順に横へ並べる。各系譜のサムネイルに必要な高さを保ち、系譜の一覧は縦に、長い系譜の列は横にスクロールする。

系譜の閲覧状態は選択中作品とは別に、同じ起点の木について開いた枝、通常表示と全体図のスクロール位置、縦横方向、全体図の開閉と倍率を保つ。全体図で節点を選んでも全体図を閉じない。新しい起点または閲覧を許されない応答ではその状態を破棄し、古い画像や本文を代わりに表示しない。削除・非公開の節点は新しい系譜応答に従って除く。系譜ヘッダは表示操作、表示中作品、起点から表示中までの経路、チェックした作品、奥書や起点操作の群に分ける。「表示中の作品」とチェック集合は別の対象である。

Web UIは記録された派生関係を系譜として辿る。Scoreの類似度で順位付けする「近い作品」の表示と自動取得は廃止し、専用の `GET /api/history/{item_id}/neighbors` も提供しない。作品の切り替え・生成完了時に類似作品の検索や取得を追加しない。保存済み作品と系譜のデータは保持する。

保存済み作品の書き出しは対象を固定した共通メニューを通す。表示中作品または1件の選択はSVG、PNG、共有カード、レイヤーアニメーションを使う。複数のチェック作品は古い順の作品アニメーションと鑑賞用・AI用コンタクトシートを使う。系譜経路は起点から表示中までの順を使う。チェック集合と経路は混ぜない。メニューを開いた時点の対象を保持し、出力直前に各作品の可用性を確認する。削除済み、利用不可、または確認に失敗した作品は出力しない。

入力側は「次に描く条件」、作品側は「表示中作品の条件」を示す。ライブラリの閲覧用プレビューを読むだけでは次の描画の条件を変えず、両側の値が異なることはエラーではない。設定は「表示と操作」「制作」「エクスポート」「接続と管理」「拡張と詳細」に分け、既存の権限による表示条件や保存形式を変えない。本文・記述は14px、補助情報と小ボタンは12pxを基準とし、選択は枠だけでなくチェックや「表示中」で示す。小さな操作にはキーボードフォーカスを持たせ、今回の編集・書き出し・設定モーダルを閉じたときは入口へフォーカスを戻す。

「表示と操作」の文字サイズスライダーは、標準を100%として90・100・110・120・130%の5段階を提供する。変更は画面へ即時反映し、操作を確定するとアカウントに保存する。標準へ戻す操作を備え、保存失敗時は表示中の大きさを保って失敗と再試行を示す。未設定・不正な設定値は標準とし、別アカウントへ以前の設定や保存応答を引き継がない。上記のpx基準は100%時の値であり、UIの固定文字サイズは共通CSSのサイズ定義と倍率を使う。相対指定も同じ基準文字サイズに従う。作品のSVG・画像や書き出し寸法は変更しない。

記述タブは入力、次に描く条件、描画操作の順に配置する。入力タブは`記述`と`バッチ`だけである。モデルと色カタログは項目名・変更操作・現在値を一つの行にまとめ、長い名称を折り返して読めるようにする。解釈と描画のモデルが異なる場合は別々に示す。写生とキャンバスはボタン内に現在値を、暴れるは入／切を示す。指示書の新規作成は制作側に、表示中作品の写生と指示書は区切りと見出しの下に置く。指示書の編集は本文の見出し横、指示書からの描画は本文直下に置く。既存の入力ロック、進捗・停止・エラー、判断が必要な通知、開閉状態の保存、指示書編集による子の保存は保持する。

指示書の新規作成と編集は共通のダイアログと編集欄を使う。短い指示を歳時記から組み立てる使い方を基本とし、上部に数行分の本文、その下に広い歳時記の一覧を置く。カテゴリ見出しと語彙を複数列で見渡せるよう並べ、作例・説明は一覧を押し動かさない別枠に表示する。狭幅でも歳時記を表示して開き、列数と作例枠の配置を調整する。語彙と簡易ガイドは開閉でき、語彙を閉じると本文の編集領域を広げられる。本文には折り返しに追従する行番号、構文の色分け、行数・文字数を示す。語彙は編集中の本文の言語に追従し、選択範囲またはカーソル位置に挿入する。描画モデルと編集時の「暴れる」は本文の外、描画・キャンセル・進捗・停止・エラーは下部に置く。モデル選択ではTabを内部で循環させ、Escapeはモデル選択だけを閉じる。指示書ダイアログを閉じると入口へフォーカスを戻す。新規は空欄から独立作品を、編集は対象作品の指示書から子作品を描く。描画失敗・停止時は編集内容を保ち、描画中は本文・語彙挿入・条件変更・閉じる操作を無効にする。

設定モーダルは中央に配置し、タブごとの見出しと説明を本文のスクロール領域から分ける。狭い画面ではナビゲーションと入力欄を組み替え、幅が必要な表は表の領域だけを横スクロールする。モデル設定ではサービスを選んで一つずつ編集し、公開中のモデルを先に表示し、接続情報は開閉できる領域に置く。モデル一覧は名前・ID・公開状態・用途を行ごとに示し、評価情報は明示的に開いて編集する。検索と公開状態・用途の絞り込みは編集中の値を保持し、一括選択・解除はその時点で表示中のモデルだけに適用する。公開数と未保存表示、保存操作は一覧のスクロール外に置く。未保存の変更がある間は一覧の再取得を無効にし、保存失敗時は入力とダイアログを保つ。子ダイアログ内でTabを循環させ、Escapeではそのダイアログだけを閉じ、開いた操作へフォーカスを戻す。

制限値タブは「描画量」「数の表現」「安全上限」の3群を一覧し、項目名・説明と入力欄を行ごとに揃える。保存済み値・既定値・単位を入力の近くに置き、変更した行を示す。狭い画面では説明と入力を上下に並べる。本文だけをスクロールし、保存結果と操作欄は本文外に置いて項目を覆わない。編集と既定値の復元は保存前の変更として扱い、変更件数を確認してまとめて保存または取り消せる。保存後はサーバーの返却値を採用し、整合性のために調整された項目を知らせる。未保存時の再取得と保存中の編集を防ぎ、失敗時は入力を保つ。

エクスポートタブは保存先・PNG・アニメーション・共有カードに分ける。PNGテンプレートは行ごとに編集して明示的に保存し、名前の必須入力と高さ64〜12000pxの整数を検証する。追加・更新・削除の保存処理は同時に行わず、失敗時は編集を保つ。アカウントに保存するPNGテンプレートと、このブラウザに保存する書き出し既定値を区別し、フォルダ指定が利用できない環境ではブラウザ既定の保存先を使うことを示す。アニメーションは1作品のレイヤーと複数作品の切り替えを選んで設定し、フォーマットと解像度は共通の既定値を使う。

ユーザー管理タブは検索と権限・所属なしの絞り込みを持ち、長い名前・メール・所属を折り返して表示する。選択中の利用者だけを編集し、追加フォームは明示的に開く。未保存の編集時には一覧更新と選択し直しを防ぎ、保存失敗時は入力とエラーを保つ。権限と所属の説明を添え、グループ管理は開閉できる領域に置く。既存の認可規則は変えない。

履歴の表示では、モデル・カタログ・キャンバスの値は取れるかぎり履歴項目から取る。次に描く条件は現在の選択から取る。Canvas パネルのヘッダは、表示中作品の世代・モデル・色カタログ・キャンバス・SVG容量・作成日時を項目名と値のまとまりで簡潔に示す。入力側の色カタログは現在値と変更操作を同じ行に置き、長い名前は折り返して読めるようにする。

設定モーダルの「表示と操作」は**履歴選択時の挙動**の操作を持つ。履歴項目を選んだときに UI の現在のキャンバス比と色カタログをその項目の値へ更新するか、現在の UI の選択を保つかを、それぞれ独立に選べる。この設定は UI の選択状態にしか効かない — 保存済みの履歴 SVG は保存されたまま表示され、描き直されない。

同じ「表示と操作」が**履歴ストリップに添える事実**の選択も持つ。世代・モデル・engine バージョン・ファイル容量の 4 つから**最大 2 つ**を選び、選んだ順ではなく**宣言順**で出す。**「何も選ばない」は既定への差し戻しではなく、保存される答えである** — したがって**未回答（列を持たない口座）と空の選択は別物として扱う**。3 つ目を選ぼうとすると、既存の選択を追い出さずに拒み、上限を画面で示す。**ファイル容量はサーバーが保存済みの作品について報告する量**であり、ブラウザが受け取った SVG を数えたものではない — 帯を埋める一覧は絵を運ばないので、受け取ったものを数えるとすべての作品が 0 と報告される。

Canvas パネルは鑑賞向けの操作も持つ。描画タブの全画面アイコンはプレゼンテーションモードを開き、現在の SVG を最大化して、履歴の移動・最新項目・スター切替・指示キャプション切替・閉じるの小さな操作バーを出す。Escape で閉じる。描画タブのキャプションアイコンは指示キャプションを切り替える。横書きのキャプションは、通常の Canvas ビューでは描画タブに対して左右 10% の余白を取り、そのタブの内側で切り取られる。プレゼンテーションモードではウィンドウに対して左右 10% の余白を取る。日本語を含む詞書きには、両画面の操作バーに「横書き／縦書き」の選択を表示する。縦書きは上から下・右から左に読み、改行と強調表現を保持する。長い縦書きの詞書きは枠内でスクロールできる。初期値は横書き・左で、日本語を含まない文章は選択値を保持したまま横書きで表示する。設定モーダルの「表示と操作」にある「詞書きの表示位置」で左／右を選び、縦書きの配置と横書きの文字揃えに適用する。**キャプションが表示するのは利用者に向けた元の指示文であって、内部で補強された Stage 1 プロンプトではない** — 感情ヒントやシステムプロンプトの素材をプレゼンテーションのキャプションから締め出すためである。
...
2作品以上では、チェックした作品は古い順、系譜経路は起点から表示中までの順で切り替える。設定のエクスポートタブの値を初期値として、形式（APNG／GIF）、解像度と任意の高さ、1作品用の分割・間隔・リプレイ、複数作品用の切り替えパターン・表示秒数を管理する。出力モーダルには対象数に対応する項目を表示する。サイズと保存先は両方式で共通で、対応ブラウザでは保存先パス指定ボタンからフォルダを選択できる。モーダル内の設定・保存先の変更は今回の出力にだけ適用し、既定設定に保存しない。保存先の指定がない場合は既存の保存先設定に従い、フォルダ選択に非対応のブラウザではブラウザのダウンロード設定を使う。今回指定したフォルダへの保存失敗はモーダル内に表示し、別の保存先へ自動で切り替えない。

系譜では表示中作品・起点から表示中までの経路・チェックした作品に別々の「書き出す」を置く。メニューを開いた時点の対象を保持し、チェックした作品の書き出しに、チェックされていない祖先や表示中作品を自動で追加しない。
...
調整候補は生成元の作品に属する一時状態であり、履歴・系譜・前後移動から作品を明示選択した場合、または新規生成・DDL描画へ進んだ場合は破棄する。調整／モデル比較のサブタブ切替だけでは候補を破棄しない。対象作品の変更時は、調整候補に加えてモデル比較結果、読み取り差分、再現描画エラー、系譜の中間作品通知と取得状態も同じ対象依存状態として初期化する。進行中のモデル比較は中断し、系譜取得は最新要求だけを反映する。
...
管理者はモデル設定のモデル一覧で評価情報を開いて編集する。ユーザーのモデル選択時にはマウスオーバーで確認できる。**段ごとに測ったモデルのマウスオーバーは推奨度を 2 行に分けて示し、通しで測ったモデルは 1 行のままとする** — 通しの値を 2 行に複製すると、行っていない計測があるように見えるためである。Vision は段で分けない。
...
生成用の歳時記projectionは既存assetの語形だけを列挙し、表示・参照用の既定注記を語形へ連結しない。参照用の注記と既定値は保持する。初回生成とcameraは同じprojectionから描画headを示し、道具・連続性・面の属性との役割を分ける。地、背景、宣言されたparameterだけのmacro呼出しを図形命令と区別し、必要な対象や結合不能な明示指定を削って形式だけを成立させない。

単独図形の共有grammarは、修飾句が接続を持ちheadを含まないこと、数量句が数と助数詞の完結した単位であることを明示する。歳時記外の太さ・相対寸法・正形・辺数の語形は、認識処理が使う既存parser定義の読取りprojectionを共有し、prompt専用の別語彙表を作らない。形修飾は有限の正則性制約であり、自然物名から自由な形容語を作るslotではない。既存の境界・後続head条件と受理可能な組合せは変えない。
...
| 分離修飾行（「揺らぎ: 小」を独立行で書く初期案） | 実装では採用されなかった。運動語彙は「線は細かく揺れる。」のようにインラインの文として書く（fixture コーパスが正）。面・地も同じで、見出し付きの「面: ...」「地: ...」は採用しない（2026-09-24にSPECから外した）。地は「和紙。」のような一文、面は図形の修飾語として書く |

**採用形式の特徴:**
- 自然文のリズム（短歌的な読み心地）
- 語彙はコアに限定（「置く」「細い」「中心」など）
- 運動語彙はインラインの文。面の質は図形の修飾語、地は「和紙。」のような一文で書く
...
**サーバーの資格情報で呼ぶモデルは、管理者が公開したものに限る（2026-09-26）。** `admins` 以外の利用者の生成、デモ指示、奥書、Vision 推敲の助言は、有効な provider がその用途の一覧に持ち、管理者が無効にしていないモデルだけを呼ぶ。それ以外を名指した要求は 403（生成は `model_not_offered`）で断る。以前は無効化が一覧から隠すだけで、名指した要求はサーバーの鍵で provider へ届いた。developer mode は判定に使わない —— 表示だけを変える設定であり、組み込みの Stage 既定は developer mode でしか表示されない provider にある。`admins` は公開前に試すため、構成済みの任意のモデルを呼べる。
...
`INKU_DEVELOPER_MODE`時だけ、authoring requestはstrict booleanの`developer_disable_llm_retries`と`developer_capture_provider_io`を独立に指定できる。前者はcatalog、Stage 1、hole completionのcore policyを各`max_attempts: 1`へ限定し、transport retryだけでなくcompiler rejection feedbackの再要求も止める。後者はproviderへ送る前にowner/execution/action単位のprivate durable recordを作成できた場合だけ送信し、実送信JSON body、provider ID/model/action tag、HTTP status、受信raw body、timeout、usage、elapsedとoutcomeを記録する。responseが途中で切れた又はsize上限で切り詰められた時はpartial/incompleteを明示する。record作成失敗なら送信せず、送信後の記録失敗もcompleteとは表示しない。rawは通常history、public view、logへ入れず、developer modeの同じownerだけが`/api/pipeline/executions/{execution_id}/provider-observations`で読む。URL、header、credential、connection config、exception textはrecordしない。developer mode外の有効化要求は拒否し、通常requestのretry、prompt、描画意味は変えない。
...
動作ownerが曖昧な未解釈句で、属性・数量・動作等を一切持たない暫定primitive headは、必ずしも新たな描画対象ではない。group・sequence・continuation・fill targetや既存参照を持たない範囲では、候補の型付きPreviousOne参照が編集範囲外の同じprimitiveを持つ先行対象へ解決された場合に限り、その語彙を参照先として保全できる。参照一件を複数headの保全へ重複使用せず、確定owner・数量・範囲外の意味と診断・承認/CASは従来どおり検証する。既存の有効なangleとlayout_directionを交換する許可ではない。

描画head・数量・関係・修飾語のownerを持たない独立の地／背景句は、他の未解決描画句と分けて候補を検証できる。同じ種類の支持句は一単位に保ち、候補にも同じsupport ownerだけを要求して描画対象の新設を拒否する。参照先のsource順ordinalを変えず、確定factと範囲外診断の保全、採用候補全体の再compileを維持する。未知の描画句同士をこの例外で分割しない。

単独primitiveのhead・actionが同一clause内で確定し、countが明示結合済み、または認識済み数値・未解決診断を持たず未指定のままで、group・sequence・continuation・fill targetを持たない単純な参照namespaceでは、その句の言い換えも個別に検証できる。候補を再compileし、全instructionのsource順head identity・count（未指定状態も含む）・actionが不変で、修正対象が外向きの参照依存を持たないことを追加確認する。未結合の数値や質的数量の診断を数量省略とみなさず、既定countを推測して補完しない。後続の未解決参照だけを理由に先行ownerの修復を拒否しない。依存構造が不明な単位はまとめたままにし、確定fact／owner／範囲外診断の既存検証と採用候補全体の再compileを省略しない。

新しい応答は、要求内で有効な短い対象IDごとに修正候補または未解決理由を一件返す。共有RustはIDの完全一致・重複・欠落を検査し、保存済み要求とcompiler lockから実hole ID、許可span、range digest、base source digestを復元する。Providerにhashやbyte位置を再生成させず、Python／Kotlin hostに意味の補修を置かない。非同期応答のaction identity、revision、compiler lockとCASの検証は維持する。

Providerのpatchは候補にすぎない。共有compilerは再compile時に確定済みfactと、そのfactが結び付いていた対象を保つかを検査し、対象の解決を確認する。原文に含まれる未認識表現の同義性すべてを機械的に証明するものではなく、解釈結果は可視DDLとして作者へ提示する。独立性を証明できた単位だけを部分採用の候補にでき、同じ総数・group・参照関係等に依存する単位や独立性不明の単位を推測で分割しない。採用候補の組合せも全体を再compileして範囲外の意味と診断を照合する。作者の明示承認時にbaseへ再検証し、sourceと次authorityを1個のCAS save actionとしてhostへ渡す。一致するsave acknowledgmentの後だけ保存されたvisible bytesを再parseし、残ったholeの自動再送は行わない。未解決・不採用の対象と安全な理由コードを保持し、現在の安全なScore／SVGを失わず、部分採用を全hole解決と扱わない。
...
共有compilerがcore生成のStage 1候補を可視commit前に完全採用できない場合、coreはStage 1の`max_attempts`と`total_timeout_ms`の残りを再正規化に使える。同じStage 1とresponse schemaへ、元の記述、未採用DDL、compilerのreason、source span、spanがある場合は対応する原文断片からなる有限projectionを渡し、完全な置換DDLを要求する。このfeedbackはpayloadを変えるため、再正規化は新しいrequest digestを持つnew logical actionである。Transport retryと再正規化actionは同じ有限Stage 1 budgetを消費する。再正規化中の候補はdurable execution replay用のrequest contextへ保持し、通常の完全採用候補または下記の条件付き残部採用だけをvisible DDL commitへ進める。

再正規化の予算を使い切った候補でも、既存compilerのsealed execution projectionとlowering・資源検査により描画可能な残部が成立する場合は、候補DDL全文を変更せず、通常のauthority revision／source digestのCAS保存へ提案できる。この残部採用は完全な意味受理ではなく、元lockを`CanonicalReady`へ変えない。元canonical semantic identityがなければNoneのまま、残部のexecution identity、元owner・span・省略理由を別々に保持する。未知spanを省略して同じ句の確定headが残る場合もあり、文単位の切取りや未知意味の同義性証明とは扱わない。

保存前の判定は可否確認に限り、hostの一致する保存ACK後の全文から再compileしたScoreだけを配送する。残部採用からknown-hole補完の追加LLM要求は作らず、`complete_with_omissions`と元の診断を通常の配送・保存へ保持する。整合性不良、描画対象なし、全省略、ACK後の不成立は停止し、背景だけの空作品を成功にしない。有効な地を描画内容とする既存規則は保つ。原文・候補全文の書換え、未指定意味の追加、guard解除、無条件のStage 1通過は行わない。
...
| 太筆・細筆 | 少し曲がった筆の帯の刷毛目。帯の向きは塗りごとに水平±60°でseedから決まる。帯の外側も通常0.92の濃さで塗り、地の透けは最大8%。濃いは0.96、薄いは全体を0.55倍にする（render engine 68） |
...
太筆・細筆の帯は道具ごとに一枚の継ぎ目のない型紙（細筆320・太筆512、短辺1000当たり）を作品に一度だけ定義し、各塗りはその型紙を自分の中心の周りに回転・ずらして参照する。塗りの数が増えても、1つ当たりのSVGは小さなpatternとmaskだけ増える。resvgはgroupの回転をfeTurbulenceへ反映しないため、筆跡の向きはnoiseではなく帯の図形で作る。粒子や線は小さな共有pattern／mask／filterで表し、油彩はfilterを使わずpathで表す。油彩の塗り幅と間隔は基準の3倍とし、閉じた形の油彩の塗り1つは最大24本の筆跡、作品内の油彩の塗り全体で120本（1つ当たり最低3本）を分け合う。大きな油彩の塗りが多い作品では筆跡を太くしてSVGの大きさを抑える（render engine 67）。内部筋の明暗差は通常0.6／濃い1.05／薄い0.6、濃いの座標簡略化は短辺1000当たり許容差0.25（拡幅前）、薄いの各筆跡opacityは0.54とする。下地と輪郭を拡幅しない。Compatはfilterを持たない近似表現であり、Displayと同一画像とはしない。
...
**記述が明示した個数は、その後のいかなる読み取りより優先する。** Canonical meaningは値をlosslessなsymbolic intentとして保持する。Step 11のpure ceiling preflightは、展開、配列確保、またはその他のO(count) materializationより前に走る。`u32::MAX`等もclampや代表数へsilent rewriteせず、単独primitiveは予算内のsource順prefixを描画して要求数と実行数を診断する。現行runtimeに残る閾値と代表化はcompatibility behaviorであり、canonical countを別の値へ変えるsemantic authorityではない。
...
バッチパネルは入力、次に描く条件、進捗、再開をまとめて複数行の指示を扱う。実行中は現在の行をハイライトし、現在の DDL 解釈を読み取り専用で表示する。バッチ実行は次のバッチ実行まで失敗レポートを保ち、バッチ指示の履歴をユーザーごとに保存する。**履歴は 50 件まで保つ（v2.13.21・それ以前は 20 件）。上限を持つのはサーバーであり、読み出しと書き込みの両方で切る。**一覧はウインドウの高さの半分を天井とし、入りきらないときはスクロールする。

**途中で終わったバッチは、止まったところから続けられる。** 同一セッションでは、開始時の元の記述、安定したrun ID、Stage 1／2モデル・色カタログ・写生・暴れる・キャンバスの条件、未完の行を保持する。停止が最初の成功前でも、再開後に再び停止しても、再開は**まだ保存されていない行だけ**を**元の行番号のまま**その開始時の条件で描く。設定を途中で編集しても、影響するのは次に新しく始めるバッチだけである。サーバーへの作品保存が成功した行は、直後の履歴更新が失敗しても完了として扱い、描き直さない。

ページを再読み込みした後は、このセッション内の未完スナップショットは残らない。既存の保存済み履歴の発見だけで再開候補を探し、行番号と記述の両方で照合する。**保存成功が0件のバッチを、再読み込み後にも再開できることは約束しない。** 再読み込み後に条件を復元できるときは、最後に保存された作品の記録を読む。記録の無い条件は作らない —— 欠落は「off」ではなく「記録より古い」を意味する。
...
デモモードは種となる語句から指示を繰り返し生成し、描画し、設定された間隔だけ待ち、また繰り返す。デモの設定はユーザーごとに保存する。**デモの結果は既定では保存しない** — 記述者は現在の描画を明示的に履歴へ保存できる。デモは入力タブではなく設定モーダルの`制作`に置く。実行中の状態と停止は設定の外にも残り、そこから設定を開き直せる。開始しても記述とバッチの入力本文を置き換えない。

デモも同じ選択で描く。作品条件ヘッダは、現在のカタログ選択だけでなく**描画結果が報告したカタログ**を反映する。**v2.9.39 までデモはユーザーごとのデモ設定に自前の `catalog_mode` を持ち、v2.9.22 までは乱択だった。**
...
SVG のダウンロードでは、Web UI は Display・Editable・Compat・Live の 4 つを出す。Display は保存済みの SVG を落とす。Editable・Compat・Live はサーバーの描画エンドポイントを呼ぶので、**DB に SVG の塊を重複させることなく**、過去の履歴も現在の書き出し構造の恩恵を受けられる。
...
サーバーはこれらを `app_settings.output_save_settings` に `enabled`・`output_dir`・`png_size` として保存する。`INKU_OUTPUT_DIR` と `INKU_OUTPUT_PNG_SIZE` が初期値を与え、未設定なら既定は `~/.local/share/inku/outputs` と 2160px。API の `PUT /api/settings/output-save` は管理者専用で、絶対パスだけを受け取り、PNG サイズを 1080 か 2160 に制限する。作品ファイルを DB から作り直す `POST /api/history/rebuild-output-files` は、自動保存が無効なら 409 で断り、1 回に 50 件までを受ける（2026-09-26）—— ファイルは要求の中で書かれ PNG 化もそこで行うので、1,000 件を許すと要求 1 つがワーカーを数分占めた。
...
App rail のユーザーメニューは、ログイン中の利用者のプロフィールダイアログを開く。ダイアログは `PATCH /api/auth/me/profile` を通じてメールアドレスとパスワードを更新できる。**パスワードの変更は現在のパスワードを要求し**、このエンドポイントは管理者のユーザー管理 API とは別である。**パスワードを設定し直すと、そのアカウントのほかの session は終わる（2026-09-26）。** 管理者による再設定と `inku-admin reset-password` はすべての session を、本人の変更は変更に使った session を残してほかを終わらせる —— 再設定は古いパスワードを知った誰かを締め出すための操作であり、その誰かが古いパスワードで作った session が残れば締め出せない。

設定の可視性は権限グループに応じる。**DB 設定は `admins` グループに属する利用者にだけ見える。ユーザー管理は `admins` と `leaders` に見え、`leaders` には自分の組織グループの通常ユーザーだけが現れる**（作者裁定・2026-09-26。API と CLI は以前から `leaders` にこの範囲を許していた）。`leaders` の画面には権限グループと組織グループの選択、組織グループの管理を出さない —— サーバーはどれも `admins` にしか許さない。プラグインタブはログイン中の全利用者に見えるが、**プラグイン設定の変更とプラグインストレージの更新 API は `admins` に制限する。**
...
並行する描画要求はアプリケーション層で上限を持つ。共有pipelineのmodel effect（色カタログ選択、写生、Stage 1、hole補完）は共有pipeline serviceのworker pool（既定4 thread）で実行し、pool は既定で8件までのexecutionを保持する。保持したexecutionが全て実行中なら、新しい開始を429で断る。各effectはproviderへ一度だけ送り、再試行の要否と待ち時間はcoreが§12.8の有限予算の中で決める。1回の実行で進めるeffect数にも上限（既定32）を置き、無限の滞留を作らない。これらの上限はinstallationのpipeline manifestが持つ。

**描画は 1 アカウントにつき同時に 1 件（2026-09-26）。** サーバー全体の描画枠（既定 2、枠が満杯なら待たずに 503）とは別に、同じアカウントの 2 件目以降は前の描画が終わるまで最大 30 秒待ち、過ぎたら同じ 503 を返す。極端な図形を持つ 1 件の描画は GB 単位のメモリを使うので、1 人が枠を占めると他の利用者の描画も止まる。拒まずに待たせるのは、Web が候補 4 件の描画を同時に求めるためである。
