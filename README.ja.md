...
夜の水面に、青い線がゆっくりほどける
```
inkuアプリケーションの入り口は、まず上のような短い詩歌・散文を書く事からはじまります。言葉は、LLMによって絵画要素に分解され、正規化された言語としてDDLに変換されます。DDLはTyped Compilerによって、ベクターグラフィックの基となるJSONデータに変換されます。このJSONデータは言わば「楽譜」として、アプリケーションの世代が変わっても、同じようにSVGによる作品を「演奏」することが可能です。コンピュータは、このJSONデータをRendererで読み解くことで、SVGイメージを生成します。LLMとTyped compiler・Rendererの組み合わせが、コントロール可能なAIベクターグラフィック生成環境を実現します。
