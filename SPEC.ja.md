...
7. **エンジンは後戻りしない** — 版木を彫り進めるように、描画エンジンは一方向にしか進まない。過去の版をシステムとして保持せず、選び直せるようにもしない。**残るのは刷り上がった作品（保存済み SVG）であって、彫る前の版木ではない**（Section 15.8）
...
暴れるは楽譜を変えずに演奏の幅を変える Renderer 層のノブである（§15.5 の呼び分けに従う）。
...
| computer | periodic_quantized | 揺れを持つが**誤差なく反復する**。整数周期の正弦と格子への丸め。材質は標本化の残り（§15.9） |
...
閾値 240 は `MAX_EXPANDED_PER_INSTRUCTION` に合わせてある。300 に置くと 241〜299 が「literal と規定されているのに coerce が 240 で切る」構造的に守れない帯になる。**「二百三十三本」は 233 本描かれる。**
...
**複数群の literal 合計が 400（`MAX_EXPANDED_PRIMITIVES`）を超える場合は、要求値の大きい群から順に代表化へ倒し、合計が 400 以下になった時点で止める。小さい群を先に削らない。** 数えられる数と数えられない数は別物であり、比例縮小は数えられる側を先に壊す。全群を代表化しても超える場合だけ、大きい群が一つの上限を分け合う。
...
**作品エディション ID = `rh3`**（詳細は §7 の render metadata 節）。
...
### 15.8 エンジンは後戻りしない（版画としての実装、v2.4.6）
...
> **render engine の版だけを通して見るなら
> [docs/spec/render-engine-history.ja.md](docs/spec/render-engine-history.ja.md) が入口である。**
> 全版（1〜15）を新しい順に 1 版 1 節で並べ、**動いた件数と不変の件数**を実測値で持つ。
> 以下の §15.9〜§15.11 は、そのうち engine 13・14・15 の詳細にあたる。

### 15.9 コンピュータのてざわり（v2.6.0、engine 13）
...
後戻りではない。engine 11 のそれは**選べない既定**であり、engine 13 のこれは**選べる語彙**である（§15.8 と衝突しない）。
...
- **「暴れる」は効かない**（§13.4）。暴れは手の話であり、`rotring` と同型に扱う
- **格子**: 中心線の座標を格子へ丸め、幅を 4 段に落とす。**engine 13 の刻みは `ストローク長 × 0.018` で、
  対象ごとに別の方眼になっていた。engine 14 で `キャンバス短辺 × 0.018` の 1 枚の方眼へ改めた**（§15.10）
...
### 15.10 一枚の方眼と、暴れるの到達（v2.7.0、engine 14）
...
§13.4 の「作品全体に一つのトグル」という位置づけとずれていた。
...
| `computer` × 残り 7 プリミティブ | 7 | `periodic` が `WILD_GAIN` を飛ばす（§15.9） |
...
### 15.11 種と痕跡の作り直し（v2.7.8、engine 15）
...
### 15.12 PNG は演奏を写す（v2.7.10、v2.7.11）
