...
def test_checked_in_score_schema_matches_the_live_pydantic_model() -> None:
    ...
    assert properties["version"]["enum"] == ["0.9.0", "0.8.0", "0.7.0", "0.6.0", "0.5.0", "0.4.0", "0.3.0", "0.2.0", "0.1.0"]
    ...
    assert {"start", "end", "layout", "at", "members"} == placement_group.keys()
    ...
    assert {"start", "end", "anchor_indices", "transform_group_indices"} == placement_member.keys()
