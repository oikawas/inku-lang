...
**Target version: v1.92.0 / Build 594**
...
v1.93 refines observation and performance. Build 593 adds an observation option (`include_trace`) that carries the RAW intermediate products of each pipeline stage back in a single generation. Only when requested does the response include a `trace` object (stage1_raw/ddl, plugin_expanded, stage15, stage2_raw_attempts, score_pre_coerce, and so on). Collection is observation-only, threaded through the interpreter and composer; it changes no generation decision, and Score and render_hash stay invariant. The trace is never persisted to the database, and `inku-cli paint --trace` saves it as `<prefix>-trace.json`. Build 594 fixes an instruction that carried both a region (`at`) and a relation having its relation silently discarded during region placement, so touching was never reached. Region placement now runs first and relation resolution follows, so plugin-member double arcs (leaf forms) perform as designed—endpoint-pinned opposing minor arcs (intent-audit finding F-1). Relations that become unresolvable only at performance time drop with a recorded warning per §14.4. The rh2 contract and Score schema are unchanged. Separately, a bench-dedicated container environment (api 8101 / web 5174, its own DB, version-pinned) was stood up on pentala alongside bare metal for release distribution and isolated bench history (operational detail lives in the local `AGENTS.md`).
