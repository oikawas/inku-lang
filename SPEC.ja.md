...
- **履歴ストリップ**: 最近の作品・ホバーのメタデータ・スター印・ページング。**サムネイルの下に添える事実は読み手が選ぶ** —— 世代・モデル・engine バージョン・ファイル容量から最大 2 つで、**0 個も答えである**（何も選ばなければ絵だけが並ぶ）
...
**v2.13.38 の追補:** Stage 2 がフォールバック経路で描かれた作品も、同じ形で履歴に `compose_fallback`（理由）を記録し、UI に層を名指した印を付ける。**この欄は 3 状態を持つ** —— 理由の文字列（落ちた）・`"none"`（落ちなかった）・記録なし（欄より前に保存された作品）。`null` を「落ちなかった」に使うと欄の導入より前の作品と区別が付かないため、書き手は落ちなかったときも `"none"` を明示的に書く。**書き手は保存を行う側**であり、描画経路では server 自身が書く（応答は行が書かれた後に返るので、クライアントは自分が保存を頼んだ作品について後から送り返せない）。**印のある作品を系譜の親として推敲するときは、実行の前に一度だけ確認する。**既存の作品へは遡って書かない。
...
対策Aは v1.98 で `POST /api/paint/stream`（NDJSON）として実装した。解釈完了時に `stage1` イベント（正規化DDL・使用モデル・トークン数・所要時間・フォールバック有無）を送出し、最後の `done` イベントで従来の `PaintResponse` を返す。既存 `/api/paint` は同一ロジックを消費するラッパであり、応答形状は不変（CLI・Android は無改修）。v2.13.39 で合図が 2 つ増えた — 写生層が動いた回だけ出る `sketch`（`stage1` より前）と、Score が確定した時点で出る `score`（`done` より前）である。どちらも本体（写生文・Score）は載せず、`done` の形も `/api/paint` の応答も変えていない。
...
v2.3.0 で閉図形の塗りを領域 fill から**素材の筆致で内側を埋めるストローク塗り**へ変更し、`filled` の意味論を復権した（`True` = 素材の筆致で内部を埋める / `False` = 輪郭のみ。従来は閉図形が `filled` に関わらず常に塗りつぶされる死にフィールドだった）。塗りは走査線と閉輪郭の交点を対で取り、内部区間 1 つ = 1 筆として `synthesize_along` に通す（clipPath 不要。凹形 cloudform も交点対のまま扱え、端点は交点から線幅の半分だけ内側へ寄るので縁が輪郭に揃う）。群は `class="fill-stroke-v1"`。走査角は演奏 seed 由来（0〜180° 一様）で図形ごとに変わり、間隔は `max(線幅 × 1.5, canvas.unit × 0.012)` に ±12% のジッタ。完全被覆は狙わず紙目（隙間）を残す。rotring は領域 fill を維持し（`True` = ベタ塗り / `False` = 輪郭のみ）、走査線 3 本未満の微小図形は領域 fill に縮退する。`surface` 指定時は素材塗りを出さない（塗り = 素材の既定の埋め方、`surface` = 明示的な版表現）。**⚠ v2.13.20（ddl engine 18）で 1 つ例外ができた** —— `surface.texture="solid"` は「素材の既定の埋め方」そのものを言う値なので、**版表現の層ではなく塗りの層へ行く**。おもての質 9 語がすべて `surface.texture` の値になり、塗りだけが `filled` へ行く非対称が消えた（`filled` は残り、coerce の枝が `solid` と `filled=true` を両方向に導出する）。あわせて surface の hatch / crosshatch を幾何直線から筆致の帯（`class="surface-stroke-v1"`）へ差し替え（中心線・角度・間隔・本数は不変、rotring は幾何直線のまま）、演奏されない variation を seed key から除外して不活性な variation の有無で演奏バイトが変わらないようにした（primitive 別の不活性判定。cloudform は輪郭生成器が quality / amplitude / frequency を常に消費するため不活性なのは dimensions のみ）。同一 Score + 同一 seed の演奏結果が変わるため render engine version を 9 へ更新した。**⚠ v2.13.24（render engine 35）で hatch / crosshatch の行は輪郭で切られるようになった** —— 塗りと同じ交点の機構を通すので、行は図形の中だけに残り、凹形では区間ごとに 1 本ずつ描いて空洞をまたがず、輪郭と交わらない行は 1 本も描かない。`clipPath` は使わないので `compat` でも同じ形に収まる。**切る前の層は動かない** —— 角度・間隔・`spacing_gradient`・1 行ごとの揺らぎはそのままで、間隔クラスの値も前の版と同じである（減るのは輪郭の外に出ていた行の本数だけ）。
...
作者のローカルサーバー固有の運用詳細は公開仕様へ含めず、Git管理外の `AGENTS.md` または `no-git-sync/` に集約する。通常開発はMacからrsyncで同期してsystemdサービスを再起動し、**負荷が継続する試験と凍結物の焼き直しは配備先ホストのテスト専用コンテナで回し**、Docker Composeはリリース前など節目の本番構成検証に使用する。
...
**SVG の属性へ書き出す数値は小数 6 桁で固定である。** SVG は拡大自由だが書き出した数値そのものは固定小数なので、この桁が実質的なマスター解像度になる（1000 単位のキャンバスでキャンバス比 1e-9）。桁は書き出し箇所ごとに決めず、**出力の単一地点で全属性へ強制する** —— 識別子として扱う `version`・`class`・`id` だけが例外である。

- **下限（再現性）**: プラットフォーム間の libm 差は 1000 単位の座標で絶対 1e-13 に相当し、この格子より 4 桁下にある。どの機械で演奏しても同じ文字列が出る
- **上限（物理）**: 100m の壁へ引き伸ばしても刻みは 100nm で、可視光の波長より細かい
- **末尾の 0 を詰めない**（2026-07-24 裁定）。桁が固定なら成果物のどの数値も `-?\d+\.\d{6}` に一致し、格子に載っていることを正規表現 1 本で機械検査できる。詰めると、丸めてから詰めた値なのか生の浮動小数なのかを出力から見分けられない

**6 桁は仕様であり、書き出す量を減らすために桁を下げない**（2026-08-16 裁定）。**この格子は座標だけでなく質感フィルタの設定値にも掛かるので、桁を下げると絵が変わる。** 実測では、本番規模の作品を 3 桁へ落とすと原寸 1618px で 19.52% の画素が動き、フィルタの数値だけ 6 桁に残すと 0.50% に収まった。2 桁では座標だけでも動く（塗りで 17.52%・本番規模の作品で 48.57%）—— 粗い格子は、手の揺れで散らばっていた点を同じ座標へ集める。**動くのは墨の量ではなく粒の置き場所である。** 桁を下げて得られるのは書き出す量だけで、gzip 後の転送量で 3 桁 −31.9%・2 桁 −44.1% にあたる。
