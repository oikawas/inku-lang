...
描画headまたはrelationを含む補完要求には、既存の修飾語・数量・動作・位置の結合構文を短く示し、語の種類と確定したownerの役割を区別する。固定参照句は既存Saijiki assetから要求言語で投影する。未知の参照表現にはrelation factがまだない場合もあるため、そのfactの認識だけを案内の前提にしない。新しいalias、文法、応答fieldは追加せず、確定fact・owner・数量・診断の検証を維持する。
...
単独primitiveのhead・明示count・actionが同一clause内で確定し、group・sequence・continuation・fill targetを持たない単純な参照namespaceでは、その句の言い換えも個別に検証できる。候補を再compileし、全instructionのsource順head identity・count・actionが不変で、修正対象が外向きの参照依存を持たないことを追加確認する。後続の未解決参照だけを理由に先行ownerの修復を拒否しない。依存構造が不明な単位はまとめたままにし、確定fact／owner／範囲外診断の既存検証と採用候補全体の再compileを省略しない。
