...
同じ `DDL から描画` の操作は解釈ボックスの下にもあり、ダイアログを開かずに素早く再演できる。候補の metadata は、当てはまるところで render、composition、variation、interpretation の seed を示す。**ダイアログ自身は描画を始めない** — 描画の操作は主となる単一描画パネルに集めたままにする。
