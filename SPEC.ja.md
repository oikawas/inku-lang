...
日本語では「1つ〜9つ」、十個を表す「10／十／とお」、および「1個・2個・10個・11個」などを使える。「10つ・11つ」は個数表現として採用しない。英語もgroup ofで組を記述し、`Line up five, alternating a group of a red circle and a blue line with a gray arc.` は同じ5単位となる。
...
作品計画の前に、任意の**写生**を一度だけ置ける。写生は記述を書き換えず、記述が言外に含むのに書いていない**場所の広がり**と**季節・時刻の光**だけを、物の言葉で1〜3文に補う。Stage 1は記述と写生文の両方を受け取り、写生文からは場や奥の層・背景の色を加えるだけで、主題・動き・向き・個数・配置は記述に従う。写生文は記述の代わりに置かれず、記述は作品の出自として保存・表示に残る（§12.16）。

写生の効かせ方は描画ごとの選択で、既定は**自動**である。

- **自動**（`auto`）：写生者はまず、記述が描くための手掛かりを足りるだけ持つかを判断する。足りていれば補わず、記述だけで描く（`not_needed`）。判断の基準は記述の形式（和歌か散文か）ではなく、手掛かりの量という一般基準である。<!-- 判定規則は開発用評価で確定する -->
- **なし**（`off`）：写生を通さない。
- **あり**（`always`）：記述に手掛かりがあっても補う。作者が作品ごとに「写生ありで描き直す」を選ぶ場合に使う。
- 作者が直した写生文、または保存済みの写生文は、写生を呼ばずにそのまま使う（`supplied`）。

写生は**確認待ちで止まらない**。描画を始めてから画が出るまでに、作者の承認を挟まない。写生の要求が失敗しても、記述だけで作品計画へ進み、描画は完走する（`fallback`）。写生文は描画後に表示・編集でき、編集すれば描き直す。作品ごとの切替「写生なし／ありで描き直す」は、選んだ作品の子として系譜へ保存する（派生種別は`sketch_grain_change`、metadataは`from_sketch_state`・`to_sketch_mode`）。

共有pipelineでは、写生はStage 1前の任意effect `generate_sketch`（結果`sketch_generated`、prompt `inku.sketch-supplement-prompt.v1`）である。開始入力と記述からの再生成は`sketch`（`off`／`auto`／`always`／`supplied`）を持てる。snapshotの`sketch`記録（`pending`／`supplemented`／`not_needed`／`fallback`／`supplied`）が写生の結果を示し、保存列`sketch_state`は`supplemented`（写生文あり、`sketch_grain`は空）・`not_needed`・`fallback`・`off`になる。再試行予算は`sketch_retry`（無ければ色カタログ選択の予算）。旧写生層（§12.15）の`fine`／`coarse`の区切りは保存互換の表示だけに残り、新しい写生では使わない。
...
粒子や線は小さな共有pattern／mask／filterで表し、油彩はfilterを使わずpathで表す。油彩の塗り幅と間隔は基準の3倍とし、閉じた形の油彩の塗り1つは最大24本の筆跡、作品内の油彩の塗り全体で120本（1つ当たり最低3本）を分け合う。大きな油彩の塗りが多い作品では筆跡を太くしてSVGの大きさを抑える（render engine 67）。内部筋の明暗差は通常0.6／濃い1.05／薄い0.6、濃いの座標簡略化は短辺1000当たり許容差0.25（拡幅前）、薄いの各筆跡opacityは0.54とする。下地と輪郭を拡幅しない。Compatはfilterを持たない近似表現であり、Displayと同一画像とはしない。
