...
### 2026-09-25 — Faster, configurable Android camera capture and description

The bottom "Camera" captures in an in-app camera, skipping the system camera app and its review screen; if the permission is refused or the camera cannot start, it falls back to the system camera. The model that turns a photo into a description can now be chosen in Settings. The default, on-device Gemma 4 E2B, keeps the photo on the device; choosing a remote model sends a downsized, re-encoded image without location data to that provider, and the settings screen says so. Gemma on Gemini runs with minimal thinking, cutting description time from a 26.7 s median to 5.2 s (20 public photos, Pixel 9). The description prompt now asks for layout, the main subjects with simple forms and counts, colors by area, light and time, and texture (`camera-description-v4`). An on-device model does not keep to a requested length, so its generation stops at a sentence end past a character budget (E2B median 17.7 s). Drawing from the description no longer uses the fixed NVIDIA NIM and `vivid_material`; it uses the drawing settings' models, color catalog (including auto), and sketch. An on-device description model loads while the camera screen is open.
