...
Only deterministic layers carry versions. Stage 1 and Stage 2 are LLM layers that can vary for the same input, so they retain the digest of the prompt actually sent as provenance rather than a version. `render_engine_version` rises when the same Score and seed perform differently, or when the performable vocabulary grows. A rename of a word or tool does not raise it: it re-freezes the affected artifacts in place rather than opening a new version directory. `ddl_engine_version` rises when a deterministic transform's output changes and when `Instruction` field declaration order changes; changing the order measures both the field that moves and the field that gives up its position. `ddl_version` rises when grammar or vocabulary is added, changed, or retired; Score `version` rises when the schema structure changes. `MODEL_CONFIG_VERSION` rises when measurements, recommendation levels, or selectability change. `web/APP_VERSION` is the single authority for `APP_VERSION`; the distributed version in `server/pyproject.toml` changes only for a release tag. `web/BUILD_NUMBER` is a shared serial that also moves for UI changes. Implementations and saved works own the current values; version-specific values and reasons remain in the [render engine version history](docs/spec/render-engine-history.md) and [changelog](CHANGELOG.md).
