...
REASON = (
    "The color cycle stops inventing an order. coerce inserted an instruction's "
    "own color into its cycle without looking, so a color already there took "
    "twice the members, and `_color_repair_order` filtered the requested colors "
    "through a six-word table that predates yellow, orange, and purple, so a "
    "work naming an old color and a new one lost the new one. The table is now "
    "a known order for determinism rather than a ranking, and unnamed colors "
    "follow it instead of falling out. None of the 15 expand cases moved -- the "
    "change is entirely inside coerce -- and 2 of the 21 coerce cases moved: "
    "B-production-fill-clause loses the duplicated white, and B-presence-from-ddl "
    "keeps a yellow the table used to drop -- which gives that group a fourth "
    "member, because a cycle with no count of its own takes one member per color."
)
