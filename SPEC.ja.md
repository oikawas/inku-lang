...
記述タブは入力、次に描く条件、描画操作の順に配置する。モデルと色カタログは項目名・変更操作・現在値を一つの行にまとめ、長い名称を折り返して読めるようにする。解釈と描画のモデルが異なる場合は別々に示す。写生とキャンバスはボタン内に現在値を、暴れるは入／切を示す。指示書の新規作成は制作側に、表示中作品の写生と指示書は区切りと見出しの下に置く。指示書の編集は本文の見出し横、指示書からの描画は本文直下に置く。既存の入力ロック、進捗・停止・エラー、判断が必要な通知、開閉状態の保存、指示書編集による子の保存は保持する。
