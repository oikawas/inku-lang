...
### 2026-09-25 — Restore label cutting, stream progress, and LLM execution status on the shared pipeline

The switch to the shared pipeline dropped two Server rules: leading numbers and bracketed comments were no longer cut away from the drawing layers, and a description made only of labels was no longer refused with 400. Labels reached Stage 1, the sketch, and color catalog selection as written. A description start and a regeneration from a description now cut labels only from the text handed to the core, and the work keeps the description as written. A label-only description is refused with 400. `/api/paint/stream` had also become a single `done` line; it now re-reads the execution and reports `sketch` (when the sketch ran), `stage1`, `score`, and `done` in order. A refusal before the first event arrives as its HTTP status, and a failure after it as an in-band `error` event. The settings execution status (`stage_execution`) counted an old Stage executor that nothing submitted to; the executor is gone, and the status now shows the shared pipeline worker pool's limits and the counts of provider effects (submitted, completed, failed, timed out, refused for capacity). The response shape is unchanged. `INKU_STAGE_WORKERS` and `INKU_STAGE_QUEUE_LIMIT` are no longer used, and SETUP and SPEC §22 now describe the shared pipeline worker pool. The route-count test constant and the API surface record match the current 105 endpoints, and stale descriptions in PROJECT_CONTEXT and the implementation were corrected. DDL, Score, and render versions do not change.
...
The hole-completion prompt now includes concise existing attachment syntax and projects fixed reference literals from the existing Saijiki asset. It explains modifier, action, and position binding beyond the vocabulary list, without withholding reference literals until a reference has already been recognized. It distinguishes unbound vocabulary from established owners and describes the default distribution domain of an unpositioned scatter, without permitting guesses that remove explicit meaning, regions, or positions. Normal Stage 1, compiler grammar, and validator preservation requirements are unchanged.
...
Hole-ID validation for normal responses and structured logs accepts the compiler's actual `hole:` prefix followed by 64 lowercase hexadecimal characters. It excludes malformed IDs without discarding valid per-hole diagnostics.
...
When an explicit count on a standalone primitive exceeds a resource limit, the source and Plan retain the request while the safe source-ordered prefix reaches the drawing. For example, a request for 300 under a per-template limit of 240 draws 240 and records requested 300, executed 240, and the budget reason in response diagnostics, history, and the structured log. Coordinated placements, Macros, and other units whose structure would be broken by partial execution remain atomic omissions.
...
A clause containing words outside the finite grammar can now enter the existing Stage 2 as one bounded known hole when it has an exact drawing head, ground, or background anchor. Its request carries the smallest region for each hole, established typed facts, finite Saijiki vocabulary, and lock/range identities. It also states the existing validator acceptance rule: retain established fact source occurrences and counts while producing a complete drawing owner/action. Phrases outside the finite grammar are normalized from the region's whole intent to the nearest existing vocabulary and grammar instead of being copied unchanged; if the allowed span happens to cover the source, all of that selected text is returned only as the edit replacement. A region with typed drawing-head or action facts cannot be resolved by background or ground alone: the validator also requires its head and action to be bound in the same executable drawing instruction. An unknown clause without an anchor is not disguised as a patchable hole and no owner is inferred from source order; it remains a local diagnostic while independently complete drawing instructions reach the Score under either error policy. When a bounded hole and a local diagnostic coexist in the same source, the local diagnostic remains visible while the existing Stage 2 completes the hole; only global or integrity failures forbid that patch. A following continuation blocked only by a patchable upstream hole defers that diagnostic until recompilation without adding the following clause to the request or rewrite range. The provider candidate is recompiled and becomes a visible patch candidate only when it retains the identity and count of established facts, preserves local diagnostics outside the selected range, and produces a drawable owner. Rejection records a safe validator class as failure detail without retaining the raw response. This adds no per-word alias, extra LLM stage, Score generation, or rewrite of unselected source.

The Stage 2 response schema now requires one edit for every selected hole. When multiple modifiers with the same owner share one explicit total, one short general rule tells the provider to make each modifier a complete member through its drawing head and bind the single total with the existing alternating-placement grammar, rather than coordinating bare modifiers. This is not a branch for a word, DDL, or history ID.

The Gemini API path sends the shared core's response schema as one forced function call and sets Gemma 4 thinking to minimal. Only the transport representation is projected to Gemini's accepted JSON Schema subset; `const` becomes an equivalent single-value `enum`. For Stage 2, repeated per-hole `oneOf` objects are compacted into one bounded enum object so function-schema depth does not grow with the hole count. Returned arguments still pass the original full shared-core schema, exact hole/range/digest pairing, and compiler-lock validation. The host does not retry automatically and does not accept free text or another function as a completion result.
...
Send the shared pipeline's JSON response schema to the model, using the established provider-specific structured output for OpenAI-compatible connections. Restore Stage 1 guidance for interpreting free descriptions, selecting Macros only for explicit subjects, and using finite instruction and reference grammar, reducing instruction-array and explanatory-text failures. Shared core retains visible-DDL validation, patch approval, and retry ownership. When its compiler rejects an uncommitted Stage 1 candidate, the remaining Stage 1 budget requests complete DDL again with the original description, rejected DDL, and bounded reason/span diagnostics under a new action digest; the rejected candidate is not committed as visible DDL or a work, and accepted author DDL is unchanged. Web distinguishes pipeline failures and patch approval from provider HTTP errors and shows the reason for the current phase. Deterministic drawing and DDL semantics, versions, and the camera vocabulary projection are unchanged.
...
The shared Rust registry is authoritative for the 11 new-paper formats. The former `pixel9_landscape_safe` option is treated as device display margins, while existing works retain their 9:5 ratio and saved images. Migrating that old preference uses `square` as the default paper for new works and does not alias 9:5 to 16:9. The standalone Android shared-Rust connection is implemented; work acceptance and deployment follow separately. iOS is outside this change and remains separately pending. APP_VERSION, non-Android version numbers, and deployment state are unchanged; Android BUILD_NUMBER changes only through the normal automatic build allocation.

### 2026-09-13 — Wait for history identification before drawing

Starting description generation or DDL drawing immediately after selecting ordinary history now waits for that same history to be identified, then takes its corresponding fork. A pending lookup no longer means a new work, preserving the original description, saved settings, and parent relation. A failed lookup is returned to the authoring operation. Cancelled operations and operations invalidated by a selection change cannot start later. The saved work remains visible while identification proceeds.

An independent review judged the Step 13 normal Server/Web connection complete. The existing representative flow checked pending and failed lookups, cancellation, and invalidated selections; the Web check also passed. Existing persistence, diagnostic, and native replay evidence was reused without repeating full suites, native drawing, or the Web build. Steps 14, 16, and 17 have not started, and this judgment does not mean the change has shipped or been deployed. Versions and deployment state are unchanged.

### 2026-09-13 — Connect the normal Server and Web to the shared authoring pipeline

The normal Web and existing interpret, compose, and paint APIs now use the same shared pipeline service. The host makes one attempt for each provider action requested by core, while core retains the retry decision. Visible DDL is CAS-saved in one transaction with immutable origin, monotonic authority, and revision, preventing an old tab from overwriting newer work. A known hole produces a completion proposal without another start operation, but the proposal is not adopted without author approval. Authority for an old work is never inferred from its text. Editing old DDL or regenerating from an old description forks a new variation with a saved parent relation and preserves the original history. An older shared-pipeline history also keeps its selected revision's performance instead of being replaced by the latest result from the same variation. Its history link stores the source digest and a fork sidecar for that point's config and host context; a missing sidecar or revision/digest mismatch stops instead of consulting the latest snapshot. Description regeneration from an existing variation preserves immutable core config by forking a new edition with the current options. New works save the raw Score and link the history row to its authority revision. The shared core is also the source of truth for the 11 canvas IDs and integer ratios. New-document MacroLock digests now include the canonical `sha256:` prefix; an actual Macro invocation passed from its owned Start through acknowledgment to Score readiness, while an unknown Macro blocks instead of masquerading as a known hole.

In a representative normal Linux-host check, an ordinary point fill requested 6,945 logical objects against a fixed fixture budget of 400 logical objects, 512 template nodes, and 400 for the other structural resources. Only that fill was omitted; a three-point circle fill and a later line continued, producing four primitive marks. Saving the SVG, raw Score, history, and authority link passed. This checked the native connection; it did not measure the six new shipping limits. The six additional shipping limits for new works are 4,096 logical objects, 128 template nodes, 4,096 anchor instances, 4,096 transform instances, 64 placement instances, and 64 fill instances. The existing four limits, administrator authority, and budgets saved with old works remain intact. Only an over-budget placement is omitted, and independent later work continues.

The normal-host Linux check for replaying a saved compact Score on the latest engine also passed. It left the raw Score, saved source, and authority unchanged, delivered a changed seed as an SVG difference, and used independently saved policy to reject a request-side hard-budget alteration while retaining four primitive marks. Managed-history lineage editing now identifies the history-row owner, takes its linked fork, and preserves the old fork. Active `/executions/{id}/author-ddl` receives source, revision, and options. With unchanged settings, a source change saves metadata while retaining the existing CAS, origin, and DDL-authority lock. A changed canvas, wild setting, or other option creates a parent-linked direct-DDL variation without altering the original source, config, or authority; changed metadata with unchanged source saves a new edition instead of discarding the existing result. History sidecar v2 immutably stores the four core diagnostics, renderer diagnostics, and `resource_execution` for the matching revision and source for ordinary history display. V1 has no diagnostic record. A corrupt sidecar warns only for that work while saved DDL, Score, and SVG remain visible, without inferring latest state or recompiling. Re-review judged integration unfinished. Selecting ordinary history (legacy history or no link hint) and drawing DDL before its discriminating GET completes creates a parentless new variation instead of a fork, without inheriting the original description or saved settings. The saved source work itself is unchanged. F2 option delivery and F3 diagnostic history are fixed. Step 14 Android integration, Step 16 work acceptance, and Step 17 deployment have not started. Connecting the normal source does not mean it has shipped. DDL engine 37, Score 0.10, render engine 59, APP_VERSION 2.14.2, and BUILD_NUMBER 1073 are unchanged.
...
The typed Stage 1 prompt contains bounded projections of finite Saijiki-derived vocabulary, resolved catalog and canvas identities, and validated Macro identities, parameters, and localized summaries. Known holes in committed DDL now cause the shared pipeline to construct a completion request without another user operation. With no holes it does not call the Stage 2 LLM; unknowns, conflicts, and integrity errors are excluded. Hole completion is limited to hole IDs, allowed spans, range digests, and base-source and compiler-lock digests explicitly reported by the typed compiler, and requires explicit author approval. The shared compiler result serializes the Score, complete diagnostic metadata, and source, semantic, effective, and resource identities; rendering also checks that canvas geometry and palette still match the compile context.

A two-buffer boundary now accepts UTF-8 JSON snapshot bytes and input-envelope bytes and returns JSON bytes. A UniFFI candidate with binding API 1.0.0 exposes that boundary unchanged, contains no semantic branch, and converts a panic to a stable `internal_invariant` envelope rather than platform exception text. The representative automatic-detection/approval/commit/reparse/replay flow without a completion-start command, authority lock, and prompt boundary checks passed. The UniFFI facade build and a representative byte call through the generated Python binding also passed; independent completion review and owner assessment passed for the shared control layer. Existing DDL and Score schema versions, DDL engine 37, Score 0.10, render engine 59, APP_VERSION 2.14.2, and BUILD_NUMBER 1073 are unchanged. Current server and Android authoring runtimes remain on legacy paths; Step 13/14 candidate host integration, Step 16/17 acceptance and cutover, binding integration into each host, deployment, and release are not included.
...
### Android — The port keeps only the roads the server has, and the guards say what they compared (android `2.1.4-android.43`, 2026-08-17, ledger I-311, I-312, I-313)

**Three leftovers from the previous cycle, closed on one branch. All three are the same story: a test was not measuring the place it believed it was measuring.**

- **A road the server does not have was removed (ledger I-311).** When the wobble branch left the machine pole's line, the only caller of a **private pass-through delegate** went with it. The function was deleted whole. **⚠ A function of the same name lives on the geometry object and has two callers, so it stays** — the contract had pointed at that one and said "do not delete it" (an issuing-side mistake). **This stage carries no perturbation**: putting a function nobody calls back changes neither a drawing nor a test. **"Only three perturbations" is not a gap; it is what the measurement implies.**
- **A weakened acceptance moved to the road where its gate is live (ledger I-313).** The test asserting that "a line is not moved by a wobble on `radius` alone" **built its line on the machine pole**, and that pole no longer reads any wobble gate — so the claim had stopped measuring **that the gate splits into a line half and a contour half**. The line moved to a hand tool (`pen`).
- **Four guards now say how many they compared (ledger I-312).** Four tests that slice a group and compare `d` were **green with nothing on either side**. Each now asserts that the extraction from the reference holds at least one mark. **Not one constant is written by hand** (the four bodies were checked mechanically for bare numbers: zero). **⚠ One of the four already carried the claim, but after the count-equality assert**, so the count was what fell first. It moved ahead so all four fail the same way.
- **⚠⚠ Both new acceptances turned out never to have fired their own lines.** The implementation found this while doing the stage-4 check:
  **(1) An acceptance whose input is the frozen corpus cannot be reddened from production code** — a perturbation can only move the port's side, and nothing it breaks empties the reference. **P-1 and P-2 do redden four tests, but what they redden is the count-equality `assertEquals` that was already there.** Firing was confirmed with a perturbation on the group name the test itself names (**the line that fell was the claim itself**). Filed as ledger I-314.
  **(2) An acceptance meant to measure a gate was measuring the seed instead.** The port's `variationJson` **carries a hand-copy of the gate's condition**, so naming an axis **rebuilds the performance seed's material regardless of the gate's answer**, and the drawing changes. With only five words in the `quality` enum, **the inputs that open the gate and the inputs that move the seed are exactly the same set**, so byte inequality cannot separate them in principle. Distance could not separate them either (the wobble is smaller than the envelope the hand already draws, and the wobbling line sits **closer** to the axis). The claim was re-seated on **how often the mark crosses its own axis** (6–9 times when the gate refuses, 38 when it opens), with the seed-only condition kept inside the test as a noise floor. The hand-copy was filed as ledger I-316.
...
### v2.13.40 — the words coerce judges a description with live in one place (Build 927, 2026-08-17, [I-115])

**coerce reads words in the description and repairs the Score. Those words were written in two places** — the declared side, `COERCE_MARKERS` in `language_support/{ja,en}.py`, holding **574 words**, and **119 more written straight into the branches of `coerce/compose.py`**. **A reader of the vocabulary file saw 574 of the 693 words; the rest were findable only by reading the branches one at a time, and the port had no way to know they existed.**

- **44 sites / 119 words moved to the declared side as 45 new systems** (systems **27 → 72**; ja 70 / en 71). **One site is one system**: bundling them would produce "declared, but its contents cannot be read", which only moves today's problem into the vocabulary file. **Not one word was dropped, added or respelled** (**574 + 119 = 693**, with no loss to duplication — though **270 words appear in two or more systems**).
- **⚠ The contract's count of "39 sites / 110 words" was wrong in four places.** The issuing side's scanning tool was blind to three shapes: **① `x not in text`** (`"crescent" not in ddl.lower()` was a judgement word), **② a tuple bound to a name** before it is used (`scene_markers = (...)`, five sites), and **③ literals matched against a `note`**, which it counted as vocabulary (−9 words). **The author ruled on 2026-08-17 that every undeclared word found by measurement moves.** The new guard sees all three shapes.
- **Not a byte of any drawing moved.** The move changed no matching expression; it replaced literal tuples with constants. **`_marker_in_text` still applies a word-boundary pattern to ASCII words while a bare `in` is a raw substring test** (`"slow"` does not match `"slowly"` through the first and does through the second), and that difference was preserved site by site.
- **Order was measured to carry no discriminating power.** **No site returns the matched word itself** (all 56 return a truth value), and **a tree with every moved system reversed runs the full suite with not one test differing from the baseline**. An acceptance about order would have nothing to hold, so none was added.
- **The two languages were deliberately not evened out** (adding a word to balance a pair would change behaviour). **The three systems that only one language has a word for are listed by name** — `autumn_leaf_fall` (ja only; the original condition had no English word), `crescent_scene` (en only) and `radius_clause` (en only; the Japanese half, `半径`, sits inside a regular expression on the same line). **A system that becomes single-language without being on that list reddens.**
- **⚠ Some literals were deliberately left in place**: eight sites whose words are all declared already (outside the range the author chose), and **three that cannot take marker form because they are regular expressions**. **Four of those sit in the same `if` chain as three that did move** — half a chain now reads the declared side.
- **The specification became literally true with this change.** §6.5 says language-specific vocabulary must not leak into the shared core and that each language support owns coerce's per-language markers. **While the 119 words lived in `compose.py`, that statement was false. Not one line of the specification was edited** (both languages carry the statement and they correspond).
...
### Android `2.1.4-android.47` — representation bands use device-local integer ratios (2026-08-22, [I-271])

- **Density bands are `3/2` and `2/3` of the representation maximum; cluster bands are `25/6`, `2/1`, and `1/1`.** Cluster band values also scale with integer arithmetic from the shipping maximum of 120, preserving the same ratios when a device-local ceiling changes.
...
### Android `2.1.4-android.51` — model routing no longer guesses from list order (2026-08-23, [I-351])

- **Provider selection now follows explicit prefix, exact ownership by exactly one enabled provider, then default-local.** Zero or multiple owners no longer select the first list entry, so Room ordering cannot change the answer.
- **Disabled providers do not count as owners, while an explicitly named disabled provider keeps its identity and stops before execution instead of redirecting.** Prefix-like unknown words and internal model-ID colons are preserved, and OpenAI-compatible requests remove only their own leading provider prefix. No per-stage setting, `ModelRequest`, provider catalog, Room schema or migration, pipeline, rendering, persistence, server, Web, or shared path changed.
- **Verification:** the focused pre-production JVM test exited 1 only on the missing resolver/helper symbols. `RoutingModelProviderTest` exited 0 during implementation, at the branch tip, and in acceptance. Full JVM, instrumentation, devices, network, real models, Room migration, reference generation, and pentala were out of scope.

### Android `2.1.4-android.52` — starring a work directly in the history strip (2026-08-23, [I-354])

- **Each history thumbnail now has a `★` or `☆` Star control at its upper-right corner.** It is a separate tap target from selecting the thumbnail, so starring alone does not replace the work shown on the canvas. The existing drawing and refinement lock disables both actions.
- **The UI connects only the existing `HistoryListItem.starred` state and `toggleStar(HistoryListItem)` action.** I-349's selection ring, taps, scrolling, and visibility rules and I-350's model label are preserved. No new producer, ViewModel action, repository or DAO query, Room schema or migration, persistence, pipeline, rendering, server, Web, or shared path changed.
- **Verification:** the focused pre-production JVM test exited 1 only on the missing helper. `HistoryThumbnailStripTest` exited 0 during implementation, at the branch tip, and in acceptance. Main did not move after the branch point and the merge had no conflicts, so the same test was not repeated after merge under the Fast rules. Full JVM, instrumentation, devices, screenshots, reference generation, and pentala were out of scope.

### Android `2.1.4-android.53` — copying the visible Prompt or JSON (2026-08-23, [I-355])

- **The Prompt and JSON tabs on the ordinary Compose screen and the existing Canvas panel now have a `Copy` control.** Prompt copies the complete Stage 1 and Stage 2 input and system-prompt text being shown; JSON copies the complete rendered-information text being shown. Display and clipboard share the same computed string, and no control appears for the `Artwork` tab or with no selected work.
- **The UI connects only the existing `LocalClipboardManager` and display helpers.** Tab switching, body text, hash copying, export, and canvas messages are preserved. No new ViewModel state or action, producer, generation rule, repository or DAO query, Room schema or migration, persistence, pipeline, rendering, server, Web, or shared path changed.
- **Verification:** the focused pre-production JVM test exited 1 only on the missing helper. `RenderTabCopyTest` exited 0 during implementation, at the branch tip, and in acceptance. Main did not move after the branch point and the merge had no conflicts, so the same test was not repeated after merge under the Fast rules. Full JVM, instrumentation, devices, screenshots, reference generation, and pentala were out of scope.

### Android `2.1.4-android.54` — reading saved provenance (2026-08-23, [I-356])

- **The ordinary Compose screen and the existing Canvas panel now have a read-only `Provenance` sheet for the selected work.** Saved sketch state, Stage 1 and Stage 2 models and languages, seeds and variation, color catalog, canvas, render hash and engine, creation time, and elapsed time are arranged in five sections. Null, blank, or malformed metadata is shown as `—`.
...
- **Verification:** the focused pre-production JVM test exited 1 only on the missing projection helper and types. It exited 0 during implementation and at the branch tip. Because main advanced with unrelated Web changes after the branch point, overlap and conflict were confirmed at zero and `GenerationInfoSheetTest` was run once after merge, exiting 0. Full JVM, instrumentation, devices, screenshots, reference generation, and pentala were out of scope.

### Android `2.1.4-android.55` — showing the color map saved on the work (2026-08-23, [I-357])

- **The Performance section of the Provenance sheet now shows each color word, code, and swatch saved in `render_color_map`.** Entries have stable color-word order, and no row appears for a missing or empty map or malformed metadata. An invalid code uses a neutral swatch without hiding or rewriting the saved string.
- **The UI reuses only the existing `workColorSnapshot(renderMetadataJson)`.** It never recalculates the assignment from the current color catalog. No producer, color-code rewrite or normalization, ViewModel state or action, repository or DAO query, Room schema or migration, persistence, SVG analysis, pipeline, rendering, server, Web, or shared path changed.
- **Verification:** the focused pre-production JVM test exited 1 only on the missing color-map projection helper and type. `GenerationInfoColorMapTest` exited 0 during implementation and at the branch tip. Main did not move after the branch point and the merge had no conflicts, so the same test was not repeated after merge under the Fast rules. Full JVM, instrumentation, devices, screenshots, reference generation, and pentala were out of scope.

### Android `2.1.4-android.56` — showing the color-catalog name saved on the work (2026-08-23, [I-358])

- **The existing `Color catalog` row in the Provenance sheet now shows the catalog name saved on the work with the ID from the same snapshot as `saved name (catalog-id)`.** It keeps the prior ID-only display when the name is missing or blank, the snapshot is absent, the map is empty, or the metadata is malformed, and it does not duplicate equal names and IDs.
- **The UI reuses only the existing `workColorSnapshot(renderMetadataJson)`.** It does not consult current `ColorCatalogs` or a rename table, and it does not show `catalogSub`, preserving the Web decision that removed the fixed tagline. No producer, ViewModel state or action, repository or DAO query, Room schema or migration, persistence, pipeline, rendering, server, Web, or shared path changed.
- **Verification:** the focused pre-production JVM test exited 1 only on the missing catalog-name formatting helper. `GenerationInfoCatalogNameTest` exited 0 during implementation and at the branch tip. Main did not move after the branch point and the merge had no conflicts, so the same test was not repeated after merge under the Fast rules. Full JVM, instrumentation, devices, screenshots, reference generation, and pentala were out of scope.

### Android `2.1.4-android.57` — inspecting both saved models from the history strip (2026-08-23, [I-359])

- **The existing compact Stage 1 label in the history strip now has a two-line tooltip for the saved Stage 1 and Stage 2 model IDs.** It preserves complete IDs with provider prefixes, shows `—` for a missing Stage 2 value, and keeps both the label row and tooltip target absent when Stage 1 is missing.
- **The visible fourteen-character label and strip interactions are preserved.** Thumbnail dimensions, work selection, Star, selected-position scrolling, and the drawing or refinement interaction lock are unchanged. The UI uses only the existing `HistoryListItem` and Material tooltip; no new state, model resolver, repository or DAO query, Room schema or migration, pipeline, rendering, server, Web, or shared path changed.
- **Verification:** the focused pre-production JVM test exited 1 only on four unresolved references to the new tooltip-text helper. `HistoryStripModelLabelTest` exited 0 during implementation and at the branch tip. Because main advanced with unrelated Web changes after the branch point, overlap and conflicts were confirmed at zero and the same test was run once after merge, exiting 0. Full JVM, instrumentation, devices, screenshots, reference generation, and pentala were out of scope.

### Android `2.1.4-android.58` — showing saved time and color-catalog ID in the history-strip tooltip (2026-08-23, [I-360])

- **I-359's model tooltip now also shows the work's saved time and saved color-catalog ID.** The timestamp uses the same ISO-8601 UTC representation as the Provenance sheet and reuses the existing localized labels. A non-positive or unconvertible timestamp or a blank catalog ID is shown as `—`.
- **The UI reads only the existing `HistoryListItem.createdAt` and `colorCatalogId`.** It does not look up a name from the current color-catalog list and preserves I-359's model IDs, missing-value behavior, fourteen-character visible label, thumbnail dimensions, work selection, Star, scrolling, and interaction lock. No new state, repository or DAO query, Room schema or migration, pipeline, rendering, server, Web, or shared path changed.
- **Verification:** the focused pre-production JVM test exited 1 only on twelve unresolved references to the four planned tooltip-formatter arguments. `HistoryStripModelLabelTest` exited 0 during implementation and at the branch tip. Main did not move after the branch point and the merge had no conflicts, so the same test was not repeated after merge under the Fast rules. Full JVM, instrumentation, devices, screenshots, reference generation, and pentala were out of scope.

### Android `2.1.4-android.59` — showing the work hash and canvas in the history-strip tooltip (2026-08-23, [I-361])

- **I-360's model tooltip now also shows the saved work hash and canvas.** The short render hash uses the established `F` prefix and existing `Provenance hash` label, while `canvasAspect` uses the existing `Canvas` label. A blank value is shown as `—`.
- **The UI reads only the existing `HistoryListItem.renderHashShort` and `canvasAspect`.** It neither loads the full item nor recalculates from current canvas options, and it preserves I-360's model IDs, saved time, color-catalog ID, fourteen-character visible label, thumbnail dimensions, work selection, Star, scrolling, and interaction lock. No new state, repository or DAO query, Room schema or migration, pipeline, rendering, server, Web, or shared path changed.
- **Verification:** the focused pre-production JVM test exited 1 only on twelve unresolved references to the four planned tooltip-formatter arguments. `HistoryStripModelLabelTest` exited 0 during implementation and at the branch tip. Main did not move after the branch point and the merge had no conflicts, so the same test was not repeated after merge under the Fast rules. Full JVM, instrumentation, devices, screenshots, reference generation, and pentala were out of scope.

### Android `2.1.4-android.60` — deriving the VersionInfoPanel instrumentation expectation from product constants (2026-08-23, [I-318])

- **`VersionInfoPanelTest` now builds its expected text from `CompatibilityConstants.renderEngineId` and `renderEngineVersion`.** The stale `default 26` literal is gone, narrowing the test from independently freezing today's number to verifying that VersionInfoPanel is wired to the product compatibility values.
- **Product behavior is unchanged.** No production Kotlin, visible text, Gradle dependency, runner, Espresso stub, persistence format, Room, pipeline, rendering, server, Web, or shared path changed. I-064 remains unchanged and open.
- **Verification:** the first instrumentation attempt stopped before the Compose hierarchy because the screen was off behind the keyguard. Private task I-362 added a read-only preflight; after unlock, valid fail-first failed only on the old literal. The same one test passed after implementation and at the branch tip. Every run used the APK-retention flag and a database evacuation, preserving two history rows, two lineage rows, schema version 9, and 1,239 thumbnails. Full instrumentation, the full JVM suite, screenshots, reference generation, and pentala were not run.

### Android `2.1.4-android.61` — toggling a work Star in place from the history grid (2026-08-23, [I-363])

- **Every work card in the history screen now keeps a visible `★` or `☆` control at its upper right.** It reflects the saved Star state, and tapping it calls only the existing `toggleStar(HistoryListItem)` path without selecting the card.
- **Existing card taps and long-press Star toggling remain available.** Selection rings, thumbnails, work titles, search, Star-only filtering, ordering, and counts are unchanged. The implementation uses only the existing `HistoryListItem.starred`, toggle action, and `HistoryBadge`; it adds no producer, ViewModel action, repository or DAO query, Room schema or migration, i18n, pipeline, rendering, server, Web, or shared change.
- **Verification:** before production editing, the focused JVM test exited 1 only on two unresolved references to the planned helper. `HistoryGridStarControlTest` exited 0 during implementation and again at the branch tip. Main had not moved after the branch point and the merge had no conflicts, so the same test was not repeated after merge under the Fast rules. Full JVM, instrumentation, devices, screenshots, reference generation, and pentala were out of scope.

### Android `2.1.4-android.62` — editing saved DDL from a lineage card (2026-08-24, [I-364])

- **Lineage cards with normal works now expose the existing `DDL edit` action.** It sits after Elements and before Model, and a non-focused card opens the existing editor with that work's saved `normalizedDdl`. Tombstones have no action.
- **The edited drawing reuses the existing `drawFromDdl` and `ddl_edit` save path.** After success, Android keeps the Lineage tab, selects the saved child, and reloads the existing graph focused on it. Existing card taps, Elements, Model and Language actions, New origin, and the ordinary Compose-origin transition remain unchanged. No producer, query, schema, or engine path was added.
- **Verification:** before production editing, the focused test expected three card actions and found only one existing `ddl_edit` derivation label, so only the new test failed. After replacing an off-screen test click with the visible non-focused card, the focused device test passed 1/1 during implementation and 1/1 at the branch tip. Every run used the display/keyguard preflight and a data evacuation; the test cleans up only the thumbnail for its generated render hash. The final evacuation retained two history rows, two lineage rows, schema version 9, and 1,239 thumbnails. Full instrumentation, the full JVM suite, screenshots, reference generation, real models, network, and pentala were out of scope.

### Android `2.1.4-android.63` — toggling a work Star in place from a lineage card (2026-08-24, [I-365])

- **Every lineage card with a normal work now keeps a visible `★` or `☆` control at the upper right of its thumbnail.** Starring a non-focused card is separate from card selection and preserves both the selected work and lineage focus. Tombstones have no control.
- **The graph reloads under the same focus after the existing Star persistence updates the work.** The implementation uses only the existing `HistoryItemEntity.starred`, `toggleStar(HistoryItemEntity)`, and `HistoryBadge`, preserving card taps, thumbnails, generation and state labels, and Elements, DDL, Model, and Language actions. It adds no producer, query, schema, or engine path.
- **Verification:** before production editing, the focused device test expected three Star controls and found zero, producing a 1/1 failure. The first implementation run passed persistence, graph refresh, and focus preservation; only the screen-text assertion needed a test-side wait for recomposition. The implementation and branch-tip final runs then passed 1/1. Each pre-run evacuation retained two history rows, two lineage rows, schema version 9, and 1,239 thumbnails. Full instrumentation, the full JVM suite, screenshots, reference generation, and pentala were out of scope.

### v2.13.47 — separate Web and rendering ownership, and move Engine 41 to the shared Rust core (Build 976, 2026-08-24)

- **Saijiki color previews now cover all nine colors.** Yellow, orange, and purple can be previewed from the vocabulary in the same way as the existing six colors.
- **The Web application's large screen modules are divided into owners by reason for change.** `+page.svelte` is now the shell for route lifecycle, view composition, and owner wiring. Session, single-work, Batch, Demo, history/lineage, Canvas viewport, refinement, and Settings each have a route-instance state owner; one Paint or refinement action is a stateless operation; and Canvas and Settings presentation belongs to focused views. A late asynchronous result from an earlier work is not applied to the current screen unless its run or target identity still matches.
- **Render Engine 41 moves Engine 40's performance into a platform-independent shared Rust core.** Rust owns planning, geometry, marks, surfaces, layers, SVG, and render metadata; Python passes one validated Score and resolved options through one thin adapter call. The former Python rendering implementation and its runtime fallback are retired. This is not an intentional drawing-quality release: it establishes a portability boundary for sharing the same core with clients such as Android.
- **Android now calls Engine 41 through a thin JNI binding and retires its Kotlin renderer and AndroidSVG fallback.** Main preview, history thumbnails, refinement preview, and PNG export use premultiplied RGBA8 derived from canonical SVG by the separate `inku-svg-raster` API. Saved SVG, `rh3`, the engine version, and the Android app version/build are unchanged. The packaged arm64 JNI library passed five canonical SVG and four raw-pixel comparisons on a Pixel 9, together with the Rust workspace and full Android JVM suite. A path-scoped CI gate now covers the shared native boundary.
- **Dependencies, gates, and the public operating contract now follow the new boundary.** Compatible Web build dependencies were refreshed, including the `nanoid` security patch. Reference-corpus CI explicitly supplies the local Rust binding when regenerating the current Engine 41 corpus. Public setup documentation now describes local Ollama as a separately installed and operated provider whose models, connection, and stage assignments must be configured, retracting the earlier claim that the whole product could run without API keys or authentication settings.

### Android `2.1.4-android.64` — keeping inku visible in the Pixel 9 launcher with the Web icon (Build 148107, 2026-08-25, [I-373])

- **inku was absent from the Pixel 9 app list because `app.inku.mobile` was not installed.** The existing manifest already gives `.MainActivity` the `MAIN` and `LAUNCHER` entry, so no activity alias, boot receiver, or persistent service was added. A normal debug install now leaves the package installed for user 0.
- **The Android launcher icon now uses the same pixel-grid mark as the Web favicon.** The former brush-and-eye design is retired. Adaptive foreground, the Web light background `#f5f3ef`, and all five legacy and round densities are built from the same black, gray, red, green, and blue grid. The Web favicon itself is unchanged.
- **Verification:** the worker characterized the old Android/Web digest difference, checked every asset's dimensions, alpha, background, palette and layout, and passed `:app:processDebugResources`. `installDebug` succeeded from main; the Pixel 9 reports version `2.1.4-android.64`, Build 148107, installed=true, hidden=false, and suspended=false. Launcher resolution returns `app.inku.mobile/.MainActivity`, cold start succeeds, and an app-list screenshot shows the `inku` label with the new icon. The full JVM suite, instrumentation, reference generation, and pentala were not run.

### Android `2.1.4-android.65` — intentionally rebuilding legacy databases with Room v10 and closing the portable gaps (Build 148108, 2026-08-25, [I-376])

- **Android advances to Room schema v10 and intentionally discards only existing v1–9 databases once before creating a fresh v10 database.** Reset removes the database, journal/WAL/SHM, and regenerable thumbnails while retaining model files. Missing and empty databases become fresh v10; existing v10 is retained. Version 11 or later, non-empty v0, and unreadable databases are refused without changing their bytes, showing only a safety explanation and Retry.
- **All three Android portable-persistence gaps are now database-enforced.** Unique indexes enforce the history/lineage-node one-to-one mappings, separate INSERT and UPDATE triggers reject self-edges, and the existing one-parent-per-child, non-unique render hash, and atomic history+node+edge transaction remain intact. Migration objects, migration-only tests, and schema snapshots 1–9 are removed; the current mapping uses Room-generated `10.json` with zero declared gaps.
- **Verification:** fail-first was red because the current schema was 9 instead of the expected 10. At the branch tip, the Pixel 9 safety runner passed the reset/refusal/retention/WAL/constraint matrix 8/8, refusal screen 1/1, and duplicate-render-hash, primary-key-collision, and lineage-atomicity gates 1/1 each. The full debug+release Android JVM suite passed 518 tests with zero failures, errors, or skips. The portable verifier reported gap 0, its focused server test passed 6 tests, and independent review found no issue. When the author later requested the device transition, its database was already schema 10 with zero history rows; after a verified backup, the exact database/sidecar/thumbnail targets were discarded, `.65` Build 148108 was installed while preserving app data, and a cold start created a fresh v10 database. The post-reset backup matched the device and reported schema 10, zero history rows, two model-asset rows, zero thumbnails, and zero real model files both before and after. Render parity, benchmark, container, and pentala were not run.

### Android `2.1.4-android.66` — history, blank batch rows, and automatic color catalogs in the writing UI (Build 148109, 2026-08-25, [I-382])

- **The Star display and action are removed from the ordinary Compose history strip.** Work selection, its ring and scrolling, the compact Stage 1 name, and saved-metadata tooltip remain; the history grid, lineage cards, Star persistence, and Star filter are unchanged. The line-oriented batch editor shows a delete control only for a blank row and removes that row together with its separator newline.
- **`Choose from description` now precedes the fixed color catalogs.** Auto adds one description call to the selected Stage 1 model per ordinary draw, batch line, or demo cycle, using the server's thirteen-catalog card, temperature 0.3, 200-token ceiling, JSON-first extraction, and known-ID fallback. Fixed selection, direct DDL drawing, saved-work replay, and refinement add zero calls, and history stores only the resolved real ID. NIM API use is free; other external providers may charge.
- **Verification:** before production editing, focused compilation was red only on the planned helpers and selector; the cancellation boundary later produced exactly one red test while cancellation was still being converted to default. At the branch tip, four focused JVM classes passed 21 tests with zero failures, errors, or skips; production Kotlin compilation and diff checks were green. A Sol/High review finding for unknown legacy-ID display was fixed. At the author's later request, the Pixel 9 schema-10 database was backed up and verified before `.66` Build 148109 was installed with data preservation via `adb install -r` and cold-started. Before and after deployment it held 6 history rows, 6 lineage nodes, 7 provider settings, 3 app settings, 2 model assets, and 6 thumbnails; the database/WAL/SHM/thumbnail backup bytes matched the device. Room/schema/migrations, pipeline/rendering/Rust, existing palette codes/maps, instrumentation, and pentala were unchanged or not run.

### Android `2.1.4-android.67` — bottom camera entry and canvas overflow menu (Build 148110, 2026-08-25, [I-388])

- **The bottom navigation is now Write, Camera, History, and Lineage.** Write restores Write mode and is not selected while Batch is visible. Camera only launches Android's standard still-camera intent; it adds no capture result, permission, URI, persistence, or database write.
- **The canvas control strip now centers Full screen and puts a three-line menu at the right.** Batch opens the existing Batch panel and Settings opens the existing Settings screen. The Write/Batch segmented controls are removed from the drawing-settings panel, leaving the menu as Batch's single visible entry.
- **Verification:** focused fail-first checks caught the four former navigation/layout behaviors and one Batch selection boundary. At the branch tip, five `AppMenuNavigationTest` cases and six `WordingLintTest` cases passed, production Kotlin compilation succeeded, and the diff check was clean. A debug APK with a matching signature and increasing version code was installed on the Pixel 9 with data preservation using only `adb install -r`, and `.67` Build 148110 cold-started. Verified pre- and post-install backups both held schema 10, 17 history rows, 17 lineage nodes, 7 provider settings, 4 app settings, 2 model assets, 3 export templates, and 17 thumbnails; database, WAL, and SHM bytes matched the device at each point. Room schema and migrations, pipeline and rendering, Rust, Server/Web/shared, instrumentation, and pentala were unchanged or not run.

### Android `2.1.4-android.68` — convert a camera image into an editable description with local Gemma (Build 148111, 2026-08-26, [I-395])

- **Camera now captures a full-size image with `TakePicture`, then converts it to a description using only the local E2B after confirmation.** Existing description text requires overwrite confirmation, and the result is placed in the editable description field. It does not automatically run Stage 1/2, generate SVG, save, create lineage, or call an external provider.
- **Image preparation and local inference now have a typed boundary.** EXIF orientation is applied, the long edge is reduced to at most 1280 px, and JPEG quality 85 is used. Text and vision share the same LiteRT-LM provider and mutex; vision is fixed to E2B with a GPU backend and one image per request. The cached capture file is reclaimed after success, failure, or cancellation.
- **Verification:** before production editing, the focused JVM checks caught seven unimplemented boundaries. At the branch tip, six focused classes with 17 tests, production Kotlin compilation, the debug APK build, documentation checks, and diff checks were green. The matching-signature `.68` Build 148111 was installed on the Pixel 9 with data preservation via `adb install -r`, and the author confirmed the real-device flow from capture through an editable description. Local E2B vision succeeded twice (720×1280, about 101 KB / 97 KB, 12.5 s / 12.8 s), with zero camera temporary files afterward. Verified pre- and post-install backups both held schema 10, 17 history rows, 17 lineage nodes, 7 provider settings, 4 app settings, 2 model assets, 3 export templates, and 17 thumbnails; database, WAL, and SHM bytes matched the device at each point. The full JVM suite, instrumentation, Server/Web/Rust, and pentala were unchanged or not run.

### Android `2.1.4-android.69` — explicitly draw a camera description with fixed NIM and Vivid Material (Build 148112, 2026-08-26, [I-398])

- **After editing local Gemma's camera description, only an explicit Draw action turns it into a work through a fixed run snapshot.** Stage 1 and 2 use `nvidia:google/gemma-4-31b-it`, the catalog is `vivid_material`, Sketch from Life is off, catalog auto-selection makes zero calls, and `autoRepair=false`. NVIDIA provider validation runs before the request, with no local, alternate-model, alternate-provider, or deterministic fallback when the route is unavailable or the response is invalid.
- **The camera route inherits neither normal settings nor lineage.** Ordinary edits and failed or stopped draws retain camera origin; Clear, history selection, a new capture, and success close it. Success saves exactly one root work through the existing transaction without changing the normal model, catalog, or sketch controls or their persisted values.
- **Verification:** the focused fail-first was red on the missing route helper before production editing. At the branch tip, four focused classes passed 15 tests, production Kotlin compilation and the debug APK build succeeded, and the diff check was clean. After signature equality and an increasing version code were verified, `.69` Build 148112 was installed on the Pixel 9 with data preservation using only `adb install -r`; the author confirmed camera → local description → explicit draw → completed SVG. The metadata trace contained one local E2B Vision call (720×1280, 122361 bytes, 12.871 s), no repeated local analysis before one fixed-NIM Stage 1 call (10.925 s), one Stage 2 call (28.492 s), and one `vivid_material` render (9 ms). Verified backups retained schema 10 and moved history 17→18, lineage nodes 17→18, edges 0→0, and thumbnails 17→18, while retaining 7 provider settings, 4 app settings, 2 model assets, and 3 export templates. The new row recorded both fixed model IDs, `vivid_material`, sketch off, a lineage node, and a thumbnail. App settings were byte-equivalent; the existing startup seed changed only `updated_at` on all seven provider rows, while names, kinds, URLs, encrypted keys, model lists, enabled flags, and default-local flags all matched. The full JVM suite, instrumentation, Server/Web/Rust, pentala, and public GitHub push were not run.

### Android `2.1.4-android.72` — complete camera-input provenance and its acceptance boundaries (Build 148115, 2026-08-26, [I-401])

- **A camera-input audit snapshot is stored in existing work metadata and shown only in Generation Info.** `render_metadata_json.input_provenance` records origin, the fixed-NIM route, local-Vision provider and actual model, prompt version, output mode, and normalized image dimensions. It stores no photo, URI, EXIF, location, digest, or local-Vision output text, and ordinary works gain no key. Room entities, DAOs, schema and migrations, exports, render hashes, and existing rows are unchanged.
- **Two generation boundaries exposed during acceptance are closed.** The local path now collects response-message `Content.Text` instead of rendering the LiteRT-LM conversation template for every streamed chunk, removes remaining markers, and joins Japanese clause-level line breaks. The NIM Stage 2 tool schema adds `minItems=1` to `instructions`, so an empty array is not accepted as drawable success.
- **Verification:** focused JVM coverage for provenance persistence and presentation, the camera route, local-output normalization, the Vision request, and the Stage 2 schema was green, including the final eight tests; production Kotlin compilation and the debug APK build succeeded. Deployment reused the verified backup for that JST date and installed `.72` Build 148115 on the Pixel 9 without clearing application data. The final trace contained one local-Vision call (720×1280, 130150 bytes, 17.288 s), one fixed-NIM Stage 1 call (10.735 s), one Stage 2 call (21.041 s), and one `vivid_material` render (18 ms); the author confirmed the completed SVG. Generation Info showed the camera, local-Vision, NIM, image-dimension, and catalog audit values. As contracted, no full database inspection, raw SQLite query, real-device instrumentation, Server/Web/Rust change, pentala deployment, or public GitHub push was performed.

### Android `2.1.4-android.73` — one continuous development experience from shutter to saved artwork (Build 148116, 2026-08-26, [I-407])

- **The shutter now owns one run through image preparation, local E2B Vision, fixed-NIM Stage 1 and 2, `vivid_material` rendering, and one root-work save through the existing transaction.** The local model and enabled NIM provider are checked before a capture file or camera intent is created. The image never goes to the external provider, and the photo, URI, and local-analysis text are not persisted. Room entities, DAOs, schema and migrations, and the ordinary description, DDL, and history routes are unchanged.
- **The wait is presented as full-paper development.** Light, forms, color and placement, development, and completion use exposure, grain and forms, vivid color fields, outline settling, and a final-artwork crossfade derived from `vivid_material`. There is no percentage, ETA, fake SVG, or artificial delay, and repeating motion stops when the device animation scale is zero. Back and Cancel `cancelAndJoin` the same run, reclaim the temporary image, and restore the pre-run UI. Only a NIM failure can retry from Stage 1 while reusing local analysis, and completion appears only after saving.
- **Verification:** before production editing, focused JVM checks were red at the unimplemented coordinator, state, presentation, and preflight boundaries. At the branch tip and again on accepted main, seven classes passed 27 tests with zero failures, errors, or skips; production Kotlin compilation, the debug APK, documentation, and diff checks were green. Deployment reused the verified backup for the current date, confirmed one retained generation, and data-preserving installed `.73` Build 148116 on the Pixel 9. The launcher resolved to `app.inku.mobile/.MainActivity`, and a cold start succeeded in 550 ms. The author's real-device checks for shutter-to-save and mid-run cancellation are the only open acceptance items; full database inspection, raw SQLite queries, real-device instrumentation, Server/Web/Rust, pentala, and public GitHub push were not run.

### Android `2.1.4-android.74` — deliver input to the development overlay's Cancel button (Build 148117, 2026-08-26, [I-407])

- **The author's device check passed shutter-to-save but found that Cancel could not be pressed during development.** The full-screen overlay's parent pointer handler consumed every event in `PointerEventPass.Initial`, before its child button could receive it. Consumption now happens in `Final`, after child controls such as Cancel and Retry have handled input, while the background remains blocked. The cancel job, run-ID fence, before-save gate, and UI-restore semantics are unchanged.
- **Verification:** the new wiring test was deliberately red against the old `Initial` implementation. After the fix, the seven I-407 focused classes passed 28 tests; production Kotlin compilation, the debug APK, and the diff check were green. Deployment reused the verified backup for the current date, confirmed one retained generation, and data-preserving installed `.74` Build 148117 on the Pixel 9. Version and Build checks and a 565 ms cold start succeeded. The author confirmed that mid-run Cancel is pressable, restores the pre-run screen, and shows no new history item, completing I-407 acceptance. Full database inspection, raw SQLite queries, real-device instrumentation, Server/Web/Rust, pentala, and public GitHub push were not run.

### Android `2.1.4-android.75` — develop an advanced on-device DDL directly from a camera image (Build 148118, 2026-08-26, [I-412])

- **Misc settings now offer Description (recommended) and Direct DDL (advanced), while Description remains the default route.** Only a capture explicitly using Direct DDL validates `camera-ddl-v1` output from local Gemma 4 E2B, makes zero NIM Stage 1 calls, and enters fixed-NIM Stage 2, `vivid_material` rendering, and the existing root-save transaction. The mode uses existing `app_settings`; missing, blank, unknown, and malformed JSON fail safe to Description. There is no Room schema or migration, new column, photo persistence, or ordinary-setting mutation.
- **The JA and EN DDL prompts derive from the existing `WebDdlSpec` and `ServerDdlText` authority, never follow image text as an instruction, and forbid identifying people or inferring personal attributes.** One typed validator applies existing template-marker, newline, and placement normalization and rejects blank, fenced, JSON/SQL, preamble-only, and zero-drawable-vocabulary output with zero remote or save calls. Successful provenance records `local_ddl_to_nim_stage2`, `ddl`, and `camera-ddl-v1`; `stage1_model` names the actual local-E2B producer and `stage2_model` names fixed NIM. A Stage 2 retry repeats neither local Vision nor Stage 1.
- **Verification:** the three new focused classes were deliberately red on missing implementation symbols before production edits. At the branch tip, the 12 classes that directly read I-412 were green, as were production Kotlin compilation, the debug APK, bilingual documentation, and the diff check. Deployment reused the verified backup for the current date, confirmed one retained generation, and data-preserving installed `.75` Build 148118 on the Pixel 9. The on-device version and Build checks and a 527 ms cold start succeeded. Only the author's real-device check of the Direct DDL route remains for acceptance; the full JVM suite, release build, full database inspection, raw SQLite queries, real-device instrumentation, Server/Web/Rust, pentala, and public GitHub push were not run.

### Android `2.1.4-android.76` — keep image-bearing local DDL prompts within LiteRT capacity (Build 148119, 2026-08-26, [I-412])

- **The first author device check reached image preparation and GPU-engine initialization, then raised `LiteRtLmJniException` before `camera-ddl-v1` generation began.** Direct DDL alone sent the ordinary-provider full display specification, 7,912 characters, alongside the image. The fix adds no camera vocabulary: it references `WebDdlSpec`'s existing compact Stage 1 authority for LiteRT and holds each JA and EN prompt including the camera boundary to at most 4,096 characters. Performance logging records only the character count, never the content.
- **Verification:** the prompt-size test was deliberately red at the old 7,912 characters. After the fix, all 12 I-412 focused classes, the debug APK including production Kotlin compilation, bilingual docs gate, and diff check were green. Deployment reused the verified backup for the current date, retained one generation, and data-preserving installed `.76` Build 148119 on the Pixel 9. The on-device version and Build checks and a 529 ms cold start succeeded. The author confirmed completion through Direct DDL; the final trace had one successful local-Vision call with a 1,376-character `camera-ddl-v1` prompt (720×1280, 94,800 bytes, 54.719 s), zero NIM Stage 1 calls, one fixed-NIM Stage 2 call (35.063 s), and one `vivid_material` render (12 ms). This completed I-412 and M5. Full database inspection, raw SQLite queries, real-device instrumentation, pentala, and public GitHub push were not run.

### Android `2.1.4-android.77` — choose an existing device photo as local-Vision input (Build 148120, 2026-08-27, [I-456])

- **The camera entry now offers Take a photo, Choose a photo, and Cancel, and routes one image-only Photo Picker result into the existing one-touch path.** The selected content URI is copied temporarily into app-owned cache and deleted under an ownership guard on success, cancellation, failure, a stale run, or `onCleared`. The source photo is neither modified nor deleted, and no storage permission or custom file browser was added. Image bytes, URI, path, EXIF, and filename never enter NIM; only local-Vision text follows the existing remote route.
- **Saved provenance extends the existing JSON compatibly with `origin=photo_picker`, and Generation Info distinguishes a camera capture from an existing photo in JA and EN.** There is no Room schema, migration, backfill, or new column, and no server/web/CLI/shared, render/DDL/catalog, or ordinary-setting change.
- **Verification and deployment:** the pre-edit fail-first, 45 focused JVM tests across 12 classes, production Kotlin compilation, debug APK, six Android naming tests, bilingual docs gate, and diff check were green. The author-accepted `b7521dfe` head was merged to main and built as `.77` Build 148120. The safe helper created the verified 2026-08-27 backup, retained two generations, and data-preserving installed it on the Pixel 9. On-device version/Build checks and a 530 ms cold start succeeded. Real-device Photo Picker interaction, the full JVM suite, release build, full database inspection, raw SQLite queries, real-device instrumentation, pentala, and public GitHub push were not run.

### v2.14.0 — complete SQLite persistence and add creation from local images (Build 1060, 2026-08-27)

- **Server persistence is now fully SQLite-only.** Connection, transaction, migration, and repository responsibilities are split into small owners, and active configuration, instructions, and support claims no longer expose PostgreSQL paths.
- **Existing Server SQLite data is preserved through versioned migrations.** Schema fingerprints, pre-migration snapshots, a single-writer lock, and a data-loss gate are enforced; unknown or inconsistent schemas fail closed instead of being repaired destructively.
- **A portable persistence contract now covers the logical Server and Android schemas.** Shared mappings validate fields, NULL behavior, identity, transactions, and constraints, state host-adapter differences explicitly, and run in CI.
- **Android has a one-time boundary into Room schema 10.** Only v1–9 databases are intentionally reset while model files are retained. There is no generic destructive fallback for v10 or later, or for corrupt databases.
...
### Android `2.1.4-android.78` — harden model, image, and provider boundaries (Build 148121, 2026-08-31)
