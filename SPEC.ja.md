...
**UIラベル方針**: SPEC と内部設計では、記述 → 楽譜 → 演奏という音楽メタファーを維持する。一方、主要な操作ボタンでは、初めて触るユーザーが押した結果を予測しやすいように、メタファー語を直感的な操作語へ置き換える。UI上では `演奏` を `タッチ`、`構図` を `配置`、`解釈` を `読み取り` として扱う。主要ボタンは `描画する` ではなく `生成する`、`DDLから描画` ではなく `コードから描画`、`候補4枚` ではなく `4案を作る`、`解釈も含める` ではなく `読み取りも変える`、`選を残す` ではなく `選んだ候補を保存` とする。tooltip では、直感語を使いながら、入力文を読み取り、配置とタッチを決めて生成すること、同じ読み取りから配置やタッチだけを変えること、必要に応じて読み取りからやり直すことを説明する。現在画像から候補を作って選ぶ操作は、記述タブから分離し、Canvas 側の `推敲` タブに集約する。モデル比較は Canvas 側の `比較` タブに集約する。`比較` タブでは前後の履歴比較は行わず、対象作品だけを基準として表示する。モデル比較では、対象作品の右側に、異なる Stage 1 推論モデルで描画した結果をモデル名付きで並べる。対象作品で使用中のモデルは、選択肢内で分かりやすく色を変えて示し、選択不可にする。比較対象モデルはユーザーが明示的に選ぶものとし、初期状態では何も選択しない。選択状態はユーザーごとに DB のモデル設定へ保存し、再訪時にも同じ比較対象を復元する。比較結果カードには、結果を履歴として残す `採用` と、スター付きで残す、または保存済み結果のスターを切り替えるスター UI を置く。比較実行時にモデルがエラーを返した場合、そのモデルを NG 色で表示し、比較対象のチェックを外してユーザーごとの保存済み選択からも外す。未選択のモデルを自動で追加実行することはなく、残っている選択済みモデルだけで比較処理を継続する。初期上限は4モデルとする。tooltip には、異なる推論モデルでの描画比較であることを明記する。これにより、SPEC の詩的なメタファーと、Web UI の操作予測性を分離して両立させる。
