...
このディレクトリは、inkuを初めて使う人が作品を作成できることと、システム管理者がアプリケーションを展開・運用できることを目的にしたマニュアルです。Web画面の操作説明はv2.15.15（Web Build 1091）に合わせています。製品仕様の正本はリポジトリ直下の`SPEC.ja.md`です。
...
日々の制作では「画像の作成方法」を参照してください。記述・指示書からの描画、系譜とライブラリ、バッチの再開、レイヤーアニメーション、設定の文字サイズまで説明しています。モデル接続や利用者の権限を管理する場合は「サーバー設定方法」を参照してください。
