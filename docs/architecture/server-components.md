...
Total: 105 (static count). The public allowlist contains three paths: `/health`, `/api/info`, and `/api/auth/login` (`test_route_authorization.py`). The standard is to leave out anything login does not need.

**⚠ The counts in this table come from counting route declarations, not from starting the live app.** `EXPECTED_ROUTE_COUNT` in `test_route_authorization.py`, named as the count's source of truth, is 95, and the API surface baseline still contains the retired `/api/prompts` and none of `/api/pipeline/*` (`known-differences.md` F-06).

## Main flows

- `/api/paint`, `/api/paint/stream`, `/api/interpret`, and `/api/compose` go through `pipeline_compat.py` to the same `PipelineService`. The stream returns the final result as one `done` line and emits no per-layer events ahead of it. When a completion proposal needs approval, all four answer 409 with the current view.
