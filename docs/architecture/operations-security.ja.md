...
- 105 endpoint（静的な数え上げ。`known-differences.ja.md` F-06）のうちguardなしは理由付きallowlist 3 pathだけで、live routeをtestが列挙する。共有pipelineの`/api/pipeline/*`は各routeで`_current_user`を要求し、認証済みの利用者をexecution・variation・historyの所有者として全操作へ渡す。他人のexecutionやhistory linkは所有者の不一致で見えない。
...
| 容量 | `INKU_MAX_CONCURRENT_REQUESTS`, `INKU_RENDER_CONCURRENCY`, `INKU_THUMBNAIL_WORKERS`, `INKU_THUMBNAIL_QUEUE_LIMIT`, `INKU_STAGE_WORKERS`, `INKU_STAGE_QUEUE_LIMIT`（後の2つは状態表示だけに残る。`known-differences.ja.md` F-10） |
