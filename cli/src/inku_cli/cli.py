...
def _paint_payload(args: argparse.Namespace, text: str) -> dict[str, Any]:
...
def command_login(args: argparse.Namespace) -> int:
    ...
    save_config(CliConfig(base_url=base_url, token=token, username=username))
...
def command_logout(args: argparse.Namespace) -> int:
    config = load_config()
    if config.token:
        client = ApiClient(args.base_url or config.base_url, config.token)
        try:
...
def command_paint(args: argparse.Namespace) -> int:
    ...
    result, _ = client.request("POST", "/api/paint", data=_paint_payload(args, text))
    ...
    if args.full_json:
        _print_json({**result, "paths": paths})
    else:
        _print_json(summary)
    return 0
...
def command_batch(args: argparse.Namespace) -> int:
    ...
    for index, line in enumerate(lines, start=1):
        try:
            result, _ = client.request("POST", "/api/paint", data=_paint_payload(args, line))
