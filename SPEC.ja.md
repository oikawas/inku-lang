...
- 記述タブで生成後に表示される解釈 box をクリックすると、大きな DDL 編集ダイアログを開く
- ダイアログ左側に行番号付きの DDL 編集エリア、右側に縦2列の歳時記語彙エリアを表示する
- ダイアログ内の `DDLから描画` ボタンは Stage 1 をスキップし、現在の DDL から `/api/compose` を実行する
- 解釈 box ヘッダーには従来どおり `歳時記` ボタンを表示し、右スライド式の歳時記ドロワーを開ける
