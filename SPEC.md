...
- 出力ファイル保存 (`cairosvg` / filesystem) の失敗可視化。v1.9 時点ではバックグラウンド保存失敗が握りつぶされる
- バッチ描画の行単位失敗可視化。v1.9 時点では個別失敗を継続処理するが、失敗件数・対象行を UI に出していない
...
- **右パネル**: タブバー（演奏/プロンプト/楽譜）→ キャンバスエリア → エクスポートバー
