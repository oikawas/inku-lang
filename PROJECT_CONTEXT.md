...
The shared object placement plan API takes verified Stage 1.5 and delivers ordinary DDL and declared flat Macro line-up / scatter / tile as one resolved plan per instruction / Emit. Omitted count is eight (one for place); size uses the canvas short edge independently of count, sharing existing normal sizes and factors. Recipes retain a horizontal row, tile rows and columns from physical aspect, or scatter centroid translation requiring a later performance seed. No instance array, RNG, or Score conversion runs. The all-shape principle currently delivers line, circle, ellipse, square, arc, cloudform, and point; Triangle / Polygon remain explicit geometry gaps with their owners. Exact geometry, appearance, angle, position, origin, and Stop / Continue are preserved. Ready plan and existing Score success are distinct. Step10X for explicit layout direction, remaining geometry, whole Step10, and runtime / UI / persistence cutover remain incomplete. See the end of SPEC §12.
