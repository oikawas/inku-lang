...
**写生文は記述の代わりに 5 つの消費者すべてへ届く**（Stage 1・プラグイン展開・Stage 1.5・Stage 2・coerce）。
...
変奏（強度 3 段）と添景の水準（`none` / `sparse` / `auto`）を持ち、いずれも作品ごとに保存される。
...
キャンバス比率、添景水準、表示モードの選択。カタログの選択はユーザーごとにサーバーへ保存する
