...
Shared Rust renders normal, dense, and faint fills for all twelve tools. Direct DDL and Macro deliver Score 0.3.0 `surface_intensity` through one lowerer while preserving reads and writes of saved 0.1.0 / 0.2.0 scores. Placement-free Macro Groups retain generated order, references, and ownership through the same delivery path, without reconnecting relations across unsupported transforms. Full typed runtime/UI/save integration remains incomplete.
