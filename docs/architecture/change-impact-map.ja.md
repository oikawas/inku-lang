...
| render core/stroke | 同一Score+seed+解決済みoption、engine前進、粗いnative 1-call境界 | `core/crates/inku-render`; Python/Android binding; `default/adapter.py`; `AndroidRenderHost` | Rust workspace test、adapter/JNI契約、reference API、bounded Android same-version byte sample | 現行Engine 41の610件、Engine 40は履歴保持 | version裁定、出力変更時2回再生成、pinned bindingとLinux/端末gate |
| SVG raster presentation | canonical SVG不変、明示pixel format/stride、allocation bound | `core/crates/inku-svg-raster`; `inku-render-android`; `RustArtworkRasterizer` | Rust raster unit、host/device raw digest、known color/alpha/stride | 現行3＋履歴1のbounded sample、full raster corpusは作らない | Android arm64 packaging、off-main、cache key、no external resource |
...
| Android host/pipeline | Kotlin Stage 1/1.5/2・coerce、共有Rust render/raster、Room schema | Kotlin pipeline/data、`AndroidRenderHost`、`RustArtworkRasterizer` | focused JVM、native CI、必要時instrumentation | DDL/Score/coerce fixturesのみ。描画はcanonical Server corpusからbounded staging | device data backup規則、実機データを消さない |
...
現行CIは通常push/PRで、server、CLI、Web、公開文書、凍結render/DDL corpus、Android design previewを実行する。`android-native.yml`は`core/**`またはAndroid native/host/presentation関連pathが変わるときだけ、Rust workspace、raster unit、Android arm64 `.so` packaging、focused host JVM testを実行する。端末instrumentationはlocal受入である。

## 決定的層の特別規則

`coerce/`、`ddl_expander.py`、`core/crates/inku-render/`、`core/crates/inku-svg-raster/`、native request境界、`render_engines/default/`、`renderer.py`、`schema.py`、`saijiki.py`、`language_support/` は決定的層である。Render Engine出力へ影響しうる変更では現行凍結corpusを再生成する。pytestのreference testは凍結fileとmanifestを読むだけの部分があり、generator再実行の代用にならない。native request/outputを変えないと証明したhost-only変更は、比例した直接検査とbounded same-version byte sampleを使う。
