...
現行consumerの`transform`は回転だけを受け入れる。複数の子図形をまとめて回転するときは、回転前の子図形全体の正確な外接矩形中心を使い、子同士の位置関係と各図形の向きを一緒に回す。キャンバス中心や各子の個別中心を、まとまりの回転中心の代わりにしない。内部の関係を解決したうえで内側から外側へ回転し、各段の子全体の外接矩形中心を使う。配置が演奏時に決まる場合は、その配置が確定してから中心を求める。`group`は範囲・参照・ownerを運ぶ透明な構造であり、それ自体の配置・描画命令にはならない。`translate`と`scale`を含むtransformは未対応のstructural subtreeとして残る。

回転だけのtransformはCount1のactual Scoreと反復symbolic planの両方へ配送する。DDL側の`TransformGroupPlan { start, end, rotation_degrees, fixed_position_indices, provenance }`は元の出力順の連続範囲を表し、内側から外側のpostorderで保持する。範囲はstart以上end未満で、互いに素か入れ子とし、同じ範囲も記載順に変換する。actual ScoreではScore 0.4.0の`transform_groups`が同じ範囲、角度、数値固定memberの元instruction indexを保持する。空のgroup listはwireへ出ず、保存済み0.1.0／0.2.0／0.3.0とversionなしartifactは従来互換を保つ。`surface_intensity`はScore 0.3.0以後で有効である。反復planでは`PlanRelation { kind, gap, target_object_index, position_authority, touching_constraints }`がchecked relation intentを保つ。回転groupの外へ向かうConnectedは先行を変えず、接続する子と先行を隔てる最外group全体を同じ差分だけ平行移動する。回転群を跨ぐ外部relationはConnectedに対応し、他の外部relationは現行performerでは診断対象とする。数値固定memberがあるgroupはnonzeroの追加平行移動を許さず、must-fitはその数値memberだけに課し、named memberの既存clippingを変えない。Stopは描画前に停止し、OmitAndContinueでのgroup失敗はgroup全体を省略し、owner・元index・seed・失われた参照を保つ。これはStep11の個体materializationやStep13のruntime / UI / 保存全面接続を完了とする記述ではない。
...
詞書の表示状態はユーザー設定としてDBへ保存する。推敲タブ内の調整／モデル比較で前後の作品へ移動するときはサブタブ文脈を維持し、対象作品だけを切り替える。
...
Planは一instruction / 一Emitにつき一件で、exact count、解決済み寸法・外観・angle・位置・layout式とsource / generated originを持つ。count比例の配列・個体geometry・Score命令複製は作らない。Stopはblockingがあればplanを返さず、Continueは既存のtyped owner / span / 理由 / 実処置を保ち、全省略をReadyにしない。未対応field・relation・coordinationを黙って捨てない。Readyは描画成功やScore成功ではなく、numeric must-fitと個体bounds検査を含むmaterializationは未実行である。既存Score wire / lowering outcome / compiler executionの成功意味と新fieldなしのcount1 / place / relation互換は変わらない。同じ`inku.geometry-resolution-policy.v1`がこの解決をattestする。残geometry、whole Step10、Step11の個体materialization、runtime / UI / 保存cutoverは未完了である。
