...
flowchart TD
    ...
    S1["Stage 1 effect\nwork-plan JSON"]
    PRINT["Normalize and print the work plan\n→ visible DDL candidate"]
...
## Stage 1 — work plan and printing

The Stage 1 model writes no visible DDL string; it returns a closed-typed **work-plan JSON** (prompt `inku.typed-stage1-work-plan-prompt.v1`). The prompt carries only the finite vocabulary derived from the saijiki, the resolved canvas and catalog identities, and the signatures and localized summaries of validated Macros. It never passes Macro bodies or expanded DDL.
...
- **Printing** — `print_work_plan` prints the plan deterministically as visible DDL in the requested language. Only that string reaches the compiler; the work plan is transient.
