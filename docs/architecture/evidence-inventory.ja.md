...
| SYS-LLM | LLM providers | 写生、色カタログ選択、Stage 1下絵、known-hole補完の外部推論 | `pipeline_provider.py` (`SingleAttemptProvider`, `resolved_stage_model`); `model_settings.py`; Android `RoutingModelProvider` | §12.5–12.8 | 確認済み | 抽象化すれば可 |
...
| API-ROUTERS | Router群 | 10分類のrouterと`/api/pipeline`の共有pipeline router。94 + 12 = 106 endpoint（件数の正本は`test_route_authorization.py`の`EXPECTED_ROUTE_COUNT`） | `api_core/routers/{public,auth,me,plugins,settings,users,history,lineage,render,feedback}.py`; `pipeline_api.py:pipeline_router` | Project Context「server」 | 確認済み | 公開可 |
...
| PIPE-S1 | Stage 1 下絵 | 記述から閉じた型の下絵JSONを受け、可視DDLへ決定的に印字する。compiler不受理時は有限予算内で再正規化し、尽きたら描画可能な残部だけを採用候補にする | `prompts.rs` (`build_stage1_prompt_with_sketch`, `parse_stage1_response`); `inku-ddl/src/work_plan.rs` (`normalize_work_plan`, `print_work_plan`); `machine.rs` (`correct_stage1`, `residual_execution_preflight`) | §12.6, §12.8 | 確認済み | 公開可 |
