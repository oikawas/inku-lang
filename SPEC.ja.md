...
通常生成のStage 1 / Stage 2はLLM処理として扱い、画像入力を読む処理はVisionとして別のユーザーモデル設定を持つ。モデル選択ダイアログは `Stage 1/2`・`Stage 1`・`Stage 2`・`Vision` を分け、管理者のモデル設定も各モデルのLLM/Vision用途を明示する。`GET /api/models` は旧CLI向けのLLM `catalog` を維持しつつ、`llm_catalog` と `vision_catalog` を返す。奥書などのVision処理はユーザーのVision設定を既定とし、APIまたはCLIの明示モデル指定は互換性のため優先する。
...
系譜カードの作品メニューは、AI自律推敲、描画要素で比較、モデルで比較、言語で比較、ゴミ箱への移動を提供する。三つの比較操作は選択カードを対象作品として既存の推敲サブビューを開き、独立した比較実装を持たない。旧手動推敲モーダルへの入口は提供しない。ゴミ箱操作は他の比較操作と視覚的に区別し、操作結果を明示する高コントラスト表示とする。
...
- 入力はderivation kindとmetadata、各作品の詞書、前後作品をサーバーでPNG化したサムネイル対、既存の特徴鏡が読む構図族・primitive・色・密度・角度・配置pathの差分である。新しい品質尺度は作らない。
