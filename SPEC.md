...
When `transform` rotates multiple child shapes together, it uses the center of their combined bounding box before rotation. Relative positions and each shape's orientation rotate together. Neither the canvas center nor separate child centers replace the group's pivot. If placement is determined during performance, that placement must be resolved before the pivot is computed. This rotation meaning is accepted; `transform` delivery to Score remains unimplemented in the current consumer described below.
...
Closed fills carry the selected tool's texture. Score 0.3.0 `surface_intensity` is `normal` (the default), `dense`, or `faint`; the normal value is omitted from serialized output. Saved Score 0.1.0 / 0.2.0 keep their original version when read and written. Direct DDL and Macro use the same lowerer, and repeated plans retain intensity in their appearance before materialization. Changing intensity does not change the selected color identity or the seed that determines shape and stroke geometry.
