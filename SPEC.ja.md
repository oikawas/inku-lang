...
| 名前 | 何の版か | 現在 | 上げる条件 |
|---|---|---|---|
| `render_engine_version` | 描画エンジン | `11` | **同一 Score + 同一 seed の演奏結果が変わるとき** |
...
| `APP_VERSION` / `server/pyproject.toml` | 製品リリース | v2.4.8 | リリースごと |
| `web/BUILD_NUMBER` | ビルド通し番号 | 698 | **UI の変更でも動く** |
...
|---|---|---|---|
| 描画 | `server/reference/render-engine-11/` | `renderer.py` / `stroke_engine.py` の演奏結果（SVG） | 220 |
