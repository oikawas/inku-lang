...
直近のv1.89 / Build 584では、奥書で選んだVisionモデルをユーザーごとに保存する。系譜の作品メニューから記述またはDDLをモーダル編集して描画でき、結果は選択作品の子として保存され、最新の子を中心にした系譜表示へ自動で戻る。Build 583のprefix-only逐次読解、成功済み所見キャッシュ、Vision画像縮小、再試行可能なタイムアウト表示も維持する。
