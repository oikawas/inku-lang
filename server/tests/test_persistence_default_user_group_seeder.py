"""Direct ownership coverage for the default organisation-group seed."""
...
from inku_server.persistence.schema import UserGroupRow


def _seeder_or_skip():
    seeder = getattr(groups, "DefaultUserGroupSeeder", None)
    if seeder is None:
        pytest.skip("default group seeder is intentionally absent during fail-first")
    return seeder


def test_groups_owns_default_seed_and_db_delegates() -> None:
    seeder = getattr(groups, "DefaultUserGroupSeeder", None)
    ...
    function = ast.parse(inspect.getsource(db._ensure_default_user_group)).body[0]
    assert isinstance(function.body[-1], ast.Return)


def test_default_seed_factory_receives_runtime_dependencies(monkeypatch) -> None:
    _seeder_or_skip()
    received = []
    ...
    monkeypatch.setattr(db._groups, "DefaultUserGroupSeeder", Recording)
...
class _Session:
    ...
    def query(self, row_type):
        assert row_type is UserGroupRow
        return self
...
def test_default_seed_preserves_existing_group_short_circuit() -> None:
    session = _Session(existing=object())
    _seeder_or_skip()(lambda: session, lambda: "id", lambda: 9).ensure(session)
    assert session.added == []
    assert (session.commits, session.flushes) == (0, 0)


def test_default_seed_preserves_exact_row_and_session_finish_behavior() -> None:
    owned = _Session()
    borrowed = _Session()
    ids = iter(["owned-id", "borrowed-id"])
