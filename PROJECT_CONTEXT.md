...
- The 106 endpoints live in the ten files under `server/src/inku_server/api_core/routers/` (`auth`,
`feedback`, `history`, `lineage`, `me`, `plugins`, `public`, `render`, `settings`, `users`) and in
the shared pipeline's `pipeline_api.py` (the twelve `/api/pipeline/*` routes).
