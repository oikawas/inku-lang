...
## 4. 実測（**着手後に追記した。上の予測は 1 文字も書き換えていない**）

**予測は 2 つとも当たった。**

| 面 | 予測 | 実測 |
|---|---|---|
| 着手前の server 満点 | 3227 passed / 31 skipped（レポートの値） | **3227 passed / 31 skipped**（13 分 33 秒・`5b65fe55` の木で自分で測った。**一致**） |
| 直した後の server 満点 | 3228 passed / 31 skipped | **3228 passed / 31 skipped**（11 分 19 秒・**一致**） |
| **P-6** | `T-78` が 1 本だけ赤・`T-75`〜`T-77` は緑 | **1 failed / 3 passed（一致）**。原本のバイトで戻し sha256 照合 ✓・`git checkout` は 0 回 |

**⚠ 測定の条件**: 同じ機械で別セッションの pytest が並走していたので、時間は木の性質ではない
（着手前 813 秒・着手後 679 秒。**本数は影響を受けない**）。

**⚠ 書き換えた既存テストが 1 本ある** —— `T-75`
（`assert "GZipMiddleware" in ...` → `assert "FlushingGZipMiddleware" in ...`）。
**変えていない主張は「app が gzip の層を持っていること」で、**名指す層の名前だけが変わった。
**製品側で直せなかったか**: 直せない —— 層そのものを差し替えたので、旧い名前は木に 1 つも無い。

**⚠ 摂動を当てていない面**: `web/` は 1 バイトも触っていないので、web の 3 面も `T-15` も測っていない。
