...
回転だけのtransformはCount1のactual Scoreと反復symbolic planの両方へ配送する。DDL側の`TransformGroupPlan { start, end, rotation_degrees, fixed_position_indices, provenance }`は元の出力順の連続範囲を表し、内側から外側のpostorderで保持する。actual ScoreではScore 0.4.0の`transform_groups`が同じ範囲、角度、数値固定memberの元instruction indexを保持する。空のgroup listはwireへ出ず、保存済み0.1.0／0.2.0／0.3.0とversionなしartifactは従来互換を保つ。`surface_intensity`はScore 0.3.0以後で有効である。反復planでは`PlanRelation { kind, gap, target_object_index, position_authority, touching_constraints }`がchecked relation intentを保つ。回転groupの外へ向かうConnectedは先行を変えず、接続する子と先行を隔てる最外group全体を同じ差分だけ平行移動する。数値固定memberがあるgroupはnonzeroの追加平行移動を許さず、must-fitはその数値memberだけに課し、named memberの既存clippingを変えない。Stopは描画前に停止し、OmitAndContinueでのgroup失敗はgroup全体を省略し、owner・元index・seed・失われた参照を保つ。これはStep11の個体materializationやStep13のruntime / UI / 保存全面接続を完了とする記述ではない。
