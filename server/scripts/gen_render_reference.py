...
REASON = (
    "a surface belongs to the shape that carries it. Of the nine words for how "
    "the inside of a shape looks, `hatch` and `crosshatch` were the only two "
    "that did not stay in their shape: each row was laid at a fixed 1.3x the "
    "bounding box's diagonal and no intersection with the outline was ever "
    "taken, so a circle asked for parallel lines came out as a striped sheet -- "
    "61 to 64 percent of the surface ink fell outside the form, and the "
    "excursion the grain gate measures was 413.9px on a triangle against a "
    "20.0px limit. Every row is now cut against the contour with `_line_spans`, "
    "the helper the fill branch already uses, so a concave form gets one stroke "
    "per span and no row crosses the void; a row that misses the outline draws "
    "nothing. Not a clip path -- `compat` emits none, and a cut only `display` "
    "can see is not a cut. Nothing above the cut moves: the angle, the pitch, "
    "the spacing gradient and the per-row jitter still decide where a row sits "
    "and how it leans, and the printed parallel line stays regular (author, "
    "2026-08-14) -- the `hatch-spacing-*` class values are unchanged in all nine "
    "cases. No case is new. The nine that move are exactly the cases whose "
    "surface texture is `hatch` or `crosshatch`; the six `wash` cases do not "
    "move, and that the wash reads as stripes is a separate ruling with no "
    "approved form yet to draw from."
)
