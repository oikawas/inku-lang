...
The display follows real processing states. It distinguishes photo preparation, on-device model loading, examining the photo, planning the composition, building colors and forms, finishing the performance, and saving the work. Each phase has a short explanation of the current activity. Photo effects operate on a display preview and never alter the input or retained original. Examining the photo layers scan lines, a moving light band, and intermittent signal noise over the preview; finishing progressively assembles many geometric forms. Form, color, and outline effects represent processing phases, not partial drawing results. Text and the source photo stay legible, without rapid full-screen flashes. There are no invented intermediate SVGs, percentages, remaining-time estimates, or delays added for presentation. Direct DDL omits the Stage 1 display it does not execute. With system animations disabled, the same phases have static displays, and the actual work appears after saving completes.
