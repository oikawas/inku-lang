...
- 正規化座標は 0.0〜1.0 のまま維持する。**痕の寸法（`size`）は縦横とも短辺基準で画素へ直し、記述が述べた形をどの比でも保つ**（円・弧の半径も同じ規則。v2.13.6 / render engine 30 で `size` を持つ全図形へ広げた）。**置き場所は従来どおり幅と高さに比例する** —— 縦横比は痕がどこに座るかを決め、痕がどんな形かは決めない
