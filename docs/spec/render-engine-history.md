...
| `ddl_engine_version` | deterministic transforms (expansion, coerce, validator) | `14` | the same input and seed produce different output, **or the declaration order of `Instruction`'s fields changes** |
...
| `APP_VERSION` | the application version | v2.13.11 | every stamping. **`web/APP_VERSION` is the one file that owns it**, and the UI, `/api/info` `version` and the CLI all read it |
...
| `web/BUILD_NUMBER` | build serial | 896 | **moves for UI-only changes too. It is a shared counter, not a per-branch value, so numbers can be skipped. Since v2.9.23 a merge driver named in `.gitattributes` keeps the larger side, so two branches bumping it no longer conflict** (run `scripts/git/setup.sh` once per clone) |
