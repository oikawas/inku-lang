...
- 白 / 黒以外の背景色では、抽出した画像背景色を余白背景に使用する。
...
- この導線は記述 / バッチの描画パネルから使用でき、設定パネル側のキャンバス設定と同じ `canvas_aspect` 設定を更新する。
...
- `描画ごとに色カタログをランダム選択` をデモ設定に追加し、オンの場合は各描画サイクルで `repository.paint` へ渡す色カタログを都度ランダムに選ぶ。
- ランダム色カタログ設定は `demo_random_color_catalog` として Room の `app_settings` に保存し、再起動後も復元する。
