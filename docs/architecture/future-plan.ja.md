...
- **streamの実況** — `/api/paint/stream` は `stage1` と `done` の2 eventしか持たなかった。Build 926で `sketch`（写生の確定時）と `score`（Score確定時）が加わった。cutover後の互換streamは最終結果を`done` 1行で返し、進行はWebの共有pipeline viewが示す（`known-differences.ja.md` F-08）。
