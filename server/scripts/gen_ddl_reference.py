...
REASON = (
    "a description that names one color is drawn in one color. The cycle hands "
    "`cycle[i % len(cycle)]` to each member of a group, so a two-color cycle "
    "gave the color the description named half the members and a color nobody "
    "named the other half. Engine 8 removed the order coerce wrote into the "
    "cycle; the cycle carrying an unrequested color at all survived it. coerce "
    "now reduces such a cycle to the named color alone and sets `color` to it, "
    "reading the "
    "DDL with its background clauses dropped and firing only when exactly one "
    "color is named, no polychrome phrase is present, and the cycle carries "
    "the named color alongside another -- a cycle without it is a delivery "
    "failure, which is a different layer's work. It is reduced rather than "
    "emptied because `_apply_color_cycle` rebuilds `color_hint` and returns "
    "early on an empty cycle: emptying it skips that rebuild, and a stored "
    "Score whose `color_hint` still carries an old machine note then hands the "
    "renderer a color the description never named. None of the 13 expand cases "
    "moved, the change being entirely inside coerce. The score moved in 8 of "
    "the 21 coerce cases; `changed_from_previous` lists all 21 because the "
    "branch report gained a 29th key, which every case carries whether or not "
    "the branch fired."
)
