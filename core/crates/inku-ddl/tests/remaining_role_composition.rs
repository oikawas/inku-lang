...
use inku_ddl::{
    CoreRoleComposition, CoreRoleKind, NeutralDiagnostic, NeutralDiagnosticKind, NeutralTokenKind,
    NormalizedDdlDocument, REMAINING_ROLE_COMPOSITION_SCHEMA_ID, RemainingRoleKind,
    ResolvedInstructionLanguage, SourceSpan, compose_core_roles, compose_remaining_roles,
    parse_neutral_lexemes,
};
use serde::Deserialize;

const FIXTURE: &str = include_str!("fixtures/remaining-role-composition-v2.json");
...
struct Case {
    ...
    language: String,
    source: String,
    expected_core_roles: Vec<ExpectedCoreRole>,
    #[serde(default)]
    expected_core_modifiers: Vec<ExpectedCoreModifier>,
    expected_remaining_roles: Vec<ExpectedRemainingRole>,
    expected_unattached_exact_numbers: Vec<ExpectedExactNumber>,
...
#[test]
fn fixture_moves_remaining_roles_and_numbers_exactly_once() {
    ...
    for case in fixture.cases {
        ...
        let core = compose_core_roles(neutral);
        let input_token_count =
            core.typed_roles.len() + core.core_modifiers.len() + core.deferred_tokens.len();
        let core_before = core.typed_roles.clone();
        let modifiers_before = core.core_modifiers.clone();

        let composition = compose_remaining_roles(core);

        assert_eq!(composition.core_roles, core_before, "{}", case.id);
        assert_eq!(composition.core_modifiers, modifiers_before, "{}", case.id);
        assert_eq!(
            composition
                .core_roles
                .iter()
                .map(project_core_role)
                .collect::<Vec<_>>(),
            case.expected_core_roles,
            "{}",
            case.id
        );
        assert_eq!(
        ...
        assert_eq!(
            composition
                .remaining_roles
                .iter()
                .map(project_remaining_role)
                .collect::<Vec<_>>(),
            case.expected_remaining_roles,
            "{}",
            case.id
        );
        assert_eq!(
            composition
                ...
                .iter()
                .map(|number| ExpectedExactNumber {
                    value: number.value,
                    start_byte: number.span.start_byte,
                    end_byte: number.span.end_byte,
                })
                .collect::<Vec<_>>(),
            case.expected_unattached_exact_numbers,
            "{}",
            case.id
        );
        assert_eq!(
        ...
        assert_eq!(
            input_token_count,
            composition.core_roles.len()
                + composition.core_modifiers.len()
                + composition.remaining_roles.len()
                + composition.unattached_exact_numbers.len()
...
#[test]
fn bilingual_rows_and_exact_numbers_keep_identity_parity() {
    let fixture = load_fixture();
    let project = |case_id: &str| {
        let case = fixture
            .cases
            .iter()
        ...
        .unwrap();
        let composition =
            compose_remaining_roles(compose_core_roles(parse_neutral_lexemes(&document)));
        (
            composition
                .remaining_roles
                .into_iter()
    ...
    let ja = project("ja-mixed-six");
    let en = project("en-mixed-six");
    assert_eq!(ja.0, en.0);
    assert_eq!(ja.1, vec![8, 8]);
    assert_eq!(en.1, vec![8, 8]);
}

#[test]
fn qualitative_holes_and_other_diagnostics_are_forwarded_unchanged() {
    let diagnostics = vec![
        NeutralDiagnostic {
            span: SourceSpan {
                start_byte: 1,
    ...
    let composition = compose_remaining_roles(CoreRoleComposition {
        typed_roles: Vec::new(),
        core_modifiers: Vec::new(),
        deferred_tokens: Vec::new(),
        diagnostics,
        delivery_conservation_count: 2,
    });

    assert!(composition.core_roles.is_empty());
    assert!(composition.remaining_roles.is_empty());
    assert!(composition.unattached_exact_numbers.is_empty());
...
fn fixture_schema_ids_and_required_boundaries_are_guarded() {
    ...
    assert_eq!(
        ...
        "inku.remaining-role-composition.v2"
    );
    assert_eq!(fixture.schema, "inku.remaining-role-composition-fixture.v2");
...
fn project_core_role(term: &inku_ddl::CoreRoleTerm) -> ExpectedCoreRole {
    ExpectedCoreRole {
...
fn project_remaining_role(term: &inku_ddl::RemainingRoleTerm) -> ExpectedRemainingRole {
    ExpectedRemainingRole {
        role: match term.role {
