...
| `APP_VERSION` / `server/pyproject.toml` | the product release | v2.9.18 | per release |
| `web/BUILD_NUMBER` | build serial | 808 | **moves for UI-only changes too. It is a shared counter, not a per-branch value, so numbers can be skipped** |
