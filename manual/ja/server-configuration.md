...
この文書は、未リリース版inku v2.13.42（Web Build 929）を継続運用する管理者向けの設定基準です。環境変数template、現行DB schema、Web管理UI、systemd参照templateを対象にします。
...
管理者は他の利用者のpasswordを変えられます。`設定`の`ユーザー管理`で対象を選び、`ユーザー変更`の`新しいパスワード`へ入力して保存します（**現在のpasswordは要りません**）。`leaders`は自分のscope内の利用者に対して同じことができます。利用者が自分で変えるときは`プロフィール`から現在と新しいpasswordの両方を入れます。**管理者が1人だけで、その人が忘れた場合はこの道が使えません** —— 戻り道は2.2の`inku-admin reset-password`です。
