...
Primitive-only direct coordinated groups with `place:center` use Score 0.7.0 `placement_groups`. Omitted internal placement is `overlap`, which aligns bounding-box centers; explicit “place in a row” is source-order `horizontal_source_order`; explicit “overlap” is `overlap`. The group bounding-box center moves to one named region resolved once from the performance seed while retaining each member's owner, count, seed, and geometry. Repetition remains a symbolic plan; this does not complete instance materialization or full runtime, UI, and save integration.
