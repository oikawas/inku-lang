...
夜の水面に、青い線がゆっくりほどける
```
inkuアプリケーションの入り口は、まず上のような短い詩歌・散文を書く事からはじまります。言葉は、LLMによって絵画要素に分解後、絵画言語DDLに変換されます。DDLはTyped Compilerによって、SVG描画が可能なJSONデータにCompileされます。
...
非決定性を持つのは2カ所のみ: LLMが介在ずる、Stage 1/2だけが非決定的な層です。DDLからJSONへの変換と、Renderer の演奏（描画）は常に決定的で、JSONに保存された情報から、作品を再現できます。ただし、inku の仕様は常に進んでおり、新たな inku が描く作品は常に昔とは違うものです。
...
- **プラグイン** — `Nature.風` などの名前空間付き語彙マクロ。コア語彙への展開のみが許され、コアは変更できません
...
- **Android アプリ** — `2.1.4-android.51`。Kotlin の描画は render engine 35 を実装し、server の render engine 40 へ後から追随する（現在Rendererはrustで共通化されています。コア部分は同じくrustで共通化作業中です）
