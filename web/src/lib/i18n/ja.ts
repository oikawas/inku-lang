...
export const ja: LangPack = {
	...
	tooltipVariationGridDefault: '同じ解釈から、別の演奏2枚と別の構図2枚を並べます。',
	tooltipVariationGridWithInterpretation: '候補4枚に、Stage 1をもう一度読ませた別解釈の候補も加えます。',
	tooltipVariationGridSaveSelected: '選択した候補を履歴に保存し、スターと選んだ理由を残せるようにします。',
	tooltipModelCompare: '同じ記述を別のStage 1モデルでも解釈し、正規化DDLと描画を横並びで見比べます。',
	tooltipDdlAutoRepair: 'Stage 2のJSON Scoreに対して、不可視色・過密・契約違反などの決定的な補正を適用します。',
	...
	tooltipCanvasVaryPerformance: '構図は変えず、線の揺らぎやテクスチャの乱数シードのみを再抽選します',
	tooltipCanvasVaryComposition: '詞書や解釈は変えず、Stage 1.5 の構図レシピや要素配置を選択し直します',
	tooltipCanvasVaryInterpretation: '記述テキストから、もう一度 Stage 1 の解釈 (正規化DDL) をやり直します',
